...
impl GraphRegistry {
    ...
    /// Notify relevant graphs of a commit change on a branch.
    ///
            ...
            graph_ids
                ...
                .filter_map(|graph_id| {
                    queries.get_mut(&graph_id).and_then(|query| {
                        ...
                        // Find the CommitSource node for this branch and process
                        let mut output_delta = DeltaBatch::new();
                        let mut commit_delta_opt: Option<CommitDelta> = None;
                        let mut branch_idx: u8 = 0;

                        // Find CommitSource for this branch and evaluate it
                        for node in query.graph.nodes_mut() {
                            if let QueryNode::CommitSource {
                                table: node_table,
                                branch: node_branch,
                                branch_index,
                                ..
                                ...
                                && node_branch == branch
                            {
                                ...
                                commit_delta_opt = node.evaluate_commit_source(
                                    object_id,
                                    new_frontier.clone(),
                                    tip_contents.clone(),
                                );
                        ...
                        // If CommitSource emitted a delta, route it to BranchMerge
                        if let Some(commit_delta) = commit_delta_opt {
                            for node in query.graph.nodes_mut() {
                                ...
                                {
                                    let row_deltas = node
                                        .evaluate_branch_merge(commit_delta.clone(), branch_idx);
