...
editing and catalogue administration. The dashboard must verify each requested
...
`{appId, capabilities}` and `X-Inspector-CSRF`. Both endpoints require the exact
configured Inspector CORS origin, credentials, and `Cache-Control: no-store`.
...
there is no browser refresh credential. Reload requires sign-in. Dashboard logout
...
command prints a URL containing only app and loopback connection metadata. Open it
...
Origin and Host, binds a single-use code to a browser-generated S256 challenge,
then requires the matching verifier before returning the session. It closes after
