---
title: Client Setup
description: Create Jazz client contexts across frameworks and runtimes, with zero-config defaults and explicit runtime source overrides when needed.
reviewedAt: "4059cdef"
---
...
- The browser quickstarts in these docs are Vite-based, and Jazz works out of the box there with no `runtimeSources` config.
- Expo / React Native uses the native Jazz runtime instead of the browser worker + Wasm asset path. Follow the Expo setup from the [Client quickstart](/docs/quickstarts/client#install) and you usually do not need `runtimeSources`.
- If your browser bundle serves Jazz assets from a custom base path, rewrites module URLs to `file://...`, or otherwise fails to resolve `jazz_wasm_bg.wasm` / `jazz-worker.js`, pass `runtimeSources` explicitly.

<Callout type="info">
  If you are not using the quickstart stack, start with the zero-config setup above. Add
  `runtimeSources` only when your bundler or runtime cannot infer the Wasm and worker locations
  correctly.
</Callout>
...
Use `runtimeSources` when Jazz cannot infer where to load the browser worker or Wasm binary from, or when your runtime wants an in-memory Wasm source/module instead of URLs.
...
| `runtimeSources.workerUrl`  | The browser worker entry has an explicit public URL                         |
...
Some runtimes do not use the browser worker boot path at all. In those cases, prefer `runtimeSources.wasmModule` or `runtimeSources.wasmSource`.
