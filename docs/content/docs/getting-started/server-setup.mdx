...
| `--upstream-url <UPSTREAM_URL>`     | Edge upstream sync is temporarily unsupported while server-to-server sync is migrated to the core engine.                                | `JAZZ_UPSTREAM_URL`           | unset                     |
...
Servers currently refuse to start when `--upstream-url` or `JAZZ_UPSTREAM_URL` is set. This prevents edge deployments from silently using the legacy alpha WebSocket sync transport.
