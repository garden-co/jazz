...
and call `context.forSession(session)`, or pass a mirrored `cookieSession` to the browser client while
the application cookie remains the transport credential.
...
- `context.asBackend()` returns a `Db` opened in explicit backend mode. Its writes are authored as `jazz:system` unless you add explicit attribution; when this process talks to a Jazz server, `backendSecret` additionally authenticates sync/auth.
...
- `context.withAttribution(issuer, userId)` keeps backend permissions, but stamps new writes and updates as that exact issuer/user pair.
...
`forRequest` reads authentication from standard HTTP headers, so `req` can be any object that exposes them: Express, Hono, Fastify, or a Web Fetch API `Request` all work. Configure `jwksUrl` or `jwtPublicKey` on `createJazzContext(...)` if those bearer tokens come from an external IdP, but do not set both at once. Without either one, backend `forRequest()` only accepts Jazz local-first tokens; set `allowLocalFirstAuth: false` to disable those too. It does not currently accept anonymous bearer tokens.
