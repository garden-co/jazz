...
| `--allow-local-first-auth`          | Allow local-first bearer auth (`Authorization: Bearer <self-signed Jazz JWT>`)                                                                     | `JAZZ_ALLOW_LOCAL_FIRST_AUTH` | see `NODE_ENV` note below |
...
Auth defaults depend on `NODE_ENV`:

- `NODE_ENV=production`: anonymous, demo, and local-first auth are disabled unless explicitly enabled
- non-production (`development`, unset, etc.): anonymous, demo, and local-first auth are enabled by default
...
`forRequest` reads authentication from standard HTTP headers, so `req` can be any object that exposes them: Express, Hono, Fastify, or a Web Fetch API `Request` all work. Configure `jwksUrl` on `createJazzContext(...)` if those bearer tokens come from an external IdP. Without `jwksUrl`, backend `forRequest()` only accepts Jazz self-signed tokens; set `allowLocalFirst: false` to disable those too.
