...
| `--upstream-url <UPSTREAM_URL>`     | Run as an edge server connected to the upstream core server. Requires `--peer-secret` and `--admin-secret`.                              | `JAZZ_UPSTREAM_URL`           | unset                     |
| `--peer-secret <PEER_SECRET>`       | Shared server-to-server secret for edge sync. Required when `--upstream-url` is set.                                                     | `JAZZ_PEER_SECRET`            | unset                     |
