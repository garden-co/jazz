...
Every client needs an `appId`. Local-first auth also needs a device secret, which the framework integrations below load or generate. Without local-first credentials, the client runs in anonymous mode and every write is rejected with `AnonymousWriteDeniedError`. Add a `serverUrl` to enable sync. For the full auth matrix (anonymous, local-first, external JWT), see [Authentication](/docs/auth/authentication).
...
>
  <Tab value="React">
    ...
    `auth="local-first"` tells `JazzProvider` to load the app-scoped device secret, or generate one on first run. Descendants can use `useLocalFirstAuth()` for backup, restore, and sign-out flows using that same identity.
  ...
  <Tab value="Solid">
    ...
    `useLocalFirstAuth()` from `jazz-tools/solid` loads the device secret (or generates one on first run) and exposes `login` / `signOut` for switching identity.
...
When you supply `driver: { type: "persistent", dbName }`, that name is one physical local
database. Jazz permanently binds it to the first app/environment/account scope that opens it, so a
different signed-in account is rejected rather than seeing or concurrently opening the old cache.
Use a distinct name per intended cache, or explicitly wipe the old local storage during an account
handoff.
...
<Tabs
  groupId="jazz-framework"
  items={["React (Vite)", "Vue (Vite)", "Svelte (Vite)", "Solid (Vite)", "SvelteKit", "Next.js", "Expo"]}
  persist
  updateAnchor
>
  ...
  <Tab value="Expo">
    ...
    Makes `EXPO_PUBLIC_JAZZ_APP_ID` and `EXPO_PUBLIC_JAZZ_SERVER_URL` available for Metro to inline,
    and returns them as `extra.jazzAppId` and `extra.jazzServerUrl`. Skipped automatically when
    `NODE_ENV=production`.
