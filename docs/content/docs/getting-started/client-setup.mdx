...
>
  <Tab value="React">
    ...
    `auth="local-first"` tells `JazzProvider` to load the device secret, or generate one on first run. Descendants can use `useLocalFirstAuth()` for backup, restore, and sign-out flows using that same identity.
