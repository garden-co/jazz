...
Every client needs an `appId` and an opaque `AccountHandle`. Prepare the account manager outside the context, then restore the selected account or create a local-first account:

```ts
import { createAccountManager } from "jazz-tools";

const scope = { appId: "my-app", serverUrl: "https://core.example" };
const accounts = await createAccountManager(scope);
const account = accounts.getLoggedIn() ?? accounts.createLocalFirst();
const config = { ...scope, account };
```

Creating a local-first account works offline once the runtime assets are available. The registry URL identifies its authority; it does not itself open a network connection. Pass `config` to the framework component below. A context's identity stays fixed: finish `shutdown({ waitForSync: true })` before linking or switching accounts, then create the next context. See [Authentication](/docs/auth/authentication).
...
>
  <Tab value="React">
    ...
    The provider receives the prepared handle. `useAccountState(accounts)` observes login progress and errors outside the context.
  ...
  <Tab value="Vue">
    ...
    Use `useAccountState(accounts)` to observe the shared account manager in Vue.
  ...
  <Tab value="Svelte">
    ...
    Use `accountState(accounts)` for a Svelte store of selection, pending operations, and errors.
  ...
  <Tab value="Solid">
    ...
    Use `createAccountState(accounts)` for a Solid accessor of the shared account state.
...
React Native is supported as an alpha through `jazz-tools/react-native`. For
Expo, prepare the account with `jazz-tools/expo`, then give the resulting
`AccountHandle` to an effect-owned client. Close that client when the effect is
retired.

```tsx
import { createAccountManager } from "jazz-tools/expo";
import { createJazzClient, JazzClientProvider } from "jazz-tools/react-native";

const accounts = await createAccountManager({ appId, serverUrl });
const account = accounts.getLoggedIn() ?? accounts.createLocalFirst();
const client = await createJazzClient({ appId, serverUrl, account });

// Render <JazzClientProvider client={client}>…</JazzClientProvider>.
// On unmount or account change: await client.shutdown();
```
