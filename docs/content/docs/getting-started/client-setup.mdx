...
<Callout type="warn" title="React Native and Expo runtime support is not available">
  The React Native API surface typechecks, but opening persistent storage throws because the SQLite
  driver is not implemented. The browser and Node.js runtime paths are the supported alpha targets.
</Callout>
...
The secret is generated on first load, persisted locally in the browser, and reused on every
subsequent run — the same device keeps the same Jazz identity.
...
Browser clients default to `driver: { type: "persistent" }`, which stores data locally for offline support. Set `driver: { type: "memory" }` to keep data in memory only. A memory database works without a server URL; add one when the in-memory client should sync.
...
For self-hosted deployments, point the server URL at your own host.
...
<Callout type="warn">
  Auto-push covers the development loop. For production, run `pnpm dlx jazz-tools@alpha deploy <appId>` to publish the validated schema and current permissions together. See [Schema changes](/docs/schemas/migrations).
</Callout>
