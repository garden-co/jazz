...
`migrations create` writes a copy-pastable file with `fromHash`, `toHash`, and minimal typed `from` / `to` witnesses. You only need to fill in `migrate(...)`, then rename the file.
...
After review, keep the same hashes and adjust the migration logic as needed:
