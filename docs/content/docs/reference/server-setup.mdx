...
Use the attribution helpers when backend code should keep backend-level access, but row edit
metadata should still record who the work is for.

<include cwd lang="ts">
  ../examples/docs/todo-server-ts/src/request-context.ts#backend-attribution-ts
</include>

This is intentionally different from `forRequest(...)` or `forSession(...)`:

- `forRequest` / `forSession` change both permission evaluation and authorship
- `withAttribution*` only changes authorship
