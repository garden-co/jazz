...
<Callout type="warn">
  Node.js 22.12 or later is required. The MCP server uses `node:sqlite` with SQLite FTS5 for docs
  search and does not provide a fallback for older runtimes.
</Callout>

## Installation

<Tabs
  groupId="jazz-mcp-client"
  items={["Claude Code", "Gemini CLI", "Codex", "OpenCode"]}
  persist
  updateAnchor
>
  ...
  <Tab value="OpenCode">
    <include lang="bash" meta='title="Terminal"'>
      ../../partials/mcp/install.sh#opencode
    </include>
  </Tab>
</Tabs>
...
`getting-started/client-setup`. Use `list_pages` to discover available slugs.
