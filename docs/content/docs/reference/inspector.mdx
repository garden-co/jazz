...
The Jazz Inspector is a standalone client for inspecting a Jazz
...
Open the Inspector deployment configured for your Jazz environment and enter:
...
The inspector can show the schema identity selected by the connected application. Use it to inspect
the current stored shape, policy bundle, subscriptions, and row data.
...
For remote or self-hosted sync servers, use an Inspector deployment that can reach the server
directly.
