...
`insert`, `update`, and `delete` apply locally with no durability guarantee and return a
`MutationResult`. Call `wait({ tier: ... })` on that result when you need confirmation that the
mutation reached a specific durability tier.
