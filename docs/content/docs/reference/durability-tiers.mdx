...
or retrying, when the connection attempt fails, or when a first connection has not come up within
five seconds. If the connection drops while an empty result is waiting, the empty result is
delivered at that moment. After the first result, a subscription behaves exactly like
`"local-first"`, and remote rows appear as they sync.
