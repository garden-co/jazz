...
the worker has one upstream connection, so every attached tab stops waiting for the server on
`"local-first-unless-empty"` reads and remote reads wait until any tab calls `db.reconnect()`.
The worker also tells attached tabs when its live connection comes up, drops, or fails. This is
...
Reads and write waits answer different questions. A strict remote read waits for
Core-authorized coverage of its query. A `wait({ tier: "global" })` confirms Core
acceptance of that particular write; a `local` wait confirms local persistence.

Strict remote subscriptions continue using Core-authorized state after the first
snapshot. Local-first subscriptions can also show cached rows and optimistic local
writes. These read choices do not change how committed writes sync to Core.
