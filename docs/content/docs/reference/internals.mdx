...
hood, the engine also tracks a small set of reserved fields that explain how each row behaves over
time, such as:
...
Conceptually, the whole row-version record lives in one row-format value space that includes both
application columns and reserved engine columns. Some Rust types still expose the application
portion separately for convenience, but that is an implementation detail rather than a different
storage model.
...
3. decodes or reprojects the row payload
