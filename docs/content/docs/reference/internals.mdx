...
Jazz uses a browser broker backed by `SharedWorker`, `MessageChannel`, and the Web Locks API
(`navigator.locks`) to keep one OPFS-backed worker per persistent browser namespace. The broker
elects a leader tab, and that leader owns the dedicated worker. Follower tabs receive broker-routed
message ports to the leader's worker instead of spawning their own OPFS worker.
