...
With `driver: { type: "persistent" }`, Jazz uses a SharedWorker broker to coordinate tabs for the
same database namespace. The broker elects one tab as the leader. That tab creates a dedicated
worker, which opens the core database with `WasmDb.openBrowser(...)`, owns OPFS, and maintains the
server connection. Follower tabs receive message ports from the broker and route their database
traffic through the leader. If the leader closes, the broker promotes another tab.

The main-thread runtime keeps a non-durable client-side peer so that application writes can be
applied synchronously while the durable worker processes them in the background.
...
React Native has compile-level adapter scaffolding with no web worker or OPFS path. In this alpha,
the adapter does not yet connect a native embedded storage backend to the v2 runtime: persistent
startup fails before opening a driver, and the explicit memory path has not been validated under
Metro/Hermes on a device. The table-first model remains the intended native architecture, not a
claim that local database persistence is currently supported.
...
identity. Schema evolution and application branch views are separate mechanisms.
