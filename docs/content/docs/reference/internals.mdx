...
Today those engine-managed fields live alongside the encoded row payload in storage records, so the
runtime does not have to jump between a "table layer" and a separate hidden history system.
...
```text
todos
  visible: (branch, row_id) -> current winner + current payload
