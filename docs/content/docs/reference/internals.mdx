...
server over the direct websocket protocol.

With `driver: { type: "persistent" }`, OPFS requires worker ownership, so Jazz starts one dedicated
module worker for that database namespace. The worker opens the real direct-core database with
`WasmDb.openBrowser(...)`, owns OPFS, and forwards direct-core row/query/write/subscription calls
...
Main thread Jazz API
  ↕ postMessage
Dedicated worker (direct-core OPFS database)
  ↕ WebSocket byte frames
Direct-core Jazz server
```
