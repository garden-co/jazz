...
An explicit `driver.dbName` selects a **physical** IndexedDB namespace; it does not turn that
namespace into a cross-account cache. On its first open Jazz pins a non-secret logical owner
(app, environment, and authentication scope) next to the page-store manifest. Reopening as that
owner, including after the worker restarts, is safe. A different account fails before Jazz opens or
changes any pages, with an error that asks it to choose another name or explicitly reset the local
database. `db.deleteClientStorage()` / logout with `wipeData` destroys the namespace and is the
explicit transfer boundary. The durable owner is neither the foreground replica/node ID nor a
credential; those remain separate identities.
