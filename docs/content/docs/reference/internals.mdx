...
With `driver: { type: "persistent" }`, OPFS requires worker ownership, so Jazz starts one dedicated
module worker for that database namespace. The worker opens the real core database with
`WasmDb.openBrowser(...)`, owns OPFS, and forwards core row/query/write/subscription calls
over `postMessage`. This worker is an OPFS owner, not a second JavaScript engine or compatibility
broker.

```text
Main thread Jazz API
  ↕ postMessage
Dedicated worker (core OPFS database)
  ↕ WebSocket byte frames
Core Jazz server
```
