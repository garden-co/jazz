...
```text
${appId}:${canonicalStringify(options)}:${query._build()}
```

- `appId` comes from `DbConfig`.
- `options` is canonicalised — keys sorted recursively — so `{tier:"edge", localUpdates:"deferred"}` and `{localUpdates:"deferred", tier:"edge"}` collide.
- `query._build()` is a stable JSON serialisation of the `QueryBuilder` (table, conditions, includes, select, order, limit, hops, gather, …).

This contract is what makes server-rendered hydration possible: the server computes the same key with the public `computeQueryKey(appId, query, options)` helper, the orchestrator's `seedSnapshot(key, snapshot)` writes a fulfilled entry under that key, and the matching client-side `useAll(query, options)` call finds it on first render. See the [Hydrate from server-rendered queries](/docs/recipes/server-rendering/hydrate-prerendered-queries) recipe.
