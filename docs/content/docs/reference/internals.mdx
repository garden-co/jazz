...
over `postMessage`. This worker is an OPFS owner, not a second JavaScript engine.
...
React Native bindings currently typecheck, but the persistent SQLite driver is not implemented.
React Native and Expo are not supported runtime targets in this alpha.
...
live subscriptions, including durability-tier gating.
