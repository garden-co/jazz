...
The portable representation is exactly `jazz-auth-v1:<43 unpadded base64url characters>`. It is a
versioned encoding of those 32 bytes—not an additional key derivation. Future formats will use a new
prefix and an explicit verified key-store migration. Rotating this root creates a new author; it
cannot preserve the existing local-first identity. App/profile scope strings are encoded exactly in
the storage-key preimage; only BIP-39 recovery phrases receive Unicode and whitespace normalization.
...
1. The 32-byte **seed** is hashed with SHA-512 after the domain separator
   `jazz-auth-sign-v1`. The first 32 bytes of the digest form the Ed25519 **signing key**.
