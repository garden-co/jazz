...
### Current rows and stable ordering

Queries return current, visible rows. `delete(...)` removes a row from ordinary reads and relation
payloads. To inspect a deleted row explicitly, add `includeDeleted()`:

```ts
const current = await db.all(app.todos);
const deleted = await db.one(app.todos.includeDeleted().where({ id: todoId }));
```

Without `orderBy`, root results use ascending `RowUuid` order. That order is deterministic and is
also the implicit tie-breaker after every explicit `orderBy`, so `limit` and `offset` are stable:

```ts
const page = await db.all(app.todos.orderBy("priority", "desc").offset(20).limit(20));
```
...
For background on how data flows between tiers, see [How Sync Works](/docs/concepts/how-sync-works).
...
| Framework | API                            | Notes                                               |
| --------- | ------------------------------ | --------------------------------------------------- |
| React     | `useAll(query)`                | Returns `{ data, isLoading, error }`                |
