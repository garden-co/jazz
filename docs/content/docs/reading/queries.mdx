...
For values that should not be materialized in full, `select(...)` can return an immutable text or
byte slice, a byte stream, or a JSON-pointer projection. See
[Partial and streaming reads](/docs/schemas/large-values#partial-and-streaming-reads).
