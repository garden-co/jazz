...
| Solid      | `useAll(() => ({ query }))`    | Returns `T[] \| undefined`  |
...
- Solid's `useAll` returns an accessor function, so Solid's fine-grained dependency tracking only re-runs computations that read changed fields.
