...
Branch selection belongs to the database handle, not the query builder. Use `db.branch(id)` when
you want one-shot reads, writes, or `Db` subscriptions to run against a branch:

```ts
const draftDb = db.branch("draft");
const todos = await draftDb.all(app.todos.where({ done: false }));
```

Query builders and `db.all` / `db.one` options do not accept `branch`. Framework hooks expose
branch selection as an adapter option instead, for example `useAll(app.todos, { branch: "draft" })`.
