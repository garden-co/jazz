...
A normal query materializes the complete native value, regardless of how Jazz stores it:

```ts
const document = await db.one(app.documents.where({ id: documentId }));

document?.body; // string
document?.attachment; // Uint8Array
document?.metadata; // parsed JSON
```

`select(...)` can return a primitive text or byte slice, or a JSON-pointer projection. The current
prototype may still materialize the complete value internally before applying that projection. See
[Partial reads](/docs/schemas/large-values#partial-reads).
