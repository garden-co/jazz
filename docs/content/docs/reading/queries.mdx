...
### Own writes and propagation

The read tier also determines how a subscription treats your own local writes:

- `"local-first"` and `"remote-if-possible"` show your own local writes immediately once the subscription has settled at least once.
- `"remote"` defers your own local writes until the server accepts them.
