...
Queries and subscriptions return results as soon as they are available locally by default. A subscription may therefore publish a materialized local preview before its requested tier is ready; `isLoading` remains `true` until the requested first result is ready. Local reads are effectively instant, and remote updates stream in as they arrive, which is normally a good default. When you need a stronger guarantee for the first result, pass a `tier` option to fetch data from a different tier before returning.
...
knowledge&hairsp;—&hairsp;for example, when a user has just navigated to a page and a
...
- `"remote-if-possible"` uses remote scope while connected and falls back to local data only after an explicit app disconnect. A timeout, error, or slow response never triggers fallback.

See [Read tiers](/docs/reference/durability-tiers#read-tiers) for pending-write
...
<Callout type="info" title="Remote subscriptions stay permission-scoped">
  Strict remote reads continue using Core-authorized state after the first result. Local-first reads
  can include cached data and optimistic writes; waiting for a write to reach Core does not change a
  separate subscription's read choice.
</Callout>
...
| Framework  | API                            | Notes                                                                       |
| ---------- | ------------------------------ | --------------------------------------------------------------------------- |
| React/Expo | `useAll(query)`                | Returns `{ data, isLoading, error, highestSettledAt }`                      |
| Vue        | `useAll(query)`                | Returns `{ data, isLoading, error, highestSettledAt }` refs                 |
| Svelte     | `new QuerySubscription(query)` | Exposes reactive `.current` / `.isLoading` / `.error` / `.highestSettledAt` |
| Solid      | `useAll(() => ({ query }))`    | Returns `{ data, isLoading, error, highestSettledAt }`                      |

### The loading state

`useAll`'s `data` field (or Vue's `data` ref and Svelte's `QuerySubscription.current`) may contain a local preview while `isLoading` is `true`. Once the requested first result is ready, `isLoading` becomes `false`; the value is an array&hairsp;—&hairsp;empty (`[]`) if no rows match, or populated with the requested data.

`highestSettledAt` records the highest settlement level observed during the current
subscription lifetime: `"unconfirmed"`, `"local"`, or `"remote"`. It is a historical
milestone, not a promise that every currently displayed row has reached that level.
...
<Tabs groupId="jazz-framework" items={["React", "Vue", "Svelte", "Solid"]} persist updateAnchor>
  ...
  <Tab value="Solid">
    <include cwd lang="tsx">
      ../examples/docs/todo-client-localfirst-solid/src/LiveQueryExample.tsx
    </include>
  </Tab>
</Tabs>
...
<Callout type="info">
  ...
  tier](#read-durability). A local preview can still be available while a stronger tier is loading;
  local storage reads are effectively instant and resolve to `[]` if no data exists yet.
</Callout>
...
- Vue's `useAll` returns `{ data, isLoading, error, highestSettledAt }`, where `data` is a deep `Ref` (not a `shallowRef`), so per-field reactivity composes with the rest of your component tree without manual unwrapping.
- React and Solid return `{ data, isLoading, error, highestSettledAt }`; React returns a fresh `data` array each tick, while Solid's store keeps fine-grained dependency tracking for fields that changed.
...
<Tabs groupId="jazz-framework" items={["React", "Vue", "Svelte", "Solid"]} persist updateAnchor>
  ...
  <Tab value="Solid">
    <include cwd lang="tsx">
      ../examples/docs/todo-client-localfirst-solid/src/ConditionalQueryExample.tsx
    </include>
  </Tab>
</Tabs>
