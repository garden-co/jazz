...
- `"local-first-unless-empty"` behaves like `"local-first"`, except that an empty local result waits for the first remote answer while the server is reachable. Use it to avoid a flash of empty UI on a fresh device. Offline, never-connected, and failed connections get the local result right away.
- `"remote-if-possible"` is a deprecated alias of `"local-first-unless-empty"`.
...
array&hairsp;—&hairsp;empty (`[]`) if no rows match, or populated with the requested data.
