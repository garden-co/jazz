...
tier](#read-durability). A local preview can still be available while a stronger tier is
loading; local storage reads are effectively instant and resolve to `[]` if no data exists yet.
