...
For values that should not be materialized in full, `select(...)` can return a primitive text or
byte slice, or a JSON-pointer projection. See [Partial reads](/docs/schemas/large-values#partial-reads).
