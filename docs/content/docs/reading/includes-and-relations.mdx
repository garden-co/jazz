...
When you `include` a relation, **rows whose `include` target can't be resolved are filtered out** (inner join semantics). This only applies if you explicitly load references using `include`. Otherwise the row appears in results without referenced data as usual.
...
These columns are computed at query time&hairsp;—&hairsp;they don't exist in the database. To use them, opt in explicitly.
