...
6. **Publish**&hairsp;—&hairsp;push the migration to the server:
   ...
   If you also want to publish the current schema and permissions in one step, you can run:
   ...
   previous permissions schema is connected to the new one on the server, tries to push a matching
   local migration if needed, and then publishes the current permissions bundle.
