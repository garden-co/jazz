...
Adding `s.rel("users", "ownerId")` to an existing `s.uuid()` column changes its reference metadata without changing the stored UUID values. The same applies to optional UUID columns and UUID arrays. Generate and publish the migration through the normal workflow: Jazz emits an explicit identity table lens, which preserves row values while making the relation available under the new schema.

The generated schema witnesses include the referenced tables and preserve their structural defaults, including bigint, timestamp, bytes, arrays, JSON, enum, UUID and nullable defaults. Both witnesses must match their canonical schemas exactly; missing or altered defaults are rejected. The automatic reference-addition path requires otherwise unchanged column defaults. Ordinary default-only changes affect future writes and need no row transform; each witness must retain its own endpoint defaults. Branch-bound column invariants still apply. Timestamp defaults must be exactly representable as JavaScript Dates. Generated witnesses for payload-enum and row-valued columns are not yet supported. CLI-computed hashes for defaults and branch bindings match the existing server schema identities; stored identities are unchanged. Existing rows become available through the relation's index when read under the new schema. Changing only a relation alias does not require a row lens. Removing or retargeting a reference, or renaming its column in the same migration, is not supported by this automatic path.
