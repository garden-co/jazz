...
The generated schema witnesses include the referenced tables. If those witnesses contain structural defaults, generation currently stops with an error: author the migration manually, preserving the exact defaults in both witnesses. Existing rows become available through the relation's index when read under the new schema. Changing only a relation alias does not require a row lens. Removing or retargeting a reference, or renaming its column in the same migration, is not supported by this automatic path.
