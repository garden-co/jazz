...
Deploy the schema and permission policy that your running clients use together. During the alpha,
make schema changes deliberately and validate the exact application build before release.
...
each table in the schema, and Jazz uses the same schema for runtime validation, query
...
That means schema hashing, permissions, runtime validation, query planning, inserts,
