...
Note that `col.ref()` columns must be named with an `Id` or `_id` suffix (e.g. `projectId`, `owner_id`). For `col.array(col.ref())`, use an `Ids` or `_ids` suffix instead. The runtime enforces this convention and will throw if a ref column name does not match.
