---
title: Setup
description: Hosted and self-hosted setup, plus runtime context setup across client and backend environments.
---

import CreateJazzClientReference from "../partials/quickstarts/create-jazz-client-reference.mdx";

## 1. Hosted Database Server (From Us)

Current onboarding is manual.

- App IDs are provisioned manually.
- Request provisioning in the `jazz2-adopters` Discord channel.
- Include:
  - app name
  - JWKS endpoint

## 2. Run a Database Server Yourself (CLI)

<include cwd lang="bash">
  ../examples/docs/todo-server-rs/docs/self-host-cli.sh#setup-self-host-cli
</include>

`jazz-tools server` currently supports:

| Option                              | Purpose                                                     | Environment variable   | Default                   |
| ----------------------------------- | ----------------------------------------------------------- | ---------------------- | ------------------------- |
| `-p, --port <PORT>`                 | Listen port                                                 | -                      | `1625`                    |
| `-d, --data-dir <DATA_DIR>`         | Persistent storage directory                                | -                      | `./data`                  |
| `--jwks-url <JWKS_URL>`             | JWKS endpoint for external JWT validation                   | `JAZZ_JWKS_URL`        | unset                     |
| `--allow-anonymous`                 | Allow local anonymous auth (`X-Jazz-Local-Mode: anonymous`) | `JAZZ_ALLOW_ANONYMOUS` | see `NODE_ENV` note below |
| `--allow-demo`                      | Allow local demo auth (`X-Jazz-Local-Mode: demo`)           | `JAZZ_ALLOW_DEMO`      | see `NODE_ENV` note below |
| `--backend-secret <BACKEND_SECRET>` | Enable backend session impersonation                        | `JAZZ_BACKEND_SECRET`  | unset                     |
| `--admin-secret <ADMIN_SECRET>`     | Enable admin operations (schema/policy sync)                | `JAZZ_ADMIN_SECRET`    | unset                     |

Local auth defaults depend on `NODE_ENV`:

- `NODE_ENV=production`: anonymous/demo are disabled unless explicitly enabled (`--allow-anonymous` / `--allow-demo`)
- non-production (`development`, unset, etc.): anonymous/demo are enabled by default

For backend examples, set `JAZZ_SERVER_URL` to your server URL and keep `JAZZ_APP_ID` aligned across clients and backends.

## 3. Context Setup

Use `JAZZ_APP_ID` as the app ID configuration key across environments.
For auth modes and upgrade/link flow, see [Authentication](/docs/authentication).

<Tabs
  groupId="jazz-framework"
  items={["React Client", "Vue Client", "Svelte Client", "TS Client"]}
  persist
>
  <Tab value="React Client">
    <include cwd lang="tsx">
      ../examples/docs/todo-client-localfirst-react/src/setup-snippets.tsx#context-setup-react-minimal
    </include>
  </Tab>
  <Tab value="Vue Client">
    <include cwd lang="vue">
      ../examples/docs/todo-client-localfirst-vue/src/App.vue
    </include>
  </Tab>
  <Tab value="Svelte Client">
    <include cwd lang="svelte">
      ../examples/docs/todo-client-localfirst-svelte/src/App.svelte#provider-svelte
    </include>
  </Tab>
  <Tab value="TS Client">
    <include cwd lang="ts">
      ../examples/docs/todo-client-localfirst-ts/src/main.ts#context-setup-ts-client
    </include>
  </Tab>
</Tabs>

### createJazzClient reference

<CreateJazzClientReference />

In browser clients, omitting local auth fields defaults to anonymous synthetic auth with a persisted local token.
For React Native/Expo clients, make sure `serverUrl` points to a host your runtime can reach (for example Android emulator often uses `10.0.2.2`).

### Backend Context Setup

<Tabs groupId="jazz-environment" items={["TypeScript", "Rust"]} persist>
  <Tab value="TypeScript">
    <include cwd lang="ts">
      ../examples/docs/todo-server-ts/src/main.ts#context-setup-ts-backend
    </include>
  </Tab>
  <Tab value="Rust">
    <include cwd lang="rs">
      ../examples/docs/todo-server-rs/src/main.rs#context-setup-rust-backend
    </include>
  </Tab>
</Tabs>

### Backend Identity Pattern

Client-side usage enforces permissions for the local user context.
Backend usage can either use `context.db()` for the configured runtime identity or derive a requester-scoped `Db` directly from the request.
In TypeScript backends, use `createJazzContext(...)` once at startup, then call `context.forRequest(req)` when you need a bearer-JWT-scoped high-level `Db`.

#### Per-request user-scoped client

<Tabs groupId="jazz-environment" items={["TypeScript", "Rust"]} persist>
  <Tab value="TypeScript">
    <include cwd lang="ts">
      ../examples/docs/todo-server-ts/src/request-context.ts#backend-request-handler-ts
    </include>
  </Tab>
  <Tab value="Rust">
    <include cwd lang="rs">
      ../examples/docs/todo-server-rs/src/docs_snippets.rs#backend-request-handler-rust
    </include>
  </Tab>
</Tabs>
