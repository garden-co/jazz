...
Create selectors by supplying typed values for the named branch columns:

```ts
const main = {
  values: { branch: { type: "String", value: "main" } },
} as const;

const draft = {
  values: { branch: { type: "String", value: "draft" } },
} as const;
...
Pass that view alongside the normal query options:
...
The app reads the row, resolves its base, and supplies concrete selectors to the same read-side API:

```ts
const uuid = (value: string) => ({ type: "Uuid", value }) as const;
const selectBranch = (branchId: string) => ({
  values: { branch_id: uuid(branchId) },
});
...
const head = selectBranch(branchRow.id);
const base = selectBranch(branchRow.base_branch_id);

const view = {
  head,
  base: branchRow.base_snapshot_ref
    ? { kind: "snapshot", branch: base, snapshot: branchRow.base_snapshot_ref }
    : { kind: "current", branch: base },
} as const;

const documents = await db.all(app.documents, { branch: view });
```
