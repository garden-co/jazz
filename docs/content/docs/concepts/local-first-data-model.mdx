...
<Callout type="info" title="Durability Tiers">
  If you need to ensure data is fully up-to-date before displaying it, you can opt in to waiting
  for a higher [durability tier](/docs/reference/durability-tiers).
</Callout>
...
So the runtime keeps a row-local history graph rather than just overwriting the row in place.
