...
Use this pattern for browser `Blob`, `File`, and `ReadableStream` values that are too large or
awkward to keep inline on a single row. Jazz now provides high-level `Db` helpers that chunk
uploads on write and stream file parts back sequentially on read.
...
`file_parts.data` stores the raw chunk bytes. `files.parts` keeps the ordered chunk ids, and
`files.partSizes` stores each chunk's byte length in the same order.

In the example above, `uploads.file` is your app-owned reference to a stored file.

## Add Permissions

<include cwd lang="ts" title="schema/files-and-blobs-permissions.ts">
...
`file_parts` inherit from `files.parts`.
...
<include cwd lang="ts" title="src/files-and-blobs-snippets.ts">
