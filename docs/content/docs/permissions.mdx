...
Validate the local files, inspect the current server head, then publish:

```bash
npx jazz-tools build
npx jazz-tools permissions status
npx jazz-tools permissions push
```

`permissions push` advances a single global permissions head. It fails loudly if the server head has moved since you last checked, so permission versions stay linear.
...
If you want to inspect the currently published auth bundle before changing it, run `npx jazz-tools permissions status`.
