---
title: Storage
description: Content storage, chunking, compression, and persistence backends
status: partial
implements:
...
The storage layer handles large content, memory-constrained devices, and persistent storage backends.

## Implementation Status

**Implemented:**
- ContentRef (Inline up to 1MB, Chunked above 1MB) - threshold is temporary, will be reduced to 1KB when sorted chunk indices are done
...
- Fixed-size chunking (1MB chunks)
- IndexedDB persistence backend for WASM/browser (groove-wasm)

**Not yet implemented:**
- FastCDC (content-defined chunking)
- Native persistence backends (SQLite, RocksDB)
...
enum ContentRef {
    Inline(Box<[u8]>),        // For small content (<= threshold)
    Chunked(Vec<ChunkHash>),  // For larger content
}
```

**Inline threshold**: Currently 1MB (temporary - will be reduced to 1KB when [sorted chunk indices](/docs/internals/indices) are implemented). Below this, store content directly to avoid hash lookup overhead. Most simple table rows will be inline.
...
Compression reduces storage and network transfer overhead.

### Design Considerations

1. **Compress after chunking**: Apply compression to individual chunks rather than before chunking. This preserves CDC deduplication benefits.

2. **Algorithm selection**:
   - **zstd**: Good balance of speed and ratio (~500 MB/s compression, ~1.5 GB/s decompression)
   - **lz4**: Fastest (~3 GB/s), lower ratio
   - Per-chunk algorithm markers for flexibility

3. **Delta encoding**: For commit content, delta-encode against parent commit. Works well for incremental row updates.

### Open Questions

1. Should compression be mandatory or opt-in?
2. Per-object compression settings?
3. Dictionary compression for similar content types?
...
3. **Native backends**: SQLite vs RocksDB for different use cases?

## Files

- `groove/src/storage.rs` - Storage traits
- `groove/src/content.rs` - ContentRef, chunking
