...
- Deleted rows filtered from SELECT queries (row ID removed from table_rows on delete)
...
3. **Per-Table Truncation Policies**: Configure automatic truncation based on age or commit count

## Open Questions

1. **Secondary index for soft-deleted rows (GCO-1064)**: Maintain separate index including deleted objects for "show deleted items" UI, undelete flows, and admin queries.

2. **UPDATE HARD modifier (GCO-1058)**: Should UPDATE support HARD like DELETE? Would allow truncating history after any mutation.
