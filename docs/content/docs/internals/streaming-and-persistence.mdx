...
**Inline threshold**: Currently 1MB (temporary - will be reduced to 1KB when [sorted chunk indices](/docs/internals/sorted-chunk-indices) are implemented). Below this, store content directly to avoid hash lookup overhead. Most simple table rows will be inline.
