...
<Callout type="warn">
  ...
  installable `jazz-rn` native-relay package has a narrow, capability-gated foreground alpha, not a
  general application setup. It requires a matching native development/release build and trusted
  platform admission; Expo Go is unsupported. Do not adapt the browser/TypeScript snippets below to
  React Native. For the current native package boundary, including bare React Native and Expo
  development-build requirements and unsupported operation families, see the [`jazz-rn` install
