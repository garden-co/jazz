...
<Callout type="warn">
  ...
  installable `jazz-rn` native-relay package has verified native packaging and autolinking, but the
  high-level `jazz-tools` foreground runtime adapter is still under restoration. It is therefore
  **not a supported React Native Jazz client**. Do not adapt the browser/TypeScript snippets below
  to React Native. For the current native package boundary, including bare React Native and Expo
  development build requirements, see the [`jazz-rn` install
