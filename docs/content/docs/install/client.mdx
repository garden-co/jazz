...
<Callout type="warn">
  The Expo/React Native binding is compile-level scaffolding in this alpha. Its default persistent
  database configuration and proposal-only `sqliteStorage` option fail before opening a driver.
  Explicit memory mode has not passed Metro/Hermes device validation and is not a supported
