...
<Tabs groupId="jazz-framework" items={["React", "Vue", "Svelte", "Solid", "Expo", "TypeScript"]} persist updateAnchor>
  <Tab value="React">
    ```bash title="Terminal"
...
<Tabs groupId="jazz-framework" items={["React", "Vue", "Svelte", "Solid", "Expo", "TypeScript"]} persist updateAnchor>
  ...
  <Tab value="Expo">
    ...
    `jazz-rn` is the native relay package and must be installed directly alongside
    `jazz-tools`. React Native runtime support is currently under restoration:
    this setup can be prebuilt into a development build, but the embedded Rust
    relay artifact is not available yet, so it is not a usable Jazz client.
    ...
    Keep this import first in the app entry point. It installs `ReadableStream` when React Native
    does not provide it.
    ...
    const config = await withJazz(getDefaultConfig(projectRoot), {
...
<Callout type="warn">
  The Expo/React Native binding is compile-level scaffolding in this alpha. Persistent client
  storage is not supported. Explicit memory mode has not passed Metro/Hermes device validation and
  is not a supported persistence alternative.
</Callout>

For local-first identity on plain React Native, pass an explicit `AuthSecretStore` backed by that
platform's secure storage to `useLocalFirstAuth({ store })`; the React Native binding deliberately
does not fall back to browser `localStorage`. Expo apps can use `ExpoAuthSecretStore` from
`jazz-tools/expo`.
...
<Tabs
  ...
  items={["React", "Vue", "Svelte", "Solid", "Expo", "TypeScript"]}
  persist
  updateAnchor
>
  <Tab value="React">
