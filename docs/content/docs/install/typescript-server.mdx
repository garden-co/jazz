...
[Learn more about schemas and validation](/docs/schemas/defining-tables).
...
<Accordions type="single">
  <Accordion title="What does the driver do?">
    ...
    There is also a `memory` driver, which does not persist data. It works for ephemeral local
    processes; add `serverUrl` when it should sync with an upstream peer that persists data.
