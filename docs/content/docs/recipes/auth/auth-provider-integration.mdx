...
4. On success, Jazz derives the opaque `session.user` author identity from the verified JWT's canonical `iss` and `sub` pair. The complete verified JWT payload, including registered fields, is inspectable through public `session.claims`; those fields do not create another authorship identity.
...
Jazz uses the exact JWT `iss` and `sub` pair as the author identity. It is fine for a provider to keep `sub` fixed to its own user ID: model any link from that identity to your application's user row explicitly.
