...
2. Your client calls `loginJWT({ getToken })` for an existing identity, or explicitly calls `registerJWT` to create its account.
3. The Jazz server validates the JWT signature against the provider's JWKS endpoint.
4. Core resolves the exact `(iss, sub)` assignment and returns an opaque account handle. Pass that handle to the Jazz context. `session.user` contains the account ID and exact acting identity; provider claims remain under `session.claims`. Authenticating an unregistered identity alone never creates an account.
...
>
  <Tab value="Better Auth">
    ...
    Use a token callback to log in or explicitly register, then pass the returned account handle in the `config` prop.
