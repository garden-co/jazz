...
4. On success, Jazz derives the opaque `session.user` author identity from the verified JWT's canonical `iss` and `sub` pair. Registered JWT fields are not exposed through public `session.claims`.
