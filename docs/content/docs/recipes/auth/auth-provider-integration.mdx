...
>
  <Tab value="Better Auth">
    ...
    Own a Jazz session and call `useBetterAuth`. Forms only call Better Auth signup or sign-in; Jazz follows the provider session automatically. Use `auth.logout()` to flush Jazz before revoking the provider session.
