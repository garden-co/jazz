...
The `allowedTo.read("project")` argument is the relationship name explicitly declared on the tasks table, such as `project: s.rel("projects", "projectId")`. Stored column names and inferred aliases are not accepted. See [Permissions](/docs/auth/permissions) for `maxDepth` and recursive inheritance.
