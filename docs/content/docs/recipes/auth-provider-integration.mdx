...
>
  <Tab value="Better Auth">
    ...
    <Callout type="info">
      If your users start unauthenticated and sign up later, see [Local-first auth](/docs/auth/local-first-auth#identity-continuity-with-betterauth) for how to preserve their identity across the transition.
    </Callout>
  ...
  <Tab value="WorkOS">
    ...
    ```bash
    jazz-server --jwks-url https://api.workos.com/sso/jwks/client_01ABC...
    ```
...
Anonymous and demo auth are disabled by default in production. See [Server setup](/docs/reference/server-setup) for the full set of server flags.
