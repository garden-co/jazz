...
If you only changed `permissions.ts`, stop there structurally. That updates the active permission bundle and does not require a migration. Validate with `npx jazz-tools build`, inspect with `npx jazz-tools permissions status`, then publish with `npx jazz-tools permissions push`.
