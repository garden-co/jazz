...
- `beginDirectBatch(...)` / `beginBatch(...)` returns `DbDirectBatch`

Writes made through an open transaction or explicit direct batch are not individually waitable.
Call `commit()` when the batch is complete, then call `.wait({ tier })` on the returned handle.
Call `rollback()` on an open transaction if you want to close that transaction handle without
committing it.
...
```ts
const batch = db.beginBatch(app.todos);
