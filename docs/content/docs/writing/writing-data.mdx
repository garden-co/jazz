...
Use `update(...)` to replace a complete primitive. Use `applyDiffs(...)` when
editing a selected page of a `bytes` or `string` column, or when setting an
existing JSON member. Splice coordinates are relative to `within`, and multiple
splices run in order against the result of the previous splice. The mutation is
still one ordinary row update, so all fields in one call succeed or fail
together.

```ts
db.applyDiffs(app.documents, documentId, {
  bytes: {
...
with the value your application last read. `applyDiffs(...)` is for existing
rows only; inserts and full replacement continue to use the ordinary mutation
methods.
