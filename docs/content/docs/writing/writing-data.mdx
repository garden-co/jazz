...
For ordinary mutations and mergeable transactions, `wait({ tier })` controls which
confirmation the caller waits for:

| Tier     | Resolves when                           |
| -------- | --------------------------------------- |
| `local`  | Saved by the local persistence driver   |
| `global` | Authorized and durably accepted by Core |

<WriteTierDiagram />

Offline, only `local` can resolve. A `global` wait stays pending until the device
reconnects and Core accepts or rejects the queued write. Both choices apply the
initial mutation locally and upload it when connected.
