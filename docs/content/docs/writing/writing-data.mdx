...
A write has both a **fate** and a **durability tier**. Fate is the authority's final accepted or
rejected verdict. Durability tells you how far an accepted write has settled. A local write is
queued for upstream delivery; local durability alone does not prove that an upstream retains it.
...
Offline, only `local` resolves; `edge` and `global` waits stay pending until the device reconnects and the queued write propagates upstream. A device can lose writes that have reached only local durability if its local storage is destroyed.
...
Use an exclusive transaction when reads and writes must commit as one serializable unit. It reads a
stable snapshot, sees its own staged writes, and is rejected if the authority detects a conflicting
change:
...
Use a mergeable transaction when you want to group several convergent writes under one transaction:
...
```ts
let transaction: ReturnType<typeof db.beginTransaction>;
beforeEach(() => {
