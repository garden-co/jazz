...
Passing a `string`, `Uint8Array`, or JSON value replaces the complete column, just as it does for a
small value. Large text and byte columns additionally accept `splice`, while append-only workloads
can use `append`:
...
The edit belongs to the normal row update: it uses the same permissions, transaction, history,
sync, and conflict semantics. `ifVersion` is a general compare-and-swap condition for the complete
row, preventing an edit calculated against stale contents. See
[Editing without materializing](/docs/schemas/large-values#editing-without-materializing) for text,
streams, files, and JSON.
