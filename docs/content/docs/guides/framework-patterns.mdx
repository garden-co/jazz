...
| --------------- | -------------------------------- | --------------------------------- | ------------------------------- |
| Provider        | `<JazzProvider client={client}>` | `<JazzProvider :client="client">` | `<JazzSvelteProvider {client}>` |
...
<Tabs groupId="jazz-framework" items={["React", "Vue", "Svelte"]} persist>
  <Tab value="React">
```tsx
// React requires manual async lifecycle management for the client.
// Vue and Svelte handle this in the provider component.
