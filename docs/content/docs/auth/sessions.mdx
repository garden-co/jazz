...
On clients, writes are automatically attributed to the current session's canonical `[issuer,
user_id]` pair. The pair is represented as canonical JSON at the Jazz boundary and interned only
inside a running core. On backends,
