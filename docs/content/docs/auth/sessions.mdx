...
values. Jazz separately derives the reserved logical `session.user` from the exact `[iss, sub]`
pair. Its portable value is an opaque canonical JSON encoding and is used for transaction authorship, `$createdBy`,
...
`session.user`; use it when an application needs to store or compare an author, rather than
