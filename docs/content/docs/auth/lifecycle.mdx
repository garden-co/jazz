...
- **Database storage**&hairsp;—&hairsp;the local relational database. Browser persistent clients store this
  in IndexedDB. React Native/Expo persistence is available only through the narrow capability-gated
  native foreground alpha; its explicit memory path is
  unverified scaffolding rather than a device-supported runtime. Supported memory drivers keep data
...
`useLocalFirstAuth()` wraps this storage for React, Expo, Vue, and Solid apps. Its `signOut()` method
clears the stored secret, which means the next login without a restored secret creates a different
Jazz identity. Svelte provides the equivalent `LocalFirstAuth` class.
...
In browser persistent mode, pass `wipeData: true` to also clear the local IndexedDB database namespace
before shutdown:
...
This API is only available for browser persistent storage. It asks the SharedWorker to clear the
namespace's IndexedDB database and coordinates across tabs, but intentionally leaves local-first
auth secrets alone.
...
| `useLocalFirstAuth()`             | React, Expo, Vue, and Solid helper for loading, replacing, and clearing the secret |
...
| `db.logout({ wipeData: true })`   | Logout plus browser IndexedDB database wipe                                        |
| `db.deleteClientStorage()`        | Development-only browser IndexedDB storage reset                                   |
