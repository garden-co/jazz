...
- **Identity secret**&hairsp;—&hairsp;the local-first auth secret. In browser apps this is usually stored
  by `BrowserAuthSecretStore` in `localStorage`.
- **Database storage**&hairsp;—&hairsp;the local relational database. Browser persistent clients store this
  in OPFS; memory drivers keep it only for the life of the process.
...
`useLocalFirstAuth()` wraps this storage for browser framework apps. Its `signOut()` method clears the
...
local-first auth secrets from `localStorage` or your external provider's
cookies/tokens.
...
| `useLocalFirstAuth()`             | React hook (and Vue composable) for loading, replacing, and clearing the secret |
