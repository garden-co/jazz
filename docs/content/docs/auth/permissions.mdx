...
shorthand for the same `$createdBy === session.author` condition. `session.author` is the
canonical `[iss,sub]` logical author; `session.user_id` remains the raw provider user id for
application-owned columns. You can still write the raw
