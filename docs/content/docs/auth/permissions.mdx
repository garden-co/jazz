...
Run `pnpm dlx jazz-tools@alpha validate` before publishing to surface warnings about tables with no explicit permission policy. Deploy the validated policy together with the application schema using `pnpm dlx jazz-tools@alpha deploy <appId>`.
