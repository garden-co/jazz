...
Plain objects in a condition are always row predicates. This includes tables with columns such as
`type`, `column`, or `operation` that also appear in Jazz's policy representation. Results from
helpers such as `isCreator`, `allowedTo.*`, `allOf`, and `anyOf` remain directly composable with
those row predicates.

For advanced cases that need manually-authored policy IR, use `raw(...)` to opt in explicitly:
...
canonical `[iss,sub]` logical author. Raw provider identity fields remain available as
`session.claims.iss` and `session.claims.sub` for application-owned columns. You can still write the raw
