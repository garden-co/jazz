...
All four provenance columns are always present. Backend writes that are not attributed to a user are
authored as `jazz:system`.

### Authorship-based policies

You can use provenance magic columns directly in `permissions.ts`. This is useful for simple
