...
canonical `[iss,sub]` logical author. Registered provider identity fields are deliberately absent
from public `session.claims`; applications that need the raw subject in their own columns should
publish it separately as a non-registered custom claim. You can still write the raw `$createdBy`
comparison yourself whenever you want the explicit form.
