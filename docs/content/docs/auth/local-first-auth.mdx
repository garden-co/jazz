...
>
  <Tab value="React">
    ...
    `auth="local-first"` makes `JazzProvider` load or create the secret. Descendants can use
    `useLocalFirstAuth()` for backup, restore, and sign-out flows using the same identity.
  </Tab>
  <Tab value="Vue">
