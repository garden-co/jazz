...
Jazz reads JWT `iss` and `sub` verbatim as `session.issuer` and `session.user_id`; it does not use a custom-claim fallback. Their pair is the stable author identity. Your application can associate that pair with whichever user row shape it chooses instead of rewriting an external provider's `sub`.
