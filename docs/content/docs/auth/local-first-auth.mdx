...
characters: exactly 32 random bytes. Browser, Expo, and native adapters share this representation;
they differ only in their secure-storage mechanism. Their default storage key is a versioned hash of
canonical `{ appId, profile }` scope data, so it contains neither a raw external subject nor a
transient session identifier. `backendSecret` is unrelated deployment-only admission material: never
put it in a client bundle or a Jazz local store.
