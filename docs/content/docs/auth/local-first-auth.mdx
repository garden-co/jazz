...
Jazz keeps JWT `iss` and `sub` verbatim under `session.claims`, and exposes their canonical pair as the opaque `session.user` author identity. It does not use a custom-claim fallback. Your application can associate that identity with whichever user row shape it chooses instead of rewriting an external provider's `sub`.
