...
Jazz stores and exports that root only as `jazz-auth-v1:` followed by 43 unpadded base64url
characters: exactly 32 random bytes. Browser and Expo adapters share this representation while
using their platform's storage mechanism. Plain React Native requires an explicitly injected native
secure-store adapter rather than falling back to browser storage. Their default storage key is a
versioned hash of byte-exact canonical `{ appId, profile }` scope data, so it contains neither a raw
external subject nor a transient session identifier. `backendSecret` is unrelated deployment-only
admission material: never put it in a client bundle or a Jazz local store.
...
Load or create a secret before starting your Jazz client. The framework integrations below store it in the browser (or equivalent on native) so subsequent loads reuse the same account.
...
>
  <Tab value="React">
    ...
    `auth="local-first"` makes `JazzProvider` load or create an `appId`-scoped secret. Descendants
    can use `useLocalFirstAuth()` for backup, restore, and sign-out flows using the same identity.
  </Tab>
  <Tab value="Vue">
  ...
  <Tab value="Solid">
    ...
    `useLocalFirstAuth()` from `jazz-tools/solid` also exposes `login` and `signOut` for switching
...
>
  ...
  <Tab value="Solid">
    <include cwd lang="ts">
      ../examples/docs/todo-client-localfirst-solid/src/auth-snippets.ts#auth-localfirst-solid-backup
    </include>
  </Tab>
  <Tab value="TypeScript">
    <include cwd lang="ts">
...
>
  ...
  <Tab value="Solid">
    <include cwd lang="ts">
      ../examples/docs/todo-client-localfirst-solid/src/auth-snippets.ts#auth-localfirst-solid-restore
    </include>
  </Tab>
  <Tab value="TypeScript">
    <include cwd lang="ts">
...
<Callout type="info">
  ...
  React/Expo/Vue/Solid, `LocalFirstAuth` in Svelte) updates the mounted provider immediately. Plain
...
browser API), typically unlocked with biometrics. `jazz-tools/passkey-backup` stores the secret as
the user handle of a discoverable WebAuthn credential. The browser only returns it after the
authenticator confirms user presence and verification. Jazz does not separately encrypt the secret.
This is not a full WebAuthn authentication flow&hairsp;—&hairsp;Jazz uses the credential to recover the secret,
not to authenticate a challenge to your server.
...
>
  ...
  <Tab value="Solid">
    <include cwd lang="ts">
      ../examples/docs/todo-client-localfirst-solid/src/auth-snippets.ts#auth-localfirst-solid-passkey-backup
    </include>
  </Tab>
  <Tab value="TypeScript">
    <include cwd lang="ts">
...
>
  ...
  <Tab value="Solid">
    <include cwd lang="ts">
      ../examples/docs/todo-client-localfirst-solid/src/auth-snippets.ts#auth-localfirst-solid-passkey-restore
    </include>
  </Tab>
  <Tab value="TypeScript">
    <include cwd lang="ts">
...
Jazz derives the opaque `session.user` author identity from the JWT's exact `iss` and `sub` pair. It does not use a custom-claim fallback. Custom provider metadata belongs at the JWT top level (for example `better_auth_user_id` or `profile_id`) and is exposed under `session.claims`; Jazz does not require or flatten a nested `claims` object. Your application can associate that identity with whichever user row shape it chooses instead of rewriting an external provider's `sub`.
