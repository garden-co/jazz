...
Use `cookieSession` when your browser app authenticates sync with an HttpOnly cookie and therefore
does not have a bearer JWT available in JavaScript. The cookie remains the transport credential;
`cookieSession` is a mirrored Jazz session used for local permission evaluation, `useSession()`, and
`db.getAuthState()`.
...
`cookieSession` is mutually exclusive with `secret` and `jwtToken`. When the cookie-backed session
changes for the same principal, call `db.updateCookieSession(nextSession)`. For sign-in, sign-out,
or a different user, recreate `Db` or `JazzProvider` with a new config.
