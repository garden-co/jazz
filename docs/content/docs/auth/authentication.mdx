...
On TypeScript backends, `await createJazzContext(...).forRequest(req)` follows the same rule set. Configure `jwksUrl` or `jwtPublicKey` on the backend context to verify external JWTs there too, but not both. Application session cookies are resolved by your app server, not by the Jazz CLI server; once you have a Jazz session from an application cookie, use `context.forSession(session)`. If the sync server should authenticate WebSocket upgrades from a Jazz JWT cookie, configure `--auth-cookie-name` / `JAZZ_AUTH_COOKIE_NAME` as described in [Server Setup](/docs/getting-started/server-setup).
