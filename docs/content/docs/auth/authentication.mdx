...
<Callout type="info">
  Policies use `session.user` when comparing `$createdBy` or `$updatedBy`. Non-registered provider
  metadata remains available under `session.claims`; registered transport fields such as `iss` and
  `sub` do not. Authorship identity is the exact `[iss, sub]` pair, so changing either component
  creates a different author.
</Callout>
