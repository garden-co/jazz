...
Jazz requires both JWT `iss` and `sub`. It exposes their admitted values as `session.issuer` and
`session.user_id` (and keeps raw provider claims available), then derives a separate logical
`session.user` from their canonical pair for every write. Two providers may issue the same
...
<Callout type="info">
  Policies use `session.user` when comparing `$createdBy` or `$updatedBy`; `session.user_id` and
  provider claims retain their raw admitted values. Authorship identity is the exact `[iss, sub]`
