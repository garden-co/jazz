...
`session.author` from their canonical pair for every write. Two providers may issue the same
...
<Callout type="info">
  Policies use `session.author` when comparing `$createdBy` or `$updatedBy`; `session.user_id` and
