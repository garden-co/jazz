...
| ------------- | --------------------------------------------------------------------------- | ------------------------------------------------ |
| `anonymous`   | Ephemeral read-only identity with no stored credential                      | Public/marketing surfaces, try-before-anything   |
...
Anonymous sessions can subscribe to queries but are **structurally denied writes**. Client APIs reject inserts, updates, and deletes with `AnonymousWriteDeniedError`; independently, the fate authority rejects raw-wire or relayed commits whose admitted permission subject is anonymous before evaluating table policy. Gate reads the same way you gate any other access, via the permissions DSL (`session.where({ authMode: "anonymous" })`).
...
>
  ...
  <Tab value="Solid">
    <include cwd lang="tsx" meta='title="App.tsx"'>
      ../examples/docs/todo-client-localfirst-solid/src/AuthJwt.tsx
    </include>
  </Tab>
...
Jazz requires both JWT `iss` and `sub`. It derives the logical `session.user` from their canonical
pair for every write. The complete verified JWT payload, including those registered fields, is
inspectable through public `session.claims`, but it never creates a second authorship identity.
Two providers may issue the same `sub` without becoming the same Jazz author; `$createdBy`
distinguishes them by `iss`.
Treat `session.user` as opaque: it is not a user-row reference, display name, or raw `sub`.
...
<Callout type="info">
  Policies use `session.user` when comparing `$createdBy` or `$updatedBy`. The complete verified JWT
  payload, including registered fields such as `iss` and `sub`, remains inspectable under
  `session.claims`; none of those fields is a second authorship path. Authorship identity is the
  exact `[iss, sub]` pair, so changing either component creates a different author.
</Callout>
...
const { secret, isLoading } = useLocalFirstAuth({ appId: "my-app" });
...
```ts
const { authMode, user, claims, error } = useAuthState();
```
...
4. **Anonymous auth**&hairsp;—&hairsp;the request includes a self-signed anonymous bearer token.
5. **No session**&hairsp;—&hairsp;none of the above matched.
...
On TypeScript backends, `await createJazzContext(...).forRequest(req)` accepts external and
local-first bearer tokens, but not anonymous bearer tokens. Configure `jwksUrl` or `jwtPublicKey` on
the backend context to verify external JWTs there too, but not both. Application session cookies are
resolved by your app server, not by the Jazz CLI server; once you have a Jazz session from an
application cookie, use `context.forSession(session)`. If the sync server should authenticate
WebSocket upgrades from a Jazz JWT cookie, configure `--auth-cookie-name` /
`JAZZ_AUTH_COOKIE_NAME` as described in [Server Setup](/docs/getting-started/server-setup).
