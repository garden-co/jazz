...
Registration, login, and linking require a connection to the core. Authenticating with your provider does not implicitly register a Jazz account. An enrollment rejection preserves the previous selection. If enrollment succeeds but local persistence or client startup fails, the new selection remains; use `jazz.retry()` to retry that local work.
