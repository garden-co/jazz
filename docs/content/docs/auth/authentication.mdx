...
For auth-required Better Auth apps, mount `connectBetterAuth(jazz, authClient)` from `jazz-tools/client`, or use `useBetterAuth(jazz, authClient)` from `jazz-tools/react`. Forms only call Better Auth's `signUp` and `signIn`. The connection watches initial hydration and provider session changes, uses atomic login-or-register, and exposes `ready`, `isPending`, `error`, and `retry`. Same-session notifications and token refresh do not recreate the Jazz client.

Use the connection's `logout()` to flush Jazz before Better Auth revokes credentials. Dispose the connection when its owner unmounts; the application still owns and closes the Jazz session. Generic providers can use `connectAuthProvider(jazz, { getToken })` and `update({ key, isPending, error })`, or React's `useAuthProvider`. The key identifies the provider session or exact issuer/subject; it must not be the rotating JWT string. Generic logout accepts the provider signout callback.
...
For a hybrid app, omit the automatic connection. Signup calls the provider and then `jazz.linkJWT(auth)` on the retained guest account; ordinary login calls `loginOrRegisterJWT`. This ensures the fresh identity is linked before any automatic account creation.
