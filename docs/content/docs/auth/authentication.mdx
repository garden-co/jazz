...
Jazz requires both JWT `iss` and `sub`. It keeps their admitted values available as
`session.claims.iss` and `session.claims.sub`, then derives the separate logical `session.user`
from their canonical pair for every write. Two providers may issue the same
...
<Callout type="info">
  Policies use `session.user` when comparing `$createdBy` or `$updatedBy`; provider claims retain
  their raw admitted values under `session.claims`. Authorship identity is the exact `[iss, sub]`
  pair. Changing either component creates a different author.
</Callout>
