...
pair for every write, but does not expose either registered field through public `session.claims`.
Two providers may issue the same
`sub` without becoming the same Jazz author; `$createdBy` distinguishes them by `iss`.
