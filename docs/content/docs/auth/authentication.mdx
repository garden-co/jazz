...
Jazz requires both JWT `iss` and `sub`. It exposes them as `session.issuer` and `session.user_id`,
and records their canonical pair as the author of every write. Two providers may issue the same
...
<Callout type="info">
  Policies can inspect `session.user_id` and application claims. Authorship identity is the exact
  `[iss, sub]` pair. Changing either component creates a different author.
</Callout>
