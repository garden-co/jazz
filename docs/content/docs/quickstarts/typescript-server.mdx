...
- `jwksUrl` verifies external JWTs inside `await context.forRequest(req)`. Without it, the backend only accepts Jazz self-signed tokens unless you set `allowLocalFirst: false`.
...
<Callout type="info">
  ...
  Jazz self-signed dev token, which works because `allowLocalFirst` defaults to `true`. If you want
