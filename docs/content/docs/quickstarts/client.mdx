...
<Accordions type="single">
  <Accordion title="Client Config">
    `appId` identifies your app for storage and sync. For all config options, see [Server
    Setup](/docs/reference/server-setup#client-config-options).
  </Accordion>
</Accordions>
...
Queries return `undefined` while loading, then an array of matching rows. For filtering, sorting, and pagination, see [Queries](/docs/reading/queries).
...
- [Authentication](/docs/auth/authentication)&hairsp;—&hairsp;anonymous, demo, and external JWT auth
