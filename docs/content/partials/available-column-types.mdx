...
Any column can be made nullable with `.optional()` and can have a literal insert default with
`.default(value)`. Non-nullable integers may use `.merge("counter")`; all other columns use
last-writer-wins merging. A non-nullable array may instead use `.merge("g-set")` for a grow-only
set: additions merge by union, and elements cannot be removed.
