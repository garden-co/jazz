There are a number of column types available which cover most common use cases:

| DSL                   | TypeScript type      | SQL type        |
| --------------------- | -------------------- | --------------- |
| `s.string()`          | `string`             | `TEXT`          |
| `s.boolean()`         | `boolean`            | `BOOLEAN`       |
| `s.int()`             | `number`             | `INTEGER`       |
| `s.bigint()`          | `bigint`             | `BIGINT`        |
| `s.float()`           | `number`             | `REAL`          |
| `s.timestamp()`       | `Date`               | `TIMESTAMP`     |
| `s.bytes()`           | `Uint8Array`         | `BYTEA`         |
| `s.uuid()`            | `string` (row ID)    | `UUID`          |
| `s.array(s.string())` | `string[]`           | `TEXT[]`        |
| `s.enum("a", "b")`    | `"a" \| "b"`         | `ENUM('a','b')` |
| `s.json()`            | `JsonValue`          | `JSON`          |
| `s.json(schema)`      | inferred from schema | `JSON`          |

Any column can be made nullable with `.optional()`.

`s.int()` values are signed 32-bit integers from `-2,147,483,648` through `2,147,483,647`. The
runtime rejects fractional, non-finite, and out-of-range values before sending a write to the
native engine. Use `s.float()` when a wider or fractional numeric range is required.

> **JSON columns are atomic.** The entire value is replaced on every write. Use `s.json()` for untyped JSON, or `s.json(schema)` with a [Zod](https://zod.dev)-compatible schema for typed validation. For structured data that needs per-field updates, use separate columns or a related table instead.

> Declare relationships separately in the second argument to `s.table`, using `s.rel("targetTable", "localUuidColumn")` and `s.reverse("sourceTable", "forwardRelationName")`. UUID columns have no naming convention.

JSON pointer selections require a JSON column (`s.json()`). Text columns containing JSON text are not JSON columns. JSON is validated when written; pointer reads can therefore skip unrelated chunks in a large value. Array indices must be canonical (`0`, `1`, …); `/01` does not select array index 1, but can select an object property named `"01"`.
