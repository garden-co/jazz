...
| `localAuthMode`  | no       | Local synthetic auth mode: `"anonymous"` or `"demo"`.                       |
| `localAuthToken` | no       | Explicit local synthetic token override.                                    |
...
| `runtime`        | no       | Runtime bootstrap overrides for browser workers, Wasm URLs, or Wasm input.  |
