...
- `permissions/type-inference.test.ts:188` chains `.where(...)` after `hopTo` with prefixed keys like `"resource_access_edges.resource"`. After the type fix, that test may break and likely needs revisiting on its own merits (the column reference shape post-hop is unclear).
