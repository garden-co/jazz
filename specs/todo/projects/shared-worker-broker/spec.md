...
## Leader-tab keepalive lock

`LOCK_NAME` keeps the Worker observable to other tabs, but it does not keep the **leader tab itself** from being frozen by the browser. Chromium freezes background tabs that have been hidden for several minutes (Memory Saver, freeze-policy heuristics). When the leader tab's main thread is frozen, the dedicated Worker it spawned is frozen with it: from the broker's point of view `LOCK_NAME` is still held, but the OPFS pipeline, upstream WebSocket, and follower MessagePort traffic all stall silently. Followers experience a stuck data plane without a clean re-election trigger.

The browser's documented escape hatch is that a tab holding any `navigator.locks` lock on its **main thread** is treated as doing work and is excluded from freeze policy. We exploit this with a separate, purely-cosmetic lock:

```ts
LEADER_KEEPALIVE_LOCK_NAME = `jazz-leader-keepalive:${appId}:${dbName}`;

let releaseKeepalive: (() => void) | null = null;
navigator.locks.request(
  LEADER_KEEPALIVE_LOCK_NAME,
  { mode: "shared" },
  () =>
    new Promise<void>((resolve) => {
      releaseKeepalive = resolve; // resolved when the tab steps down
    }),
);
```
...
- **Held only by the leader tab's main thread.** Followers do not hold this lock; a frozen follower stalls only its own data plane, which the existing election machinery already tolerates.
- **Shared mode.** This is not a coordination primitive. Shared mode is used precisely so concurrent acquirers (e.g. during a leader transition where an old leader has not yet released and a new one is acquiring) do not contend or queue. Any mode that keeps a lock held on the main thread satisfies the freeze-policy heuristic; shared mode has the cleanest semantics.
- **Not a safety boundary.** `LOCK_NAME` remains the single writer-safety mechanism. The keepalive lock may legitimately fail to acquire, be released early, or be entirely absent on browsers without `navigator.locks` without affecting correctness — only the freeze-resistance hint is lost.
- **Acquired after `LEADER_READY` is sent to the broker** (i.e. once the Worker holds `LOCK_NAME`).
- **Released when the tab is no longer leader**: on `STAND_DOWN`, on observed loss of `LOCK_NAME` (Worker crash), on broker disconnect that drops leadership, or when the tab unloads (browser auto-release). Release calls `releaseKeepalive()` if available.

This does not address iOS Safari BFCache (see flow 8); it specifically mitigates Chromium-style background-tab freezing of the leader.
...
The leader-tab keepalive lock (above) handles Chromium-style freeze policy by signalling that the leader main thread is doing work. iOS Safari BFCache uses a different mechanism that is not always inhibited by a held Web Lock, so the safety-first behaviour below still applies on Safari.
...
| **NEW** `packages/jazz-tools/src/runtime/broker/broker-client.ts`                  | Tab-side facade. Resolves `SharedWorker` URL/name, sends scoped/versioned `HELLO`, exposes `connect()`, `claimLeadership()`, `onLeaderPortChanged(cb)`, `handleAttachFollowerPort(msg)` (transfers broker-minted leader-side port into the Worker, forwards Worker ack/failure), and follower `PEER_PORT` lifecycle. Acquires `LEADER_KEEPALIVE_LOCK_NAME` in shared mode after `LEADER_READY` and releases it on stand-down / disconnect to inhibit Chromium tab-freeze policy on the leader. | medium                                                                                                           |
...
- **`LEADER_KEEPALIVE_LOCK_NAME`** — `jazz-leader-keepalive:${appId}:${dbName}`. Held in shared mode by the leader tab's main thread purely to inhibit Chromium background-tab freezing. Not a coordination primitive; does not gate correctness.
