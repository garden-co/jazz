...
Codebase audit findings. Excludes stale legacy storage driver notes and blob removal (done on `removing-blob-feature` branch).
...
Remaining `#[allow(dead_code)]` are acceptable: Axum extractors, benchmark helpers, test utilities.
