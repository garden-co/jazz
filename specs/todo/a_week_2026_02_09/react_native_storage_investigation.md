...
De-risk synchronous storage and legacy storage on React Native.

## Overview

React Native can't use OPFS or Web Workers. The first step is proving that legacy storage's synchronous storage API works on mobile:

- Compile legacy storage for iOS (xcframework) and Android (JNI/NDK)
...
- legacy storage opens, writes, reads, and flushes on both iOS simulator and Android emulator
