...
The new design replaces `SyncSender` with channel-based communication. `TransportManager` is a standalone async task that talks to `RuntimeCore` exclusively through two channels: an outbound outbox channel and an inbound transport-event channel. It has no reference to RuntimeCore, no lock, no borrow — just channels in, channels out. This eliminates lock contention between the JS thread and the background transport thread (NAPI/RN) and borrow conflicts (WASM).
...
│                                                                  │
│  Recv loop:                                                      │
│    ws.recv() → decode ServerEvent / control frame                │
│             → inbound_tx.send(TransportInbound::...)             │
│             → notify scheduler via tick_notify                   │
│                                                                  │
...
`server_id` is allocated once when the transport is created and stays stable across reconnects. This preserves the current runtime behavior: a reconnect is a re-attachment of the same upstream server, not a brand-new logical upstream.

### TransportInbound

The inbound channel carries both sync payloads and transport control events. This is the missing piece that lets Rust-owned transport preserve today's `addServer()` / `removeServer()` semantics without giving `TransportManager` a `RuntimeCore` reference.

```rust
pub enum TransportInbound {
    /// WebSocket auth handshake succeeded and the upstream server is attached.
    Connected {
        catalogue_state_hash: Option<String>,
        next_sync_seq: Option<u64>,
    },
    /// Regular sync payload from the server event stream.
    Sync {
        entry: InboxEntry,
        sequence: Option<u64>,
    },
    /// Connection dropped after previously being attached.
    Disconnected,
}
```
...
pub fn batched_tick(&mut self) {
    ...
    if let Some(ref mut transport) = self.transport {
        while let Ok(event) = transport.inbound_rx.try_recv() {
            match event {
                TransportInbound::Connected { catalogue_state_hash, next_sync_seq } => {
                    let server_id = transport.server_id;

                    // Preserve today's "reattach" semantics on reconnect:
                    // remove any prior upstream state, then add it back.
...
```rust
/// Platform-specific: notifies the scheduler that inbound transport events arrived.
...
The trait is intentionally **not** `Send`. WASM uses `Rc`-based scheduler state and runs via `spawn_local`, so `!Send` is correct there. Native spawn sites impose `Send` on the concrete `TransportManager<NativeWsStream, NativeTickNotifier>` they launch with `tokio::spawn` or a background thread.
    ...
    pub async fn run(mut self) {
        loop {
            match W::connect(&self.url).await {
                Ok(mut ws) => {
                    let connected = self.perform_auth_handshake(&mut ws).await;
                    let _ = self.inbound_tx.send(TransportInbound::Connected {
                    ...
                    let _ = self.inbound_tx.send(TransportInbound::Disconnected);
                    self.tick.notify();
        ...
        loop {
            select! {
                ...
                frame = ws.recv() => {
                    match frame {
                        Ok(Some(data)) => {
                            match decode_server_event(&data) {
                                ServerEvent::SyncUpdate { seq, payload } => {
                                    let entry = InboxEntry {
                                        source: Source::Server(self.server_id),
                                        payload: *payload,
                                    };
                                    let _ = self.inbound_tx.send(TransportInbound::Sync {
                                        entry,
                                        sequence: seq,
                                    });
                                }
                                ...
                                ServerEvent::Connected { .. } => {
                                    unreachable!("Connected is handled by perform_auth_handshake()");
                                }
                                other => {
                                    let entry = inbox_entry_from_server_event(self.server_id, other);
                                    let _ = self.inbound_tx.send(TransportInbound::Sync {
                                        entry,
                                        sequence: None,
                                    });
...
RuntimeCore.batched_tick()
  ...
  └── for each OutboxEntry:
        transport.outbox_tx.send(entry)
...
```
Server sends regular `ServerEvent` traffic (`SyncUpdate`, `Subscribed`, etc.)
  │
  ▼
TransportManager recv loop:
  ws.recv() → binary frame
  ├── decode ServerEvent
  ├── inbound_tx.send(TransportInbound::Sync {
  │       entry: InboxEntry,
  │       sequence: seq,
  │   })                             [channel push]
...
RuntimeCore.batched_tick()
  ...
  │     └── park each Sync event, preserving sequence when present
...
### Flow 3: Upstream attach / detach semantics

Today the JS stream layer calls `addServer()` when the stream first sends `Connected`, and `removeServer()` when the stream disconnects. The Rust-owned transport must preserve exactly that behavior or catalogue replay and server sequencing drift.

With `TransportInbound`, those lifecycle transitions become explicit:

```
TransportManager handshake succeeds
  │
  ├── inbound_tx.send(TransportInbound::Connected {
  ...
  ▼
RuntimeCore.batched_tick()
  ├── remove_server(server_id)                 [reattach semantics]
  ├── add_server_with_catalogue_state_hash(...)
  └── set_next_expected_server_sequence(...)

... later ...

TransportManager detects disconnect
  │
  ├── inbound_tx.send(TransportInbound::Disconnected)
  └── tick.notify()
  │
  ▼
RuntimeCore.batched_tick()
...
This is the key detail that keeps reconnect behavior equivalent to today's `addServer()` / `removeServer()` flow while moving transport ownership into Rust.

### Flow 4: Connection lifecycle
...
Platform crate creates:
  1. TransportHandle (stable server_id + channels) → given to RuntimeCore
...
### Flow 5: Authentication handshake
...
`TransportManager` consumes this first `Connected` frame itself and converts it into `TransportInbound::Connected`. That is why the regular recv loop can treat later `Connected` frames as unreachable.
...
TransportManager detects:
  ...
  │
  ├── inbound_tx.send(TransportInbound::Disconnected)
  │   [detaches upstream server state before backoff]
  │
  ├── Outbox channel stays alive — messages buffer
  │   (`outbox_tx.send()` doesn't fail, just queues)
  │
...
W::connect(url) succeeds
  │
  ├── perform_auth_handshake() — same client_id for incremental sync
  ├── inbound_tx.send(TransportInbound::Connected { ... })
...
4. ...time passes, server processes, sends response...
   ...
   ────────────────────────────────────►
                                          ...
                                          ├── decodes frame → TransportInbound::Sync
                                          ├── inbound_tx.send(event)
                                          │   [pushes to channel — non-blocking]
                                          └── tick.notify()
                                              └── scheduler.schedule_batched_tick()
                                                  └── spawn_local(tick_future)
...
5. JS event loop picks up batched_tick future.
   ────────────────────────────────────►
                                          batched_tick() runs:
                                          ├── inbound_rx.try_recv() → got event
                                          │   parks sync payload with sequence
                                          ├── handle_sync_messages()
                                          ├── immediate_tick()
...
In NAPI and React Native, `TransportManager` runs on a real async runtime (`tokio::spawn` or a background thread). Because TransportManager only touches channels (no RuntimeCore reference), there is **zero lock contention** between the JS thread and the background transport thread. The native concrete types are `Send`: `OutboxEntry` and `TransportInbound` are plain data, and `NativeTickNotifier` uses thread-safe scheduler handles.
...
async fn handle_ws_connection(mut socket: WebSocket, state: AppState) {
    ...
    // 4. Send Connected response (handshake success)
...
On authentication failure during the handshake, the server does **not** register the upstream server connection at all. It returns an auth error and closes the socket before entering the bidirectional loop.
...
- **TransportManager stays execution-model agnostic** — on native platforms the concrete manager is `Send`; on WASM it is intentionally `!Send` and runs with `spawn_local`.
...
**Inbound channel:** TransportManager pushes both sync payloads and transport lifecycle events to a channel. `batched_tick()` drains it via `try_recv()`, reattaches/removes the upstream server when needed, and parks sync messages (with sequence when present). The `TickNotifier` tells the scheduler to run `batched_tick()` when new messages arrive.
...
**What stays:** `onSyncMessageReceived()` on the main thread runtime — it is still called by the worker bridge (via `postMessage`), not by network code. Similarly, `onSyncMessageToSend()` callback stays for the worker bridge's main-to-worker direction (main thread mutations need to reach the worker for server sync). Shared JS helpers that are about **worker bridging** rather than **server transport** may stay in JS.
...
| --------------------------------------------------- | ------------------------------------------------- | -------------------------------------------------------------- |
| `JsSyncSender` (jazz-wasm, **worker runtime only**) | WASM callback-based sync sender for server sync   | Replaced by `TransportHandle` channels                         |
...
| `park_sync_message()` / `park_sync_message_with_sequence()` | Inbound message parking stays — `batched_tick()` still uses them after decoding transport events    |
| `SyncManager` / outbox / inbox                              | Sync logic is transport-agnostic                                                                    |
| Binary frame format (4-byte length + JSON)                  | Wire format is unchanged                                                                            |
| `transport_protocol.rs` sync payload types                  | `SyncBatchRequest`, `SyncPayload`, and regular `ServerEvent` sync traffic stay structurally aligned |
| Auth logic in `middleware/auth.rs`                          | Reused in WebSocket handshake handler                                                               |
| Browser worker bridge (`postMessage`)                       | Still needed for main-thread runtime ↔ worker runtime sync                                          |
| `onSyncMessageReceived()` / `onSyncMessageToSend()`         | Still needed on the browser side for inter-runtime delivery                                         |
...
- New: reconnect lifecycle test — `Connected` reattaches the same upstream server ID and `Disconnected` removes it
