...
## Follow-Up: Remaining Native Write Hot Path

After the prepared write-context cleanup, the focused 100k insert benchmark no
longer calls runtime `getSchema()` or `getSchemaHash()` from TypeScript. The
remaining native profile weight is mostly ordinary row mutation work:

- `apply_local_row_history_write -> apply_row_batch_with_context ->
apply_prepared_row_mutation -> apply_index_mutations -> index_insert`
- `apply_row_batch_with_context -> encode_flat_history_row_with_codecs ->
decode_row_with_layout`
- `apply_row_batch_with_context -> encode_flat_visible_row_entry_with_codecs ->
decode_row_with_layout`
- `load_local_batch_record -> decode_local_batch_record_with_branch_ords ->
decode_row/compiled_row_layout/hash_row_descriptor`

The next larger cleanup should avoid decoding the freshly encoded user row into
`Vec<Value>` just to construct flat history and visible rows. A cleaner API
would encode flat rows from the prepared user row bytes plus the prepared flat
history/visible descriptors, filling only the reserved Jazz columns as new
materialized values.

A second follow-up is to give storage system rows prepared static codecs. Local
batch record decode still uses generic descriptor-based `decode_row` in the
profile, which means layout compilation and descriptor hashing can reappear even
after the application table write path is prepared.

Finally, index application is now prominent enough to deserve its own pass.
Future profiling should isolate key encoding, `BTreeMap` insertion/memcmp cost
in memory storage, and whether repeated sequential writes should coalesce or
batch index mutations more aggressively before touching raw tables.
