...
- [**catalogue-state-hash-recomputed-on-every-call**](todo/issues/catalogue-state-hash-recomputed-on-every-call.md) — `SchemaManager::catalogue_state_hash()` collects known schemas, sorts them, encodes each with `encode_schema`, and feeds the bytes into blake3 on every call. Under a reconnect storm of N clients the server pays N full catalogue rehashes.
