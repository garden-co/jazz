...
- [**hopto-wrong-type-inference**](todo/issues/hopto-wrong-type-inference.md) — `Query.hopTo("relation")` keeps the source table's row type instead of switching to the destination, so any consumer that infers from `QueryBuilder<T>` (e.g. svelte `QuerySubscription`, `RowOf<>`) sees the wrong row type.
