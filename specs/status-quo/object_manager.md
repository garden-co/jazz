...
- `FjallStorage`: `crates/groove/src/storage/fjall.rs` (native durable storage)
...
4. **Pluggable storage via trait**: `Storage` trait decouples ObjectManager from backend (MemoryStorage, FjallStorage, OpfsBTreeStorage). The same ObjectManager code runs in browser WASM and native Rust.
