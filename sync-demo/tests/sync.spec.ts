...
test.describe("Groove Sync", () => {
  ...
  test("two tabs can connect to sync server", async ({ browser }) => {
    ...
    try {
      ...
      await expect(page1.getByTestId("status")).toContainText("Ready", {
        timeout: 10000,
      });
      await expect(page2.getByTestId("status")).toContainText("Ready", {
        timeout: 10000,
      });
      ...
      // Wait for connection
      await expect(page1.getByTestId("log")).toContainText("Connected", {
        timeout: 10000,
      });
      await expect(page2.getByTestId("log")).toContainText("Connected", {
        timeout: 10000,
      });
    } finally {
  ...
  test("insert is visible locally", async ({ page }) => {
    ...
    await expect(page.getByTestId("tableContents")).toContainText("Item-", {
      timeout: 5000,
    });
  });
  ...
  test("new subscriber receives existing data", async ({ browser }) => {
    ...
    try {
      ...
      await expect(page1.getByTestId("tableContents")).toContainText("Item-", {
        timeout: 5000,
      });
      ...
      await expect(page2.getByTestId("tableContents")).toContainText("Item-", {
        timeout: 5000,
      });
    } finally {
      await context1.close();
