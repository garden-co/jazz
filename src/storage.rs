...
#[cfg(all(target_os = "linux", feature = "spdk", not(target_arch = "wasm32")))]
...
impl LeafStorage {
    pub(crate) fn new(config: Arc<Config>, buffer_ptr: Option<*mut u8>) -> Self {
        ...
        Self::new_with_vfs(config, buffer_ptr, vfs)
    }

    /// Create LeafStorage with a custom VFS instance.
    /// Used by WASM to create storage with async-initialized OPFS VFS.
    pub(crate) fn new_with_vfs(
        config: Arc<Config>,
...
pub(crate) fn make_vfs(
    ...
    match storage_backend {
        ...
        StorageBackend::Opfs => {
            // OPFS requires async initialization, so this path shouldn't be used directly.
            // Use the wasm::open_tree_with_opfs async function instead.
            panic!("OPFS backend requires async initialization. Use wasm::open_tree_with_opfs instead.")
        }
    }
}

/// Create a VFS from an already-initialized OpfsVfs instance (for WASM).
#[cfg(target_arch = "wasm32")]
pub(crate) fn make_vfs_from_opfs(opfs: crate::fs::OpfsVfs) -> Arc<dyn VfsImpl> {
    Arc::new(opfs)
}
