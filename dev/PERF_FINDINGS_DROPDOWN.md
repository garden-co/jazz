...
## Retired window-consolidation evidence

The following historical investigation is archived as **rejected / not implemented** evidence. The window codec, its scheduling, cursor, range-index, and convergence mechanisms were removed in the 2026-08-09 row-only storage format cut. Its prior CPU receipts do not describe the current architecture and must not be used to justify retaining compatibility readers or maintenance hooks.
