...
const codec = {
  ...
  decode(bytes: Uint8Array) {
    if (bytes[0] === 0 && bytes[1] === NATIVE_RELAY_ABI_V2)
      return { type: "probe" as const, abiVersion: NATIVE_RELAY_ABI_V2 };
...
test("foreground receipt sends the v1 Probe, Tick, and Close byte commands", () => {
  ...
  const foreground = {
    execute(command: Uint8Array) {
      ...
      if (command[0] === 0) return Uint8Array.of(0, NATIVE_RELAY_ABI_V2);
...
test("foreground receipt rejects a live alias after claimed revocation", () => {
  const liveForeground = {
    execute: () => Uint8Array.of(0, NATIVE_RELAY_ABI_V2),
    tick() {},
    close: () => false,
  };
  assert.throws(
...
test("foreground ABI mismatch records its boundary before any open", () => {
  ...
  assert.throws(
    () =>
      proveForegroundByteAbi(
        {
          abiVersion: NATIVE_RELAY_ABI_V2 + 1,
          openAttached() {
