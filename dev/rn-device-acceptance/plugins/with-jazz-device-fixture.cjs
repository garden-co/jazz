...
function injectAndroidBuildConfig(root) {
  ...
  const marker = 'buildConfigField "String", "REACT_NATIVE_RELEASE_LEVEL", "\\"${findProperty(\'reactNativeReleaseLevel\') ?: \'stable\'}\\""';
  const fields = [
  ...
  if (!source.includes(marker)) throw new Error("Expo app build.gradle no longer has the BuildConfig insertion marker");
...
module.exports = function withJazzDeviceFixture(config) {
  ...
  config = copyTemplate(config, "android", "android/JazzDeviceFixtureModule.kt", `${androidSource}JazzDeviceFixtureModule.kt`);
  config = copyTemplate(config, "android", "android/JazzDeviceFixturePackage.kt", `${androidSource}JazzDeviceFixturePackage.kt`, (root) => {
    ...
    if (!source.includes("add(JazzDeviceFixturePackage())")) {
      if (!source.includes(marker)) throw new Error("Expo MainApplication template no longer has the fixture package marker");
      fs.writeFileSync(mainApplication, source.replace(marker, `${marker}\n              add(JazzDeviceFixturePackage())`));
    }
