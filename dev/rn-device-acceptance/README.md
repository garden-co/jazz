...
Run source checks with `pnpm --filter rn-device-acceptance verify`. After a real development build exists, the drivers require `JAZZ_DEVICE_APK` (Android) or `IOS_SIMULATOR_UDID` plus `JAZZ_DEVICE_APP` (iOS). They fail if the app emits no platform receipt or any scenario is still TODO/blocked/failed; a green process cannot be manufactured by a fixture log line.
