...
export function assertFixtureNetworkPolicy(manifest, policy) {
  ...
  if (
    ...
    !/<domain-config\s+cleartextTrafficPermitted="true">\s*<domain\s+includeSubdomains="false">10\.0\.2\.2<\/domain>\s*<\/domain-config>/.test(
      xml,
    ) ||
...
export function verifyAndroidReleaseNetworkPolicy(projectRoot) {
  ...
  const manifests = readdirSync(mergedRoot, { recursive: true }).filter((path) =>
  ...
  assertFixtureNetworkPolicy(
    ...
    readFileSync(
      join(projectRoot, "app/src/main/res/xml/jazz_device_network_security.xml"),
      "utf8",
    ),
  );
}
