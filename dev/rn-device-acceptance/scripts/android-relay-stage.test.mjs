...
function staged({ omit = false } = {}) {
  ...
  const files = androidRelayFiles.filter((_, index) => !omit || index !== 0).map((file) => {
  ...
  writeFileSync(join(root, "android/jazz-native-relay.manifest.json"), JSON.stringify({
    format: 2, nativeRelayAbi: 3, sourceRevision: revision, files,
  }));
  return root;
}

test("accepts a staged manifest only with the exact four Android ABI libraries", () => {
  assert.equal(verifyAndroidRelayStage({ packageRoot: staged(), sourceRevision: revision }).files.length, 4);
});
test("rejects a manifest that stages only the emulator ABI", () => {
  assert.throws(() => verifyAndroidRelayStage({ packageRoot: staged({ omit: true }), sourceRevision: revision }));
});
test("rejects a staged manifest from another source revision", () => {
  assert.throws(() => verifyAndroidRelayStage({ packageRoot: staged(), sourceRevision: "b".repeat(40) }));
});
