...
const nativeExpectation = (exportName, fingerprint) =>
...
function fixture(label, wasmFingerprint, napiFingerprint) {
  ...
  for (const [path, value] of [
    ...
    [
      ...
      nativeExpectation("EXPECTED_WASM_ARTIFACT_FINGERPRINT", wasmFingerprint),
    ],
    [
      join(root, "packages", "jazz-tools", "src", "runtime", "native-artifact-fingerprint-napi.ts"),
      nativeExpectation("EXPECTED_NAPI_ARTIFACT_FINGERPRINT", napiFingerprint),
    ],
  ]) {
...
test("producer manifest binds sealed artifacts to every relevant source input", () => {
  ...
  try {
    ...
    const consumerEnvironment = correctnessArtifactConsumerEnvironment(root);

    // The generated expectations are producer byproducts and are excluded
    // from source identity. Restoring a release expectation must therefore
    // be rejected by the sealed-snapshot admission itself, rather than later
    // surfacing as a confusing WASM ABI mismatch inside a browser test.
    writeFileSync(
      join(root, "packages/jazz-tools/src/runtime/native-artifact-fingerprint-wasm.ts"),
      nativeExpectation("EXPECTED_WASM_ARTIFACT_FINGERPRINT", "f".repeat(64)),
    );
    assert.throws(
      () => correctnessArtifactConsumerEnvironment(root),
      /generated WASM fingerprint expectation differs from sealed snapshot/,
    );
    assert.throws(
