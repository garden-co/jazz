...
function topLevelBlocks(source, identifier) {
  const blocks = [];
  let depth = 0;
  let state = "code";
  let quote = "";
  for (let index = 0; index < source.length; index++) {
    ...
    if (
      ...
      const opening = index + identifier.length + (source.slice(index + identifier.length).match(/^\s*/) ?? [""])[0].length;
      if (source[opening] === "{") blocks.push(source.slice(index, matchingBrace(source, opening)));
    }
    if (character === "{") depth++;
    if (character === "}") depth--;
  }
...
function topLevelFunction(source, declaration) {
  ...
  for (let index = start; index < source.length; index++) {
    ...
    if (character === ")") parentheses--;
    if (character === "{" && parentheses === 0) return source.slice(start, matchingBrace(source, index));
  }
  throw new Error(`missing body for ${declaration}`);
}

test("jazz-rn Android resolves Kotlin plugin and stdlib versions through the same override", async () => {
  const source = await readFile(
    new URL("../../../crates/jazz-rn/android/build.gradle", import.meta.url),
    "utf8",
  );
  const buildscript = topLevelBlocks(source, "buildscript")[0];
  const dependencies = topLevelBlocks(source, "dependencies").at(-1);
  const helper = topLevelFunction(source, "def getExtOrDefault");
  assert.ok(buildscript && dependencies, "Android build script must expose its version DSL");

  const directory = await mkdtemp(join(tmpdir(), "jazz-rn-kotlin-contract-"));
  try {
    await writeFile(join(directory, "settings.gradle"), 'rootProject.name = "kotlin-contract"\n');
    await mkdir(join(directory, "contract"));
    await writeFile(join(directory, "contract/buildscript.gradle"), buildscript);
    await writeFile(join(directory, "contract/dependencies.gradle"), `${helper}\n${dependencies}\n`);
    await writeFile(
      join(directory, "build.gradle"),
      `class ExtValues {
  final Map values
  ExtValues(Map values) { this.values = values }
  boolean has(String name) { values.containsKey(name) }
  Object get(String name) { values[name] }
}
class RootProjectStub {
  final ExtValues ext
  RootProjectStub(Map values) { ext = new ExtValues(values) }
}
class ProjectStub {
  final Map properties
  ProjectStub(Map properties) { this.properties = properties }
}
class DependencyCollector {
  final List classpaths = []
  final List implementations = []
  void classpath(Object notation) { classpaths << notation.toString() }
  void implementation(Object notation) { implementations << notation.toString() }
}
class ContractDsl {
  final DependencyCollector collector
  ContractDsl(DependencyCollector collector) { this.collector = collector }
  void configure(Closure body, Object delegate) {
    body.delegate = delegate
    body.resolveStrategy = Closure.DELEGATE_FIRST
    body.call()
  }
  void repositories(Closure body) { configure(body, this) }
  void google() {}
  void mavenCentral() {}
  void dependencies(Closure body) { configure(body, collector) }
}

def sourceDirectory = file("contract")
def buildscriptSource = new File(sourceDirectory, "buildscript.gradle").text
def dependenciesSource = new File(sourceDirectory, "dependencies.gradle").text

def evaluateBuildscript = { Map rootValues, Map projectValues ->
  def collector = new DependencyCollector()
  def dsl = new ContractDsl(collector)
  def binding = new Binding([
    rootProject: new RootProjectStub(rootValues),
    project: new ProjectStub(projectValues),
    buildscript: { Closure body -> dsl.configure(body, dsl) },
  ])
  new GroovyShell(binding).evaluate(buildscriptSource)
  collector
}

def evaluateStdlib = { Map rootValues, Map projectValues ->
  def collector = new DependencyCollector()
  def dsl = new ContractDsl(collector)
  def binding = new Binding([
    rootProject: new RootProjectStub(rootValues),
    project: new ProjectStub(projectValues),
    dependencies: { Closure body -> dsl.configure(body, collector) },
  ])
  new GroovyShell(binding).evaluate(dependenciesSource)
  collector
}

tasks.register("assertKotlinVersionContract") {
  doLast {
    def declaredVersion = "2.0.21"
    def overrideVersion = "1.9.24"
    def defaultPlugin = evaluateBuildscript([:], [JazzRn_kotlinVersion: declaredVersion])
    def defaultStdlib = evaluateStdlib([:], [JazzRn_kotlinVersion: declaredVersion])
    assert defaultPlugin.classpaths == [
      "com.android.tools.build:gradle:7.2.1",
      "org.jetbrains.kotlin:kotlin-gradle-plugin:" + declaredVersion,
    ], "absent root override must use JazzRn_kotlinVersion for the plugin"
    assert defaultStdlib.implementations == [
      "org.jetbrains.kotlin:kotlin-stdlib:" + declaredVersion,
    ], "absent root override must use JazzRn_kotlinVersion for stdlib"

    def overriddenPlugin = evaluateBuildscript([kotlinVersion: overrideVersion], [JazzRn_kotlinVersion: declaredVersion])
    def overriddenStdlib = evaluateStdlib([kotlinVersion: overrideVersion], [JazzRn_kotlinVersion: declaredVersion])
    assert overriddenPlugin.classpaths[1] ==
      "org.jetbrains.kotlin:kotlin-gradle-plugin:" + overrideVersion
    assert overriddenStdlib.implementations == [
      "org.jetbrains.kotlin:kotlin-stdlib:" + overrideVersion,
    ], "a root kotlinVersion override must win for stdlib"
  }
}
`,
    );
    ...
    execFileSync(
      gradle,
      ["--offline", "--no-daemon", "-p", directory, "assertKotlinVersionContract"],
      { stdio: "inherit" },
    );
  } finally {
