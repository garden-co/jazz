...
browser_tests_command=${JAZZ_BROWSER_TEST_COMMAND:-"pnpm --recursive --workspace-concurrency=2 --filter jazz-tools --filter inspector --filter band-chat-nextjs-betterauth --filter record-player-next-betterauth --filter auth-workos-chat test:browser"}
node_tests_pid=""
browser_tests_pid=""
log_monitor_pid=""
