...
function runId() {
  return (
    process.env.JAZZ_RECEIPT_RUN_ID ??
    new Date()
      .toISOString()
      .replace(/[-:]/g, "")
      .replace(/\.\d{3}Z$/, "Z")
...
function parseJsonDocuments(text) {
  ...
  try {
    ...
    const lineRows = text.split(/\r?\n/).flatMap((line) => {
      try {
        return rowsFromJson(JSON.parse(line));
      } catch {
        return [];
      }
    });
    ...
    for (let start = 0; start < text.length; start += 1) {
      ...
      for (let end = start; end < text.length; end += 1) {
        ...
        if (depth !== 0) continue;
        try {
          return rowsFromJson(JSON.parse(text.slice(start, end + 1)));
        } catch {
          break;
        }
      }
...
function criterionRows(startedAt) {
  ...
  const rows = [];
  for (const estimatesFile of walk(criterionRoot).filter((file) =>
    file.endsWith("/new/estimates.json"),
  )) {
    ...
    try {
      ...
      const sample = fs.existsSync(sampleFile)
        ? JSON.parse(fs.readFileSync(sampleFile, "utf8"))
        : null;
      const perIteration =
        sample?.times
          ?.map((time, index) => time / sample.iters[index])
          .filter(Number.isFinite)
          .sort((a, b) => a - b) ?? [];
      const samples = perIteration.length || null;
      const p95 = perIteration.length
        ? perIteration[Math.ceil(perIteration.length * 0.95) - 1]
        : null;
      const relative = path.relative(
        path.join(root, "target/criterion"),
        path.dirname(path.dirname(estimatesFile)),
      );
      rows.push({
        phase: "criterion",
        benchmark: relative,
        unit: "ns",
        p50_ns: estimates.median?.point_estimate ?? null,
        p95_ns: p95,
...
function previousWallTime(ledger, scenario) {
  ...
  for (const match of summaries.reverse()) {
    ...
    const lines = text
      .slice(match.index + match[0].length, end === -1 ? undefined : end)
      .split("\n")
...
function appendLedger({
  ...
  const header =
    fs.existsSync(ledger) && fs.statSync(ledger).size > 0
      ? ""
...
function statusFor(result, output) {
  return result.status === 0
    ? "pass"
    : /panicked at|thread '.*' panicked/.test(output)
      ? "panic"
      : "fail";
}
...
async function main() {
  ...
  const resultRoot = path.resolve(
    process.env.JAZZ_RECEIPT_RESULT_ROOT ?? path.join(root, "dev/benchmarks/results"),
  );
  const logRoot = path.resolve(
    process.env.JAZZ_RECEIPT_LOG_DIR ?? path.join(root, "target/benchmark-receipts"),
  );
  const ledger = path.resolve(
    process.env.JAZZ_RECEIPT_LEDGER ?? path.join(root, "dev/benchmarks/SMOKE_LEDGER.md"),
  );
  ...
  const commandResult = spawnSync(args.command[0], args.command.slice(1), {
    cwd: root,
    encoding: "utf8",
    env: process.env,
  ...
  for (const file of args.jsonFiles) {
    const resolved = path.resolve(root, file);
    if (fs.existsSync(resolved))
      rows.push(...parseJsonDocuments(fs.readFileSync(resolved, "utf8")));
  }
  ...
  rows.push({
    ...
    phase: "harness",
    status,
    wall_us: Math.round(elapsed * 1_000_000),
    wall_s: Number(elapsed.toFixed(6)),
  ...
  console.log(`Receipt: ${path.relative(root, jsonl)}`);
