# alpha.54 historical subscription fan-out backfill

This permanent harness branch must not merge into main or any release. It
starts at released engine `7cb8a9b4e088c5810540f221c0d3434069566e6b` (`v2.0.0-alpha.54`) and imports the W1
...
`verify-alpha54-fanout-backfill.py`. Cargo.toml, Cargo.lock, dependency
...
bytes, so no alpha.54 measurement is produced.
...
`docs/lib/perf-timeline/backfills.ts`. Effective date `2026-09-10T04:26:57.178Z` is npm
`jazz-tools` time[2.0.0-alpha.54]. Keep the actual CodSpeed measurement
