...
test("native benchmark catalog defines RocksDB and SQLite variants for each native scenario", () => {
  ...
  assert.ok(ids.has("native-criterion:rocksdb:r10_direct_sync_fanout"));
  assert.ok(ids.has("native-criterion:sqlite:r10_direct_sync_fanout"));
...
test("native benchmark catalog targets storage-backed engine-specific Criterion groups", () => {
  ...
  assert.equal(rocksdbCrud.criterion_filter, "realistic_phase1_direct/r1_crud");

  assert.ok(sqliteCrud, "expected SQLite CRUD benchmark entry");
  assert.equal(sqliteCrud.storage_engine, "sqlite");
  assert.equal(sqliteCrud.criterion_filter, "realistic_phase1_direct/r1_crud");

  assert.ok(rocksdbColdLoad, "expected RocksDB cold-load benchmark entry");
  assert.equal(rocksdbColdLoad.criterion_filter, "realistic_phase1_direct/r3_rocksdb_cold_load");
});
...
test("native Criterion command opts into the SQLite storage backend", () => {
  ...
  assert.deepEqual(command, [
    ...
    "realistic_phase1_direct",
    "--",
    "realistic_phase1_direct/r2_reads",
  ]);
});
...
test("benchmark workflow prebuilds the RocksDB-backed and SQLite-backed native binaries", () => {
  ...
  assert.match(
    workflow,
    /cargo bench -p jazz-tools --features rocksdb --bench realistic_phase1_direct --no-run/,
  );
  assert.match(
    workflow,
    /cargo bench -p jazz-tools --features sqlite --bench realistic_phase1_direct --no-run/,
  );
});
