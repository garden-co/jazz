# alpha.55 historical subscription fan-out backfill

This permanent harness branch must not merge into main or any release. It
starts at released engine `11738be1b2b442ffb0ea37c3c3891d538ec02251` (`v2.0.0-alpha.55`) and imports the W1
...
`verify-alpha55-fanout-backfill.py`. Cargo.toml, Cargo.lock, dependency
...
bytes, so no alpha.55 measurement is produced.
...
`docs/lib/perf-timeline/backfills.ts`. Effective date `2026-09-15T22:33:17.491Z` is npm
`jazz-tools` time[2.0.0-alpha.55]. Keep the actual CodSpeed measurement
