...
const requestedTargets = process.argv
  ...
  .filter(
    (argument, index, arguments_) =>
      ...
      arguments_[index - 1] !== '--package-root'
  );
...
if (!/^[0-9a-f]{40}$/i.test(sourceRevision ?? ''))
  throw new Error(
    'JAZZ_NATIVE_RELAY_SOURCE_REVISION must be the exact 40-character source commit'
  );

const abiSource = readFileSync(join(relaySource, 'src/lib.rs'), 'utf8');
const abi = Number(
  /pub const NATIVE_RELAY_ABI_VERSION: u16 = (\d+);/.exec(abiSource)?.[1]
);
if (!Number.isSafeInteger(abi))
  throw new Error('could not read native relay ABI from Rust source');
const tsAbiSource = readFileSync(
  join(root, 'crates/jazz-rn/src/native-relay-abi.ts'),
  'utf8'
);
const tsAbi = Number(
...
function decodeXmlText(text) {
  return text.replace(
    /&(?:amp|apos|gt|lt|quot|#x[0-9a-fA-F]+|#[0-9]+);/g,
    (entity) => {
      if (entity === '&amp;') return '&';
      if (entity === '&apos;') return "'";
      if (entity === '&gt;') return '>';
...
function parseXmlPlist(xml) {
  ...
  const readTag = () => {
    if (xml[cursor] !== '<')
      throw new Error('expected XML tag in iOS XCFramework Info.plist');
    const end = xml.indexOf('>', cursor + 1);
    if (end === -1)
      throw new Error('unterminated XML tag in iOS XCFramework Info.plist');
  ...
  const expectClosing = (name) => {
    ...
    const tag = readTag();
    if (tag !== `/${name}`)
      throw new Error(`expected </${name}> in iOS XCFramework Info.plist`);
  };
  const parseValue = () => {
    skipWhitespace();
    const tag = readTag();
    if (tag === 'dict') {
      ...
      while (true) {
        ...
        if (typeof key !== 'string')
          throw new Error(
            'iOS XCFramework Info.plist dictionary has a non-string key'
          );
    ...
    if (tag === 'string' || tag === 'key') {
      ...
      if (end === -1)
        throw new Error(`unterminated <${tag}> in iOS XCFramework Info.plist`);
  ...
  while (xml.startsWith('<?', cursor) || xml.startsWith('<!', cursor)) {
    ...
    if (end === -1)
      throw new Error(
        'unterminated XML preamble in iOS XCFramework Info.plist'
      );
    cursor = end + 1;
    skipWhitespace();
  }
  ...
  if (cursor !== xml.length)
    throw new Error('trailing data in iOS XCFramework Info.plist');
...
function verifyIosSlices(expected) {
  const plist = parseXmlPlist(
    readFileSync(join(targets.ios.root, 'Info.plist'), 'utf8')
  );
  const slices = plist.AvailableLibraries;
  if (!Array.isArray(slices))
    throw new Error(
      'iOS XCFramework Info.plist has no AvailableLibraries array'
    );
  ...
  for (const slice of slices) {
    if (typeof slice !== 'object' || slice === null || Array.isArray(slice))
      throw new Error(
        'iOS XCFramework AvailableLibraries contains a non-dictionary slice'
      );
    ...
    if (!identifier || !library || !library.endsWith('.a'))
      throw new Error(
        `iOS ${role} XCFramework slice is missing its static library path`
      );
  ...
  for (const [role, present] of requiredRoles)
    if (!present)
      throw new Error(
        `iOS XCFramework is missing its ${role} static-library slice`
      );
}
...
function filesUnder(root, directory = root) {
  ...
  for (const name of readdirSync(directory).sort()) {
    ...
    if (stat.isSymbolicLink())
      throw new Error(`relay artifact contains symbolic link: ${file}`);
...
for (const requested of requestedTargets) {
  ...
  if (!target)
    throw new Error(
      `usage: verify-relay-artifacts.mjs <android|ios>... (unknown ${requested})`
    );
  ...
  if (
    ...
    (!/^[0-9]+\.[0-9]+\.[0-9]+$/.test(manifest.toolchain?.cargoNdk ?? '') ||
      (cargoNdkVersion !== undefined &&
        manifest.toolchain.cargoNdk !== cargoNdkVersion))
  )
    throw new Error(
  ...
  for (const entry of manifest.files) {
    ...
    if (expected.has(entry.path))
      throw new Error(
        `${requested} relay manifest has duplicate ${entry.path}`
      );
  ...
  if (
    ...
    throw new Error(
      `${requested} relay artifact inventory differs from its manifest`
    );
