...
import { NATIVE_RELAY_ABI_V1, NATIVE_RELAY_ABI_V2 } from "../native-relay-abi";
...
it("rejects an installed native build with an incompatible ABI before executing a command", async () => {
  ...
  await expect(relay.executeNativeRelayCommand("AA==")).rejects.toThrow(
    "Jazz native relay ABI 1 is incompatible with JavaScript ABI 2..=2; install a matching native development or release build.",
  );
...
it("rejects a missing, malformed, or ABI-incompatible bindings-installed JSI foreground factory", () => {
  for (const factory of [
    undefined,
    {},
    { abiVersion: NATIVE_RELAY_ABI_V1, openAttached: () => foregroundFixture() },
  ]) {
    const nativeRelay: FixtureNativeRelay = {
      getAbiVersion: () => NATIVE_RELAY_ABI_V2,
      execute: jest.fn(),
