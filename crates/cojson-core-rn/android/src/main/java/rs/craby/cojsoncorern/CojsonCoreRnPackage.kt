...
class CojsonCoreRnPackage : BaseReactPackage() {
  companion object {
    val JNI_PREPARE_MODULE_NAME = setOf(
      "__crabyCojsonCoreRn_JNI_prepare__"
    )
  }

  init {
    SoLoader.loadLibrary("cxx-cojson-core-rn")
  }

  override fun getModule(name: String, reactContext: ReactApplicationContext): NativeModule? {
    if (name in JNI_PREPARE_MODULE_NAME) {
      nativeSetDataPath(reactContext.filesDir.absolutePath)
      return CojsonCoreRnPackage.TurboModulePlaceholder(reactContext, name)
    }
    return null
  }
