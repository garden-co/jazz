...
TransactionResult HybridCoJSONCoreRN::tryAddTransactions(const SessionLogHandle& handle, const std::vector<std::string>& transactionsJson, const std::string& newSignature, bool skipVerify) {
...
TransactionResult HybridCoJSONCoreRN::addNewPrivateTransaction(const SessionLogHandle& handle, const std::string& changesJson, const std::string& signerSecret, const std::string& encryptionKey, const std::string& keyId, double madeAt) {
    auto result = add_new_private_transaction(toRustHandle(handle), rust::String(changesJson), rust::String(signerSecret), rust::String(encryptionKey), rust::String(keyId), madeAt);
    return TransactionResult(result.success, std::string(result.result), std::string(result.error));
}

TransactionResult HybridCoJSONCoreRN::addNewTrustingTransaction(const SessionLogHandle& handle, const std::string& changesJson, const std::string& signerSecret, double madeAt) {
    auto result = add_new_trusting_transaction(toRustHandle(handle), rust::String(changesJson), rust::String(signerSecret), madeAt);
    return TransactionResult(result.success, std::string(result.result), std::string(result.error));
}

TransactionResult HybridCoJSONCoreRN::testExpectedHashAfter(const SessionLogHandle& handle, const std::vector<std::string>& transactionsJson) {
    // Convert std::vector<std::string> to rust::Vec<rust::String>
    rust::Vec<rust::String> rustTransactions;
    for (const auto& tx : transactionsJson) {
        rustTransactions.push_back(rust::String(tx));
...
TransactionResult HybridCoJSONCoreRN::decryptNextTransactionChangesJson(const SessionLogHandle& handle, double txIndex, const std::shared_ptr<margelo::nitro::ArrayBuffer>& keySecret) {
