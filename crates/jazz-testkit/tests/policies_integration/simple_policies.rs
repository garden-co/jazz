...
async fn insert_policies_boolean() {
    tokio::task::LocalSet::new()
        .run_until(insert_policies_boolean_inner())
        .await;
}

async fn insert_policies_boolean_inner() {
    ...
    let insert_true_id = seed_document(
        &alice,
        "documents_insert_true",
        super::ALICE_ID,
        "original",
    ...
    .await;
    let insert_false_id = seed_document(
        &alice,
        "documents_insert_false",
        super::ALICE_ID,
        "original",
    ...
    let query = Query::from("documents_insert_true");
    let bob_rows = wait_for_rows(&bob, query, "bob sees inserted row", |rows| {
        rows.iter()
            .any(|(id, values)| {
                *id == insert_true_id
                    && *values == boolean_policy_document_values(super::ALICE_ID, "original", false)
            })
            .then_some(rows)
    })
    .await;
    ...
    assert_eq!(
        bob_rows[0].1,
        boolean_policy_document_values(super::ALICE_ID, "original", false)
    );

    let query = Query::from("documents_insert_false");
...
async fn select_policies_boolean() {
    tokio::task::LocalSet::new()
        .run_until(select_policies_boolean_inner())
        .await;
}

async fn select_policies_boolean_inner() {
    let schema = SchemaBuilder::new()
        .table(make_documents_schema(
    ...
    let select_false_id = seed_document(
        &alice,
        "documents_select_false",
        super::ALICE_ID,
        "hidden",
    ...
    .await;
    let select_true_id = seed_document(
        &alice,
        "documents_select_true",
        super::ALICE_ID,
        "visible",
    ...
    let query = Query::from("documents_select_true");
        ...
        |rows| {
            rows.iter()
                .any(|(id, values)| {
                    *id == select_true_id
                        && *values
                            == boolean_policy_document_values(super::ALICE_ID, "visible", false)
                })
                .then_some(rows)
        },
    )
    ...
    assert!(bob_rows.iter().any(|(id, values)| {
        *id == select_true_id
            && *values == boolean_policy_document_values(super::ALICE_ID, "visible", false)
    }));

    let query = Query::from("documents_select_false");
    let bob_rows = wait_for_rows(
        &bob,
        query,
...
async fn select_policy_dependency_data_is_retrieved_as_part_of_query_inner() {
    ...
    let (grant_id, _, grant_transaction_id) = writer
        .insert(
            "access_grants",
            jazz::row_input!("principal_id" => super::ALICE_ID, "active" => true),
        )
    ...
    jazz_testkit::wait_for_edge_txs(
        ...
        &[
            ...
            record_transaction_id.expect("protected record insert should commit immediately"),
        ],
    )
    .await;
...
async fn select_policies_filter_out_archived_rows() {
    tokio::task::LocalSet::new()
        .run_until(select_policies_filter_out_archived_rows_inner())
        .await;
}

async fn select_policies_filter_out_archived_rows_inner() {
    ...
    let active_id = seed_document(&alice, table_name, super::ALICE_ID, "active", false).await;
    let archived_id = seed_document(&alice, table_name, super::ALICE_ID, "archived", true).await;
    ...
    let bob_rows = wait_for_rows(
        ...
        |rows| {
            let active_values = boolean_policy_document_values(super::ALICE_ID, "active", false);
    ...
    assert!(has_row(
        &bob_rows,
        active_id,
        &boolean_policy_document_values(super::ALICE_ID, "active", false),
    ));
...
async fn update_policies_boolean() {
    tokio::task::LocalSet::new()
        .run_until(update_policies_boolean_inner())
        .await;
}

async fn update_policies_boolean_inner() {
    let schema = SchemaBuilder::new()
        .table(make_documents_schema(
    ...
    let update_true_id = seed_document(
        &alice,
        "documents_update_true",
        super::ALICE_ID,
        "original",
    ...
    .await;
    let update_false_id = seed_document(
        &alice,
        "documents_update_false",
        super::ALICE_ID,
        "original",
...
async fn delete_policies_boolean() {
    tokio::task::LocalSet::new()
        .run_until(delete_policies_boolean_inner())
        .await;
}

async fn delete_policies_boolean_inner() {
    let schema = SchemaBuilder::new()
        .table(make_documents_schema(
    ...
    let delete_true_id = seed_document(
        &alice,
        "documents_delete_true",
        super::ALICE_ID,
        "original",
    ...
    .await;
    let delete_false_id = seed_document(
        &alice,
        "documents_delete_false",
        super::ALICE_ID,
        "original",
...
async fn archived_state_policies_gate_insert_update_and_delete() {
    tokio::task::LocalSet::new()
        .run_until(archived_state_policies_gate_insert_update_and_delete_inner())
        .await;
}

async fn archived_state_policies_gate_insert_update_and_delete_inner() {
    ...
    let rejected_insert_id =
        seed_document(&alice, table_name, super::ALICE_ID, "already-done", true).await;
    let active_id = seed_document(&alice, table_name, super::ALICE_ID, "task", false).await;
    ...
    let bob_rows = wait_for_rows(
        ...
        |rows| {
            rows.iter()
                .any(|(id, values)| {
                    *id == active_id
                        && *values == boolean_policy_document_values(super::ALICE_ID, "task", false)
                })
                .then_some(rows)
        },
    )
    ...
    assert!(has_row(
        &observer_rows,
        active_id,
        &boolean_policy_document_values(super::ALICE_ID, "task", false),
    ));
    ...
    let observer_rows = wait_for_rows(
        ...
        |rows| {
            rows.iter()
                .any(|(id, values)| {
                    *id == active_id
                        && *values == boolean_policy_document_values(super::ALICE_ID, "task", true)
                })
                .then_some(rows)
        },
    )
    ...
    assert!(has_row(
        &observer_rows,
        active_id,
        &boolean_policy_document_values(super::ALICE_ID, "task", true),
    ));
...
async fn select_policies_scalar_comparators_filter_rows() {
    tokio::task::LocalSet::new()
        .run_until(select_policies_scalar_comparators_filter_rows_inner())
        .await;
}

async fn select_policies_scalar_comparators_filter_rows_inner() {
    let schema = SchemaBuilder::new()
    ...
    let ne_match = create_row(
        &alice,
        "documents_select_ne",
        jazz::row_input!("title" => "different", "priority" => 5i32),
    )
    .await;
    let ne_hidden = create_row(
        &alice,
        "documents_select_ne",
        jazz::row_input!("title" => "exact", "priority" => 3i32),
    )
    .await;
    ...
    let gt_match = create_row(
        &alice,
        "documents_select_gt",
        jazz::row_input!("title" => "higher", "priority" => 5i32),
    )
    .await;
    let gt_hidden = create_row(
        &alice,
        "documents_select_gt",
        jazz::row_input!("title" => "equal", "priority" => 3i32),
    )
    .await;
    ...
    let gte_low = create_row(
        &alice,
        "documents_select_gte",
        jazz::row_input!("title" => "low", "priority" => 1i32),
    )
    .await;
    ...
    let gte_high = create_row(
        &alice,
        "documents_select_gte",
        jazz::row_input!("title" => "high", "priority" => 5i32),
    )
    .await;
    ...
    let lt_match = create_row(
        &alice,
        "documents_select_lt",
        jazz::row_input!("title" => "lower", "priority" => 1i32),
    )
    .await;
    ...
    let ne_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_ne"),
    ...
    let gt_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_gt"),
    ...
    let gte_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_gte"),
    ...
    let lt_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_lt"),
    ...
    let lte_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_lte"),
...
async fn null_predicates_on_nullable_columns_gate_reads_and_writes() {
    tokio::task::LocalSet::new()
        .run_until(null_predicates_on_nullable_columns_gate_reads_and_writes_inner())
        .await;
}

async fn null_predicates_on_nullable_columns_gate_reads_and_writes_inner() {
    let schema = SchemaBuilder::new()
    ...
    let select_eq_null_visible = create_row(
        &alice,
        "documents_select_eq_null",
        jazz::row_input!("title" => "unassigned", "reviewer_id" => Value::Null),
    )
    .await;
    let select_eq_null_hidden = create_row(
        &alice,
        "documents_select_eq_null",
        jazz::row_input!("title" => "assigned", "reviewer_id" => super::ALICE_ID),
    )
    .await;
    ...
    let insert_eq_null_visible = create_row(
        &alice,
        "documents_insert_eq_null",
        jazz::row_input!("title" => "allowed null", "reviewer_id" => Value::Null),
    )
    .await;
    let insert_eq_null_hidden = create_row(
        &alice,
        "documents_insert_eq_null",
        jazz::row_input!("title" => "rejected non-null", "reviewer_id" => super::ALICE_ID),
    )
    .await;
    ...
    let insert_ne_null_hidden = create_row(
        &alice,
        "documents_insert_ne_null",
        jazz::row_input!("title" => "rejected null", "reviewer_id" => Value::Null),
    )
    .await;
    let insert_ne_null_visible = create_row(
        &alice,
        "documents_insert_ne_null",
        jazz::row_input!("title" => "allowed non-null", "reviewer_id" => super::ALICE_ID),
    )
    .await;
    ...
    let update_is_null_allowed = create_row(
        &alice,
        "documents_update_is_null",
        jazz::row_input!("title" => "becomes null", "reviewer_id" => super::ALICE_ID),
    )
    .await;
    let update_is_null_rejected = create_row(
        &alice,
        "documents_update_is_null",
        jazz::row_input!("title" => "stays null", "reviewer_id" => Value::Null),
    )
    .await;

    let select_eq_null_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_eq_null"),
    ...
    let select_ne_null_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_ne_null"),
        "ne null keeps only non-null rows",
        |rows| {
            (rows.len() == 1
                && rows[0].0 == select_ne_null_visible
                && rows[0].1 == review_values("assigned", Some(super::ALICE_ID))
    ...
    let select_is_null_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_is_null"),
    ...
    let insert_eq_null_rows = wait_for_rows(
        &bob,
        Query::from("documents_insert_eq_null"),
    ...
    let insert_ne_null_rows = wait_for_rows(
        &bob,
        Query::from("documents_insert_ne_null"),
        "insert ne null allows only non-null reviewer rows",
        |rows| {
            (rows.len() == 1
                && rows[0].0 == insert_ne_null_visible
                && rows[0].1 == review_values("allowed non-null", Some(super::ALICE_ID))
    ...
    let update_rows = wait_for_rows(
        &bob,
        Query::from("documents_update_is_null"),
...
async fn row_level_contains_and_in_list_policies_filter_rows() {
    tokio::task::LocalSet::new()
        .run_until(row_level_contains_and_in_list_policies_filter_rows_inner())
        .await;
}

async fn row_level_contains_and_in_list_policies_filter_rows_inner() {
    let schema = SchemaBuilder::new()
    ...
    let contains_match = create_row(
        &alice,
        "documents_select_contains",
        jazz::row_input!("title" => "Launch Checklist", "status" => "active", "archived" => false),
    )
    .await;
    let contains_hidden = create_row(
        &alice,
        "documents_select_contains",
        jazz::row_input!("title" => "Backlog", "status" => "active", "archived" => false),
    )
    .await;
    ...
    let in_active = create_row(
        &alice,
        "documents_select_in_list",
        jazz::row_input!("title" => "Active", "status" => "active", "archived" => false),
    )
    .await;
    let in_trial = create_row(
        &alice,
        "documents_select_in_list",
        jazz::row_input!("title" => "Trial", "status" => "trial", "archived" => false),
    )
    .await;
    let in_hidden = create_row(
        &alice,
        "documents_select_in_list",
        jazz::row_input!("title" => "Archived", "status" => "archived", "archived" => true),
    )
    .await;
    ...
    let empty_hidden = create_row(
        &alice,
        "documents_select_empty_in_list",
        jazz::row_input!("title" => "Should stay hidden", "status" => "active", "archived" => false),
    )
    .await;

    let contains_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_contains"),
    ...
    let in_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_in_list"),
    ...
    let empty_rows = wait_for_rows(
        &bob,
        Query::from("documents_select_empty_in_list"),
...
async fn read_and_write_policies_remain_independent_inner() {
    ...
    let write_only_id = seed_document(
        &alice,
        "documents_write_only",
        super::ALICE_ID,
        "hidden",
...
async fn authorized_mutations_emit_visibility_scoped_subscription_deltas_inner() {
    ...
    .await;
    let query = Query::from(table_name);
    let visible_values = boolean_policy_document_values(super::ALICE_ID, "visible", false);
    let renamed_visible_values =
        boolean_policy_document_values(super::ALICE_ID, "visible renamed", false);
    let revealed_hidden_values = boolean_policy_document_values(super::ALICE_ID, "hidden", false);
    ...
    wait_for_subscription_update(
        ...
        |log| has_added_id(log, visible_id),
    )
    .await;
    ...
    assert!(
        !has_added_id(&observer_log, visible_id) && !has_removed(&observer_log, visible_id),
    ...
    assert!(
        !has_added_id(&observer_log, visible_id),
    ...
    wait_for_subscription_update(
        ...
        |log| has_added_id(log, hidden_id),
    )
    .await;
    assert!(
        has_added_id(&observer_log, hidden_id),
...
async fn admin_secret_ws_client_bypasses_row_select_policies() {
    tokio::task::LocalSet::new()
        .run_until(admin_secret_ws_client_bypasses_row_select_policies_inner())
        .await;
}

async fn admin_secret_ws_client_bypasses_row_select_policies_inner() {
    ...
    let mut privileged_ctx =
        server.make_client_context_for_user(schema.clone(), super::OBSERVER_ID);
    privileged_ctx.backend_secret = None;
    privileged_ctx.admin_secret = Some(server.admin_secret().to_string());
    let privileged = jazz_testkit::connect(privileged_ctx)
        .await
        .expect("connect privileged observer");

    let row_id = seed_document(&alice, table_name, super::ALICE_ID, "private doc", false).await;
    let expected_values = boolean_policy_document_values(super::ALICE_ID, "private doc", false);
