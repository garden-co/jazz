...
fn title_document_input(title: &str) -> HashMap<String, Value> {
    jazz::row_input!("title" => title.to_string())
}
...
fn group_document_input(group_slug: &str, title: &str) -> HashMap<String, Value> {
    jazz::row_input!("group_slug" => group_slug.to_string(), "title" => title.to_string())
}
...
fn claim_compound_input(group_slug: &str, published: bool, title: &str) -> HashMap<String, Value> {
    jazz::row_input!("group_slug" => group_slug.to_string(), "published" => published, "title" => title.to_string())
}
...
async fn admin_role_claims_allow_admin_mutations_and_member_reads() {
    tokio::task::LocalSet::new()
        .run_until(admin_role_claims_allow_admin_mutations_and_member_reads_inner())
        .await;
}

async fn admin_role_claims_allow_admin_mutations_and_member_reads_inner() {
    ...
    wait_for_subscription_update(
        ...
        |log| has_added_id(log, admin_doc),
    )
    .await;
...
async fn admin_role_claims_reject_member_mutations() {
    tokio::task::LocalSet::new()
        .run_until(admin_role_claims_reject_member_mutations_inner())
        .await;
}

async fn admin_role_claims_reject_member_mutations_inner() {
    let table_name = "documents";
    let admin_policy = pe::session_where("claims.role", "ADMIN");
    let schema = SchemaBuilder::new()
        .table(make_title_documents_schema(
    ...
    wait_for_subscription_update(
        ...
        |log| has_added_id(log, barrier_doc),
    )
    .await;
...
async fn claim_array_id_policy_gates_updates_by_primary_key_inner() {
    let table_name = "documents";
...
async fn role_claim_presence_gates_row_visibility_inner() {
    let table_name = "documents";
    let schema = SchemaBuilder::new()
        .table(make_title_documents_schema(
            table_name,
...
async fn groups_allowed_claim_arrays_gate_visibility_and_live_updates_inner() {
    ...
    wait_for_subscription_update(
        ...
        |log| has_added_id(log, eng_follow_up) && has_added_id(log, sales_follow_up),
    )
    .await;
    wait_for_subscription_update(
        &mut eng_stream,
        ...
        |log| has_added_id(log, eng_follow_up),
    )
    .await;
    wait_for_subscription_update(
        &mut sales_stream,
        ...
        |log| has_added_id(log, sales_follow_up),
    )
    .await;
...
async fn claim_null_checks_distinguish_explicit_null_from_missing_paths_inner() {
    ...
    let null_query = Query::from(null_table);
    let present_query = Query::from(present_table);
...
async fn row_and_claim_predicates_compose_under_and_and_or_inner() {
    ...
    let all_of_query = Query::from(all_of_table);
    let any_of_query = Query::from(any_of_table);
