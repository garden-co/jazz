...
async fn edge_catalogue_http_reads_and_writes_forward_to_real_core() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            edge_catalogue_http_reads_and_writes_forward_to_real_core_impl(),
        ))
        .await
}
...
async fn edge_catalogue_publish_reaches_peer_edge_through_core_sync() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            edge_catalogue_publish_reaches_peer_edge_through_core_sync_impl(),
        ))
        .await
}
...
async fn persisted_stale_edge_reconnect_replays_catalogue_before_client_work() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            persisted_stale_edge_reconnect_replays_catalogue_before_client_work_impl(),
        ))
        .await
}
...
async fn persistent_dynamic_edge_reopens_ready_catalogue_before_first_client() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            persistent_dynamic_edge_reopens_ready_catalogue_before_first_client_impl(),
        ))
        .await
}
...
async fn core_permission_retightening_reaches_subscribed_clients_on_every_edge() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            core_permission_retightening_reaches_subscribed_clients_on_every_edge_impl(),
        ))
        .await
}
...
async fn edge_migration_publish_forwards_to_real_core_and_is_readable_through_edge() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            edge_migration_publish_forwards_to_real_core_and_is_readable_through_edge_impl(),
        ))
        .await
}
...
async fn dynamic_server_denies_reads_until_permissions_head_is_published() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            dynamic_server_denies_reads_until_permissions_head_is_published_impl(),
        ))
        .await
}
...
async fn dynamic_server_keeps_pre_permissions_user_write_hidden_after_publish() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            dynamic_server_keeps_pre_permissions_user_write_hidden_after_publish_impl(),
        ))
        .await
}
...
async fn dynamic_server_rejects_user_write_after_permissions_timeout() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            dynamic_server_rejects_user_write_after_permissions_timeout_impl(),
        ))
        .await
}
...
async fn dynamic_server_live_subscription_replays_on_first_permissions_head_and_retightening() {
    tokio::task::LocalSet::new().run_until(Box::pin(dynamic_server_live_subscription_replays_on_first_permissions_head_and_retightening_impl())).await
}
...
async fn column_addition_new_client_can_read_old_rows() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(column_addition_new_client_can_read_old_rows_impl()))
        .await
}
...
async fn cannot_read_from_old_schema_until_lens_is_added() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            cannot_read_from_old_schema_until_lens_is_added_impl(),
        ))
        .await
}
...
async fn multi_hop_column_additions_new_client_can_read_old_rows() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            multi_hop_column_additions_new_client_can_read_old_rows_impl(),
        ))
        .await
}
...
async fn multi_hop_column_renames_new_client_can_read_old_rows() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            multi_hop_column_renames_new_client_can_read_old_rows_impl(),
        ))
        .await
}
...
async fn multi_hop_column_renames_old_client_can_read_new_rows() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            multi_hop_column_renames_old_client_can_read_new_rows_impl(),
        ))
        .await
}
...
async fn table_rename_new_client_can_read_old_rows() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(table_rename_new_client_can_read_old_rows_impl()))
        .await
}
...
async fn table_rename_subscription_reacts_to_old_branch_updates() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            table_rename_subscription_reacts_to_old_branch_updates_impl(),
        ))
        .await
}
...
async fn table_rename_update_and_delete_copy_on_write() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(table_rename_update_and_delete_copy_on_write_impl()))
        .await
}
...
async fn table_rename_join_query_translates_join_target_on_old_branch() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            table_rename_join_query_translates_join_target_on_old_branch_impl(),
        ))
        .await
}
...
async fn table_rename_fk_array_lookup_finds_related_rows_on_old_branch() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            table_rename_fk_array_lookup_finds_related_rows_on_old_branch_impl(),
        ))
        .await
}
...
async fn local_join_query_uses_current_permissions_for_joined_provenance_after_lens_transform() {
    tokio::task::LocalSet::new().run_until(Box::pin(local_join_query_uses_current_permissions_for_joined_provenance_after_lens_transform_impl())).await
}
...
async fn multi_hop_table_renames_and_column_rename() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(multi_hop_table_renames_and_column_rename_impl()))
        .await
}
...
async fn removed_table_then_readded_does_not_resurface_old_rows() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(
            removed_table_then_readded_does_not_resurface_old_rows_impl(),
        ))
        .await
}
...
async fn column_addition_old_client_can_read_new_rows() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(column_addition_old_client_can_read_new_rows_impl()))
        .await
}
...
async fn keeps_authorization_through_v1_head() {
    tokio::task::LocalSet::new()
        .run_until(Box::pin(keeps_authorization_through_v1_head_impl()))
        .await
}
