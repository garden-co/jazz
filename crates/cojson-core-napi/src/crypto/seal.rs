...
pub fn seal(
  ...
  napi::Result::Ok(seal_internal(message, &sender_secret, &recipient_id, nonce_material)?.into())
}
...
mod tests {
  ...
  fn test_seal_unseal() {
    ...
    let unsealed = unseal_internal(&sealed, &sender_secret, &recipient_id, nonce_material).unwrap();
