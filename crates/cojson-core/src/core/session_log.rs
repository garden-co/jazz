...
impl SessionLogInternal {
    ...
    pub fn try_add(
        ...
        let transactions = transactions
            .iter()
            .map(|v| stable_stringify(&serde_json::from_str(v)?))
        ...
        for tx in transactions {
            self.transactions_json.push(tx);
        }
    ...
    pub fn add_new_transaction(
        ...
        let new_tx = match mode {
            TransactionMode::Private { key_id, key_secret } => {
                ...
                let mut ciphertext = changes.as_bytes().to_vec();
...
mod tests {
    ...
    fn test_add_from_example_json_multi_tx() {
        ...
        let transactions: Vec<String> = example
            .transactions
            .into_iter()
