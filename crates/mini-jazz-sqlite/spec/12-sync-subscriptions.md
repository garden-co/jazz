...
read tiers for main-branch current table reads and built queries. This is the
first reusable lowering for #947-style semantics: local reads include accepted
local optimistic state, edge reads require an edge receipt or global epoch, and
global reads require a global epoch. Branch snapshot reads and subscription
delivery should reuse the same tier predicate rather than adding bespoke SQL.
