...
fn insert_row_in_tx(args: InsertRowInTx<'_>) -> Result<()> {
    ...
    let effective_values = if args.op == 1 {
        ...
        let mut current = current_row_values(args.db, table, row_num, args.branch_num)?
...
fn current_row_values(
    ...
    if branch_num != 1 {
        if let Some(base_epoch) = branch::base_global_epoch(conn, branch_num)? {
            if !current_row_exists_on_branch(conn, &table.name, row_num, branch_num)? {
    ...
    let sql = format!(
        ...
        field_columns.join(", "),
        crate::schema::current_table(&table.name),
        current_effective_branch_sql("current", &table.name, branch_num)
    );
...
fn snapshot_row_values(
    ...
    let sql = format!(
        ...
        field_columns.join(", "),
        crate::schema::history_table(&table.name),
        history_table = crate::schema::history_table(&table.name),
    );
    let mut stmt = conn.prepare(&sql)?;
...
fn row_values_from_sql(
    ...
    for (idx, field) in table.fields.iter().enumerate() {
        values.insert(
            field.name.clone(),
            sql_value_to_json(conn, field, &row[idx])?,
        );
    }
    Ok(Some(values))
}
