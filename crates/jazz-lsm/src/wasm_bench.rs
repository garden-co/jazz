...
async fn run_cold_seq_read(count: u32, value_size: u32) -> Result<BenchmarkResult, JsValue> {
    ...
    for i in 0..(count as usize) {
        ...
        let v = db
            .get(&k)
            .map_err(|e| JsValue::from_str(&e.to_string()))?
            .ok_or_else(|| JsValue::from_str("missing key during cold_seq_read benchmark"))?;
        checksum = checksum.wrapping_add(v[0] as u64);
    }
    push_phase(&mut phase_times_ms, "op_read_loop", op_start);
    let elapsed_ms = high_res_now_ms() - op_start;
    ...
    Ok(BenchmarkResult {
        operation: "cold_seq_read".to_string(),
        value_size,
        count,
        seed,
        wall_elapsed_ms,
...
async fn run_cold_random_read(count: u32, value_size: u32) -> Result<BenchmarkResult, JsValue> {
    ...
    for &i in &order {
        ...
        let v = db
            .get(&k)
            .map_err(|e| JsValue::from_str(&e.to_string()))?
            .ok_or_else(|| JsValue::from_str("missing key during cold_random_read benchmark"))?;
        checksum = checksum.wrapping_add(v[0] as u64);
    }
    push_phase(&mut phase_times_ms, "op_read_loop", op_start);
    let elapsed_ms = high_res_now_ms() - op_start;
    ...
    Ok(BenchmarkResult {
        operation: "cold_random_read".to_string(),
        value_size,
        count,
        seed,
        wall_elapsed_ms,
...
#[wasm_bindgen]
pub async fn bench_opfs_cold_sequential_read(
    count: u32,
    value_size: u32,
) -> Result<JsValue, JsValue> {
    to_js_value(&run_cold_seq_read(count, value_size).await?)
}

#[wasm_bindgen]
pub async fn bench_opfs_cold_random_read(count: u32, value_size: u32) -> Result<JsValue, JsValue> {
    to_js_value(&run_cold_random_read(count, value_size).await?)
}
