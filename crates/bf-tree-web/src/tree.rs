...
impl BfTree {
    ...
    /// Create a BfTree with an OPFS VFS backend and a custom configuration.
    ///
    /// Like [`with_opfs_vfs`](Self::with_opfs_vfs), but allows full control over
    /// the tree configuration (page sizes, key lengths, record sizes, etc.).
    #[cfg(target_arch = "wasm32")]
    pub fn with_opfs_vfs_and_config(
        opfs_vfs: crate::fs::OpfsVfs,
        config: Config,
    ) -> Result<Self, ConfigError> {
        let vfs: Arc<dyn crate::fs::VfsImpl> = Arc::new(opfs_vfs);
    ...
    fn with_config_inner(
        ...
        if config.cache_only {
            let leaf_storage = match custom_vfs {
                Some(vfs) => LeafStorage::new_with_vfs(config.clone(), buffer_ptr, vfs),
                None => LeafStorage::new(config.clone(), buffer_ptr),
            };
