...
/// The generic ordered-KV payload family supplied by Groove itself.
///
/// Higher layers compose this with their own opaque durable-codec families.
/// Groove deliberately owns neither those IDs nor their semantics: the
/// profile is input to the adapter-open boundary, not a hidden global registry.
pub const GROOVE_EPOCH_1_CODECS: &[&str] = &["groove.ordered-kv.v1"];
...
mod tests {
    ...
    fn epoch_1_codec_corpus_round_trips_committed_bytes_exactly() {
        ...
        assert_eq!(
            StorageEpochManifest::decode(committed)
                .unwrap()
                .encode()
                .unwrap(),
            committed
    ...
    #[test]
    fn epoch_1_codec_corpus_rejects_malformed_trailing_noncanonical_and_unknown_versions() {
        let committed = manifest().encode().unwrap();
        let mut malformed = committed.clone();
        malformed[0] = b'X';
        assert!(StorageEpochManifest::decode(&malformed).is_err());

        let mut trailing = committed.clone();
        trailing.push(0);
        assert!(StorageEpochManifest::decode(&trailing).is_err());

        // The decoder sees the duplicate as a set, but canonicality requires
        // the committed sorted unique spelling rather than accepting it.
        let noncanonical = b"JSM1\0\x01\0\x01\x06memory\x02\x14groove.ordered-kv.v1\x14groove.ordered-kv.v1\x01\x09key-order\0\x16unsigned-lexicographic";
    ...
    fn selected_codec_profile_rejects_omitted_extra_and_unknown_ids_at_admission() {
        ...
        assert!(
            StorageCodecProfile::new(["groove.ordered-kv.v1", "groove.ordered-kv.v1"]).is_err()
        );
        ...
        for codecs in [
            ...
            let encoded = unchecked_manifest(&codecs).encode().unwrap();
            assert_eq!(
            ...
            assert!(expected.admit_existing(&unchecked_bytes(&codecs)).is_err());
        }
    }
    #[test]
