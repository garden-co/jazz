...
- `package-lock.json` - npm lockfile used by Pulumi Deployments
...
Current working stack settings for `garden-co/jazz2` / `garden-computing/jazz-cloud2/dev`:

- repository: `garden-co/jazz2`
- branch: `main`
- project path: `crates/jazz-cloud-server/deploy/pulumi`
- PR previews: enabled
- deploy on commit to `main`: enabled

Current working deployment environment:

- AWS credentials provided by Pulumi Deployments OIDC
- AWS OIDC role: `arn:aws:iam::327252143262:role/pulumi-staging-deployment-role`
- pre-run command: `npm ci`
- skip automatic dependency installation: enabled
- no committed `awsPrimaryProfile` / `awsDnsProfile` in `Pulumi.<stack>.yaml`
- `route53DelegationRoleArn` committed in the stack config for shared Route53 access

This setup was required because the Pulumi Deployments executor for this stack did
not reliably install Node dependencies from the built-in dependency download step.
Using `package-lock.json` plus an explicit `npm ci` pre-run made previews/apply work
consistently.
...
For the `dev` stack today, the PR generated by the deploy workflow updates the
`containerImageTag` value in `Pulumi.dev.yaml`. The preview may also show unrelated
runtime drift, such as a newer ECS AMI or secret version replacement, because Pulumi
re-evaluates the full program on each run.
...
2. Leave `dispatch_deploy=true` so it triggers `.github/workflows/deploy-cloud-server-image.yml`
3. That workflow opens or updates a deployment PR that changes `jazz-cloud2:containerImageTag`
4. Wait for the Pulumi Cloud preview on that PR
5. Merge the PR so Pulumi Cloud runs `pulumi up`
...
- source repo `garden-co/jazz2`, branch `main`, repo dir `crates/jazz-cloud-server/deploy/pulumi`
- AWS OIDC role `arn:aws:iam::327252143262:role/pulumi-staging-deployment-role`
- pre-run command `npm ci`
- skip automatic dependency installation enabled

Secrets can be supplied either way:

- committed stack secrets in `Pulumi.dev.yaml`, or
