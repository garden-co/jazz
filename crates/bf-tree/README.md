...
## Fork Additions (bf-tree-web)

This fork adds **WASM/browser support** to the original [microsoft/bf-tree](https://github.com/microsoft/bf-tree):

- **WASM target** (`wasm32-unknown-unknown`) - run bf-tree in browsers and other WASM environments
- **OPFS storage backend** - persistent storage using the browser's Origin Private File System API
- **WAL on WASM** - write-ahead logging with fire-and-forget semantics (no background thread)
...
See `tests/wasm/` for a complete browser example with benchmarks.
