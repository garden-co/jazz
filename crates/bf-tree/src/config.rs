...
pub enum StorageBackend {
    ...
    Spdk,
    /// OPFS (Origin Private File System) backend for WASM.
    /// Requires running in a Web Worker with HTTPS.
...
impl WalConfig {
    ...
    pub fn new(file_path: impl AsRef<Path>) -> Self {
        Self {
            ...
            // WAL needs persistent storage for crash recovery
            #[cfg(not(target_arch = "wasm32"))]
    ...
    /// Change the storage backend for potentially better performance.
    ///
    /// On native: defaults to Std, can use IoUring on Linux.
    /// On WASM: defaults to Opfs for persistence.
    pub fn storage_backend(&mut self, backend: StorageBackend) -> &mut Self {
        self.storage_backend = backend;
        self
    }
