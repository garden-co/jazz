...
impl WriteAheadLog {
    ...
    ///
    /// On WASM, no background thread is started - writes are fire-and-forget with
    /// immediate flush when the buffer is full.
    ...
    /// Create a new WAL instance for WASM.
    ///
    /// WASM version does not start a background thread - writes are fire-and-forget.
    ...
    pub(crate) fn new(config: Arc<WalConfig>) -> Arc<Self> {
        ...
        let wal = WriteAheadLog {
            ...
            flushed_cond: Condvar::new(),
            need_flush_cond: Condvar::new(),
            background_job_running: AtomicBool::new(false), // No background job in WASM
            config,
        };

        Arc::new(wal)
        // No background flush job on WASM - fire-and-forget writes
    }
    ...
    #[cfg(target_arch = "wasm32")]
    pub(crate) fn stop_background_job(&self) {
        // No-op on WASM - no background job to stop
    }
    ...
    /// Append a log entry and wait for it to be flushed.
    ///
    /// On native platforms, this waits for the background flush job.
    /// On WASM, this uses fire-and-forget - the write is buffered and flushed
    ...
    /// WASM version: Fire-and-forget append.
    ///
    /// Writes are buffered and flushed immediately when the buffer is full.
    /// Since WASM is single-threaded, there's no background flush job.
    #[cfg(target_arch = "wasm32")]
    #[must_use = "The returned flushed lsn must be write to page meta"]
    pub(crate) fn append_and_wait<'a>(
        &self,
        ...
        if required_bytes > remaining {
            // Buffer full - flush immediately (fire-and-forget style)
            inner.flush();
        }
        ...
        // Fire-and-forget: don't wait for flush, just return the LSN
        // The data is in the buffer and will be written on next full-buffer flush
        // or when explicitly flushed
        inner.flushed_lsn = lsn;
        lsn
    }

    /// Explicitly flush the WAL buffer.
    ///
    /// On WASM, this should be called periodically or before closing
    /// to ensure all buffered writes are persisted.
    #[cfg(target_arch = "wasm32")]
    pub fn flush(&self) {
        let mut inner = self.inner.lock().unwrap();
        inner.flush();
...
///
/// Note: Not available on WASM - use OPFS APIs for WAL recovery in browser environments.
///
/// Example
/// ```ignore
