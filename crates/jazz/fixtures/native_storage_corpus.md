# Native Jazz current storage corpus

The executable producer for the final V1 positive fixtures is
...
native adapters under the final V1 profile. Its published bytes and complete
inventory are `current-native-jazz-corpus-v1.md`.
...
The current producer's exact logical pack is committed as
`current-native-jazz-producer.pack.base64`; it is base64 only to keep its
canonical binary values safe in a text repository. Its SHA-256 and complete
store inventory are recorded in `current-native-jazz-corpus-v1.md`. It
explicitly lists empty authoritative families as well as entries, so an omitted
opened family cannot look the same as an empty one.
...
- `current-native-jazz.sqlite.gz.base64` — gzip payload SHA-256
  `305af9de09fe297f4d6fd161dacd0ccddbda64932121efde4a8ba74987d2eb46`,
  decompressed SQLite SHA-256
  `421ccbeb742c85a13811b3468720f47e1b4eea16cbfd193aa32ee561e778014e`.
- `current-native-jazz-rocksdb.tar.gz.base64` — archive SHA-256
  `c718d70d9fe9f26f9af7d9430cf1240c00ca241786d72771d1b55c59f04276e0`.

The historical `epoch-1-native-jazz-*` artifacts are retained separately as
negative evidence. They are never regenerated by this producer and are never
admitted as current V1 roots.

`committed_native_jazz_physical_corpus_reopens_and_accepts_current_writes`
checks each _current_ payload before materializing it, inspects SQLite and
RocksDB using their read-only physical APIs, opens the exact current logical
snapshot through Jazz, materializes the full indirect value, writes one new
current-format row, and reopens it. The paired corruption test proves a changed
archive is rejected before any target file or extraction directory is created.
SQLite pages and RocksDB files remain backend-owned bytes, not interchange
encodings. The `epoch-1-native-jazz-*` blobs are separate immutable negative
evidence and must never be reopened or extended by a final V1 adapter.
...
fresh root and runs the complete current reopen/mixed-write receipt against
...
The checked-in current fixtures are regenerated by the exact source-closure
storage transition using the final V1 storage profile. The deterministic authority namespace is
...
```sh
JAZZ_NATIVE_CORPUS_PACK_OUT=/tmp/current-native-jazz-corpus.pack \
JAZZ_NATIVE_CORPUS_SQLITE_OUT=/tmp/current-native-jazz.sqlite \
JAZZ_NATIVE_CORPUS_ROCKS_ARCHIVE_OUT=/tmp/current-native-jazz-rocksdb.tar.gz \
