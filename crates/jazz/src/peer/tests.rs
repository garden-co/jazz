...
fn late_initial_drain_resets_the_complete_retained_source_closure() {
    ...
    let SyncMessage::ViewUpdate(update) = peer.query_update(&mut core, &shape, &binding).unwrap()
        ...
        panic!("expected completed source reset")
    };
    let rows = update
        .supporting_rows
        .added_rows()
...
fn direct_supporting_manifest_preserves_owned_reference_wire_bytes() {
    ...
    for step in 0..4 {
        ...
        let expected = maintained.supporting_rows().cloned().collect::<Vec<_>>();
        ...
        payload.supporting_rows = expected_update;
        assert_eq!(
            crate::wire::encode_sync_message(&update).unwrap(),
...
fn supporting_rows_deduplicate_same_table_self_join_source_roles() {
    ...
    let shared_tx = core
        .commit_mergeable_settled(
            MergeableCommit::new("todos", shared, 1_000).cells(title_cells("shared")),
        )
        .unwrap();
    accept_global(&mut core, shared_tx, 1);
    let shape = Query::from(table("todos").alias("root"))
        .flat_join(table("todos").alias("peer"), "root.title", "peer.title")
        .validate(&schema())
        .unwrap();
    ...
    let _ = peer.rehydrate_query(&mut core, &shape, &binding).unwrap();
    let updated_tx = core
        .commit_mergeable_settled(
            MergeableCommit::new("todos", shared, 1_001).cells(title_cells("updated")),
        )
        .unwrap();
    ...
    assert_eq!(
        ...
        1,
        "shared aliases deduplicate physical witnesses"
    );
    ...
    assert_eq!(
        peer.publication_states[&subscription]
            .supporting_rows()
            .filter(|input| input.row == shared && input.version.tx == updated_tx)
            .count(),
        1,
        "self-join roles share a single retained physical input"
    );
...
fn client_fast_cursor_authorization_proof_controls_rehydrate_reset() {
    ...
    // members.
    assert_eq!(
        program_fact_adds.added_rows().iter().count(),
        1,
...
fn duplicate_structured_query_authorization_mismatch_forces_reset() {
    ...
    assert_eq!(
        supporting_rows.added_rows().len(),
        2,
        "duplicate receives complete supporting closure"
    );
}
...
fn served_subscription_paths_fail_closed_when_a_relay_binding_is_removed() {
    ...
    assert_eq!(
        direct
            .served_subscription_policy_binding(subscription)
            .unwrap(),
        (alice, claims.clone())
    );
...
fn incremental_delivery_removes_a_superseded_output_occurrence() {
    ...
    assert_eq!(
        replacement_removals(
            &indexed_members(std::slice::from_ref(&previous)),
            &[replacement]
        ),
        vec![previous],
...
fn incremental_delivery_keeps_terminal_children_with_their_root() {
    ...
    let update = |adds, removes| (subscription, false, adds, removes);
    let mut peer = PeerState::default();
    apply_test_result_delta(
        &mut peer,
        &update(vec![root_member.clone(), child_member.clone()], Vec::new()),
    );
    apply_test_result_delta(&mut peer, &update(Vec::new(), vec![child_member]));
...
fn maintained_delivery_does_not_leak_intermediate_replacement_refcounts() {
    ...
    let update = |adds, removes| (subscription, false, adds, removes);
    let mut peer = PeerState::default();
    apply_test_result_delta(&mut peer, &update(vec![tx10.clone()], Vec::new()));
    apply_test_result_delta(
        &mut peer,
        &update(vec![tx11.clone(), tx12.clone()], vec![tx10, tx11]),
    );
    apply_test_result_delta(&mut peer, &update(Vec::new(), vec![tx12]));
...
fn maintained_delivery_rekeys_delta_and_reset_by_output_occurrence() {
    ...
    let update = |reset_input_set, adds, removes| (subscription, reset_input_set, adds, removes);
    let mut peer = PeerState::default();

    apply_test_result_delta(
        &mut peer,
        &update(true, vec![first.clone(), second.clone()], Vec::new()),
    );
    apply_test_result_delta(
        &mut peer,
        &update(false, vec![replacement.clone()], vec![first]),
    );
    apply_test_result_delta(
        &mut peer,
        &update(true, vec![replacement.clone(), second.clone()], Vec::new()),
    );
...
fn edge_support_hydration_uses_writer_claims_and_fails_closed_when_missing() {
    ...
    assert_eq!(
        system_serving_peer.link_identity(),
        Some(transport_identity)
    );
    assert_eq!(
        system_serving_peer.permission_subject(),
    ...
    assert_eq!(
        wrong_type_peer.permission_subject(),
        Some(transport_identity)
    );
}
...
fn public_peer_schema(builder: PublicSchemaBuilder) -> JazzSchema {
    crate::schema::JazzSchema::new(&builder.build()).expect("peer-test public schema compiles")
}
...
fn edge_ingest_uses_monotonic_scope_ttl_and_retains_wall_admission_time() {
    ...
    assert!(
        ttl_ms > 0,
        "this receipt requires the normal positive edge-scope TTL"
    );
    ...
    assert!(
        peer.idle_edge_scope_subscriptions
    ...
    assert!(
        ...
        "the retry remains deferred without support"
    );
    assert!(
        !peer
...
fn covered_input_result_row(input: &crate::protocol::SupportingRow) -> Option<ResultRowEntry> {
    (input.version.layer == crate::protocol::ResultRowLayer::Content)
        .then(|| (input.version_table.clone(), input.row, input.version.tx))
}
...
fn assert_view_update_rows(
    ...
    let mut actual = if payload.supporting_rows.is_snapshot() {
        BTreeSet::new()
    } else {
        rows.clone()
    };
        ...
        .filter_map(covered_input_result_row)
    {
        assert!(
            actual.remove(&entry),
            "wire removal must exist in the test predecessor"
        );
    }
    for entry in payload
        .supporting_rows
        ...
        .filter_map(covered_input_result_row)
    {
        assert!(
            actual.insert(entry),
            "wire addition must be absent from the test predecessor"
        );
    }
...
fn maintained_branch_view_reconcile_retains_undeleted_base_members() {
    ...
    let base =
        BranchSelector::new([("branch_id", Value::Uuid(uuid::Uuid::from_bytes([0x99; 16])))]);
    let head =
...
fn maintained_structured_change_ships_only_covered_inputs() {
    ...
    let user = row(0xa1);
    let user_tx =
        core.commit_mergeable_settled(MergeableCommit::new("users", user, 1_000).cells(
    ...
    let child_tx = core
        .commit_mergeable_settled(MergeableCommit::new("todos", row(0xb1), 1_001).cells(
            BTreeMap::from([
    ...
    assert_eq!(
        program_fact_adds.revision(),
        previous_supporting_rows.revision()
    );
    assert!(program_fact_adds.added_rows().is_empty());
    assert!(
        program_fact_adds.removed_rows().is_empty(),
        "idempotent confirmation has no membership changes"
    );
}
...
fn maintained_subscription_view_limit_one_installs_subscription() {
    ...
    assert_view_update_rows(
        &mut expected_snapshots,
        update,
        vec![("todos", row_from_u64(10), lower_tx)],
        vec![],
    );
}
...
fn maintained_subscription_view_order_by_limit_updates_move_rows_across_boundary() {
    ...
    assert!(
        !version_carriers.is_empty(),
        "the covered input chooses its exact version body through the ordinary bundle path"
    );
...
fn maintained_subscription_view_aggregate_updates_incrementally() {
    ...
    assert!(program_fact_adds.added_rows().iter().any(|fact| {
        {
            let input = fact;
            input.version_table.as_ref() == "todos" && input.row == row(0x12)
        }
    }));
}
...
fn maintained_subscription_view_hit_metrics_and_footprint_update() {
    ...
    let re_restored_tx = core
        .commit_mergeable_settled(
            MergeableCommit::new("todos", row(0x51), 1_004).deletion(DeletionEvent::Restored),
        )
        .unwrap();
...
fn maintained_storage_fallback_batches_multi_row_replacement_removals() {
    ...
    let second_tx = core
        .commit_mergeable_settled(
            MergeableCommit::new("todos", second, 1_001).cells(title_cells("match")),
        )
        .unwrap();
    accept_global(&mut core, second_tx, 2);
    let shape = Query::from("todos")
    ...
    .unwrap();
    let (replacement_tx, _unit) = core
        .commit_exclusive_settled(tx, AuthorSubject::SYSTEM, 1_002)
        .unwrap();
...
fn maintained_subscription_view_range_literal_stays_maintained() {
    ...
    assert_eq!(
        ...
        BTreeSet::from([row(0x81), row(0x83)])
    );
    assert_eq!(peer.maintained_subscription_view_metrics().hits_out, 2);
}
...
fn maintained_subscription_view_reversed_range_literal_stays_maintained() {
    ...
    assert_eq!(
        view_update_added_rows(update),
        BTreeSet::from([row(0x85), row(0x87)])
    );
    assert_eq!(peer.maintained_subscription_view_metrics().hits_out, 2);
}
...
fn maintained_subscription_view_any_literal_stays_maintained() {
    ...
    assert_eq!(
        view_update_added_rows(update),
        BTreeSet::from([row(0x89), row(0x8a)])
    );
    assert_eq!(peer.maintained_subscription_view_metrics().hits_out, 2);
}
...
fn maintained_subscription_view_in_literal_stays_maintained() {
    ...
    assert_eq!(
        view_update_added_rows(update),
        BTreeSet::from([row(0x8b), row(0x8c)])
    );
    assert_eq!(peer.maintained_subscription_view_metrics().hits_out, 2);
}
...
fn maintained_subscription_view_null_predicates_stay_maintained() {
    ...
    for (case, non_null) in [(0xa3, false), (0xa4, true)] {
        ...
        assert_eq!(
            view_update_added_rows(update),
            BTreeSet::from([row(case), added_row])
        );
        assert_eq!(peer.maintained_subscription_view_metrics().hits_out, 2);
    }
}
...
fn maintained_subscription_view_exclusive_delta_stays_maintained() {
    ...
    core.tx_write(tx, "todos", row(0x61), title_cells("match"), None)
        .unwrap();
    let (tx_id, _unit) = core
        .commit_exclusive_settled(tx, AuthorSubject::SYSTEM, 1_000)
        .unwrap();
    accept_global(&mut core, tx_id, 1);
...
fn maintained_subscription_view_tags_terminal_columns_by_table() {
    ...
    let schema = public_peer_schema(
        PublicSchemaBuilder::new()
            .table(
                PublicTableSchemaBuilder::new("warehouses").column("ytd", PublicColumnType::Double),
            )
            .table(
                PublicTableSchemaBuilder::new("stock").column("ytd", PublicColumnType::Timestamp),
            )
            .table(
    ...
    let line_tx = core
        .commit_mergeable_settled(MergeableCommit::new("orderLines", line, 12).cells(
            BTreeMap::from([
    ...
    assert_eq!(version_bundles.len(), 3);
    assert_eq!(
        version_bundles
            .iter()
            .map(|bundle| bundle.tx.tx_id)
            .collect::<BTreeSet<_>>(),
        BTreeSet::from([warehouse_tx, stock_tx, line_tx]),
    );
}
...
fn maintained_subscription_view_rehydrate_replaces_subscription_and_fresh_indexes() {
    ...
    let SyncMessage::ViewUpdate(crate::protocol::ViewUpdatePayload { .. }) = &rehydrate else {
        panic!("expected view update");
    };
    assert_view_update_rows(
        &mut expected_snapshots,
...
fn peer_state_dedups_version_payloads_across_subscription_views() {
    ...
    let tx_id = core
        .commit_mergeable_settled(
            MergeableCommit::new("todos", row, 10).cells(title_cells("shared")),
        )
        .unwrap();
    accept_global(&mut core, tx_id, 1);
    let mut peer = PeerState::new();
...
fn policy_revocation_withdraws_covered_input_without_tombstoning_cached_row() {
    ...
    assert!(payload.supporting_rows.added_rows().iter().all(|fact| {
        !{
            let input = fact;
            input.row == doc && input.version.layer == crate::protocol::ResultRowLayer::Deletion
        }
    }));
...
fn concurrent_policy_revoke_cannot_cross_authorize_another_rows_tombstone() {
    ...
    let revoke_tx = core
        .commit_mergeable_settled(
            MergeableCommit::new("docAccess", revoked_grant, 14).deletion(DeletionEvent::Deleted),
        )
        .unwrap();
...
fn rehydrate_keeps_peer_payload_dedup_but_resends_result_set() {
    ...
    let SyncMessage::ViewUpdate(crate::protocol::ViewUpdatePayload { .. }) = &missed_remove else {
        panic!("expected view update");
    };
...
fn peer_state_sends_result_removes_after_deletes() {
    ...
    let SyncMessage::ViewUpdate(crate::protocol::ViewUpdatePayload { .. }) = &removed else {
        panic!("expected view update");
    };
...
fn duplicate_usage_reconciles_canonical_membership_after_deletion_witness() {
    ...
    let target_reset = peer
        ...
        .unwrap();
    let SyncMessage::ViewUpdate(crate::protocol::ViewUpdatePayload { .. }) = target_reset else {
...
// These checks exercise internal result bookkeeping, not a peer wire payload.
fn apply_test_result_delta(
    peer: &mut PeerState,
    update: &(
        SubscriptionKey,
        bool,
...
fn maintained_publication_reuses_complete_successor_closure() {
    ...
    use crate::node::supporting_frontier::SOURCE_CLOSURE_TRAVERSALS;
    let (_dir, mut core) = open_node_with_uuid(node(0x93));
    ...
    assert_eq!(
        traversals, 0,
        "ordinary delta publication never traverses the retained closure"
    );
    let reconstructed = wire_membership.observe(&update);
    let SyncMessage::ViewUpdate(view) = update else {
        panic!("expected complete manifest")
    };
    assert_eq!(view.supporting_rows.added_rows().len(), 1);
    assert_eq!(view.supporting_rows.removed_rows().len(), 1);
    assert_eq!(
        view_update_added_rows(reconstructed),
        expected,
        "all 150 rows remain after applying the one-row delta"
    );
    assert_eq!(
        view.supporting_rows
            .added_rows()
            .iter()
            .find(|row| row.row == row_from_u64(0))
            .unwrap()
            .version
            .tx,
        tx,
        "changed row carries its current exact version"
    );
    let state = &peer.publication_states[&subscription];
    assert_eq!(
        state
