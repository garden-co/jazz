...
fn legacy_inline_json_wire_descriptor_is_rejected_before_storage() {
    ...
    let legacy = versions.into_iter().map(|version| {
        let descriptor = groove::records::RecordDescriptor::new(version.record().descriptor().fields().iter().map(|field| (field.name.clone().unwrap(), if field.name.as_deref() == Some("user_payload") { groove::records::ValueType::Nullable(Box::new(groove::records::ValueType::String)) } else { field.value_type.clone() })));
