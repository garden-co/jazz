    ...
    S: OrderedKvStorage,
{
    ...
    pub(super) async fn ensure_query_program_request_supported(
        ...
        // Edge serving can allocate/retire reader-specific inputs during
        // preflight. Branches, overlays, inline snapshots and covered inputs
        // likewise stay on the ordinary path. No data-sensitive source is
        // admitted from a remembered success.
        let key = admission_program_key(self.edge_query_serving, &request, &access_paths);
