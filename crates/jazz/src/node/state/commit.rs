    ...
    S: OrderedKvStorage,
{
    ...
    pub(super) async fn commit_mergeable_many_at_with_branch_merge(
        &mut self,
        commits: Vec<MergeableCommit>,
        made_at: TxTime,
    ...
    pub(super) async fn commit_mergeable_many_at_with_schema_versions(
        ...
        for (write_schema_version, commit) in commits {
            ...
            let previous_current =
                match self
                    .query_local_layer_winner(&table_schema.name, commit.row_uuid, layer)
                    .await?
                {
                    Some(previous) => Some(previous),
                    None => {
                        self.query_global_layer_winner(
                            &table_schema.name,
                            commit.row_uuid,
                            layer,
                        )
