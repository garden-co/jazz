...
impl InMemoryServerShell {
    ...
    pub fn start_with_storage(
        ...
        let mut bootstrap_schema = config.schema.clone();
        ...
        let db = match &storage_config {
            ...
            StorageConfig::RocksDb { path } => {
                ...
                // The runtime lane bootstraps from the schema the store opened
                // with, never from a requested schema the store does not hold.
                bootstrap_schema = schema.clone();
