    ...
    S: ResidentStorage,
{
    ...
    /// A durable local receiver restores pending authored units before its
    /// first protocol transition. The asynchronous owner acquires this phase;
    /// synchronous nodes complete it directly at the start of their tick.
    pub(super) local_subscriber_restore_pending: bool,
    /// Number of shared upstream commands whose durable inputs were acquired
    /// for the next resident tick. Commands generated by another connection
    /// during that tick remain queued for the next owner poll.
    pub(super) prepared_upstream_command_count: Option<usize>,
...
pub(super) enum ConnectionLink {
    ...
    Subscriber {
        ...
        /// Whole-table publications prepared by the asynchronous owner. Like
        /// maintained group updates, these may need cold historical witnesses
        /// and therefore cannot be assembled inside the resident peer tick.
        prepared_current_row_updates: BTreeMap<SubscriptionKey, SyncMessage>,
    ...
    S: ResidentStorage + ReopenableStorage + 'static,
{
    ...
    pub(super) fn prepare_local_subscriber_restore(
        &mut self,
        node: &mut NodeState<S>,
    ) -> Result<(), crate::node::Error> {
        if !self.local_subscriber_restore_pending {
            return Ok(());
        }
        let ConnectionLink::Subscriber {
            ...
            self.local_subscriber_restore_pending = false;
            return Ok(());
        };
        let author = ingest_context.identity;
        ...
        for tx_id in &pending {
            collect_local_replay_commit_units(node, *tx_id, &mut visited, &mut replay_units)?;
        }
    ...
    /// Acquire every durable input needed to describe pending outbound
    /// subscriptions before the synchronous peer state machine announces
    /// them. Reconnects may need a persisted known-state declaration (or the
    /// canonical rows for a slow exact declaration); neither read is allowed
    /// to escape from the resident-only peer tick.
    pub(super) fn prepare_pending_upstream_inputs(
        &mut self,
        node: &mut NodeState<S>,
    ) -> Result<(), crate::node::Error> {
        ...
        let shared_command_count = upstream_subscriptions.borrow().len();
        let pending = pending
            .iter()
            .chain(
                upstream_subscriptions
                    .borrow()
                    .iter()
                    .take(shared_command_count),
            )
            .filter_map(|command| match command {
                PendingUpstreamCommand::Subscribe(subscription) => Some(subscription.clone()),
                PendingUpstreamCommand::Unsubscribe(_)
                | PendingUpstreamCommand::AuthorizationScopeIntent { .. } => None,
            })
            .collect::<Vec<_>>();
        for pending in pending {
            let values = binding_values_in_param_order(&pending.shape, &pending.binding);
            let _ = node.known_state_declaration_for_subscription(
                &pending.shape,
                &pending.binding,
        ...
        let pending_commits = outbox
            .borrow()
            .iter()
            .filter(|pending| pending.unit.is_none() && !uploaded.contains(&pending.tx_id))
            .map(|pending| pending.tx_id)
            .collect::<Vec<_>>();
        for tx_id in pending_commits {
            let _ = node.commit_unit_for(tx_id)?;
        }
        self.prepared_upstream_command_count = Some(shared_command_count);
        Ok(())
    }
    ...
    pub(super) fn prepare_subscriber_serving_inputs(
        ...
        for (subscription, table) in served_current_rows.iter() {
            if prepared_current_row_updates.contains_key(subscription) {
                continue;
            }
            let update = peer.current_rows_update(node, table)?;
            prepared_current_row_updates.insert(*subscription, update);
        }
        Ok(())
    }
    ...
    pub fn tick(&mut self) -> Result<DbTickStats, Error> {
        if self.local_subscriber_restore_pending {
            ...
            self.prepare_local_subscriber_restore(&mut node.borrow_mut())?;
        }
        let mut stats = DbTickStats::default();
        let connection_epoch = self.connection_epoch;
        let prepared_upstream_command_count = self.prepared_upstream_command_count.take();
        self.observe_shared_subscriber_dirty_epoch();
        self.bind_subscriber_session_claims();
        ...
        match &mut self.link {
            ConnectionLink::Upstream {
                ...
                let available_commands = upstream_subscriptions.borrow().len();
                let shared_command_count = if self.external_durable_ingress {
                    prepared_upstream_command_count.unwrap_or_default()
                } else {
                    available_commands
                };
                pending.extend(
                    upstream_subscriptions
                        .borrow_mut()
                        .drain(..shared_command_count.min(available_commands)),
                );
                    ...
                    )
                {
                    ...
                    for (subscription, table) in served_current_rows.iter() {
                        let update = if let Some(update) =
                            prepared_current_row_updates.remove(subscription)
                        {
                            update
                        } else if self.external_durable_ingress {
                            all_groups_prepared = false;
