...
fn string_grant_role_access_filter_matches_uuid_literal_in_list() {
    ...
    let mut db = block_on(Db::open_history_complete(DbConfig {
        schema: schema.clone(),
...
fn seeded_membership_grant_and_revoke_propagate_incrementally() {
    ...
    let mut client = open_db(0x61, member, &schema);
    let (client_transport, server_transport) = duplex();
    let mut _upstream = client.connect_upstream(client_transport);
    let mut _subscriber = server.accept_subscriber(server_transport, member);
    let mut subscription =
        prepared_subscribe(&mut client, &Query::from("res_i"), ReadOpts::default()).unwrap();
    assert!(opened_rows(block_on(subscription.next_raw()).unwrap()).is_empty());
...
fn same_table_seeded_membership_identity_key_update_propagates_incrementally() {
    ...
    let mut client = open_db(0x68, member, &schema);
    let (client_transport, server_transport) = duplex();
    let mut _upstream = client.connect_upstream(client_transport);
    let mut _subscriber = server.accept_subscriber(server_transport, member);
    let mut subscription =
        prepared_subscribe(&mut client, &Query::from("resources"), ReadOpts::default()).unwrap();
    assert!(opened_rows(block_on(subscription.next_raw()).unwrap()).is_empty());
...
fn inherited_child_policy_parent_revocation_propagates_incrementally() {
    ...
    let mut client = open_db(0x64, member, &schema);
    let (client_transport, server_transport) = duplex();
    let mut _upstream = client.connect_upstream(client_transport);
    let mut _subscriber = server.accept_subscriber(server_transport, member);
...
fn inherited_child_insert_uses_parent_update_where_old_only() {
    ...
    assert_eq!(
        prepared_read(&mut member_db, &Query::from("children"))[0].row_uuid(),
        allowed.row_uuid()
    );

    let mut other_db = open_db(0x67, other, &schema);
...
fn served_subscription_rows_for_author(
    ...
    let mut client = open_db(author.0.as_bytes()[0], author, schema);
    let (client_transport, server_transport) = duplex();
    let mut _upstream = client.connect_upstream(client_transport);
...
fn served_many_subscription_rows_for_author(
    ...
    let mut client = open_db(author.0.as_bytes()[0].wrapping_add(0x40), author, schema);
    let (client_transport, server_transport) = duplex();
    let mut _upstream = client.connect_upstream(client_transport);
    let mut _subscriber = server.accept_subscriber(server_transport, author);
...
fn served_group_entry_rows_via_relay(
    ...
    let mut relay = open_db(0x71, AuthorId::SYSTEM, schema);
    let mut client = open_db(0x72, author, schema);
    let (relay_transport, core_transport) = duplex();
    let mut _relay_upstream = relay.connect_upstream(relay_transport);
    let mut _core_subscriber = server.accept_subscriber(core_transport, AuthorId::SYSTEM);
