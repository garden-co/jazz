...
#[test]
fn local_membership_update_refreshes_policy_dependent_subscription() {
    let schema = membership_scoped_relation_schema();
    let db = open_db(0xb6, AuthorSubject::SYSTEM, &schema);
    let reader = AuthorSubject::for_test_bytes([0xa6; 16]);
    db.set_test_provider_claims(
    ...
    // Updating membership avoids the broad invalidation triggered by deletion,
    // so this specifically exercises the subscription's policy dependencies.
    db.update(
        "chat_members",
        membership,
        BTreeMap::from([(
            "user_id".to_owned(),
            Value::String("someone_else".to_owned()),
        )]),
        Default::default(),
    )
    .unwrap();
