...
fn prepared_query_discards_graph_handle_when_runtime_changes() {
    ...
    let mut db = open_db(0xb7, AuthorId::SYSTEM, &schema);
    let mut prepared = db.prepare_query(&joined_issue_query()).unwrap();
    let mut runtime_token = db.node.node.borrow().groove_runtime_token();
    assert!(
        prepared
...
fn seed_issue_project(db: &mut Db, author: AuthorId) {
...
fn prepared_current_write_query_installs_and_reads_non_simple_plan() {
    ...
    let mut db = open_db(0xa1, author, &schema);
    seed_issue_project(&mut db, author);
    ...
    let mut rows = db.read(&prepared).unwrap();
...
fn local_subscribe_uses_prepared_non_simple_plan() {
    let mut schema = issue_schema();
    let mut author = AuthorId::from_bytes([0xa1; 16]);
    let mut db = open_db(0xa2, author, &schema);
    seed_issue_project(&mut db, author);
...
fn subscription_reset_preserves_ordered_window_rank() {
    ...
    let mut author = AuthorId::from_bytes([0xa1; 16]);
    let mut db = open_db(0xa3, author, &schema);
...
fn filtered_root_prepared_query_still_reads_without_preinstalled_plan() {
    let mut schema = schema();
    let mut author = AuthorId::from_bytes([0xa1; 16]);
    let mut db = open_db(0xa4, author, &schema);
...
fn branch_read_view_relation_snapshot_uses_query_engine_relation_edges() {
    ...
    let mut binding = prepared_query
        .shape()
        .bind(BTreeMap::new())
        .expect("bind relation snapshot shape");
    let mut discriminators = db
        .node
        .node
        .borrow_mut()
...
fn relation_query_one_shot_hop_uses_unified_query_path() {
    ...
    let mut snapshot = block_on(db.all_relation_query(&query, ReadOpts::default())).unwrap();
...
fn relation_query_one_shot_multi_hop_scalar_fk_uses_nested_join_path() {
    let mut schema = relation_hop_schema();
    let mut db = open_db(0xc1, AuthorId::from_bytes([0xc1; 16]), &schema);
    db.insert_with_id(
        "orgs",
        row(0x01),
    ...
    let mut query = users_to_orgs_relation_query();
...
fn relation_query_subscription_hop_uses_unified_query_path() {
    ...
    let mut opened = opened_rows(stream.try_next_event().expect("opened event"));
...
fn relation_query_subscription_hop_preserves_projected_self_reference_cells() {
    let mut schema = relation_hop_schema();
    let mut db = open_db(0xc1, AuthorId::from_bytes([0xc1; 16]), &schema);
    let mut parent = row(0x10);
    let mut team = row(0x11);
    let mut user = row(0x21);
    db.insert_with_id(
        "teams",
        parent,
    ...
    let mut query = users_to_teams_relation_query();
    let mut snapshot = block_on(db.all_relation_query(&query, ReadOpts::default())).unwrap();
...
fn relation_query_gather_uses_unified_reachable_lowering_for_reads_and_subscriptions() {
    ...
    let mut db = open_db(0xc1, AuthorId::from_bytes([0xc1; 16]), &schema);
    let mut query = teams_gather_relation_query();
    let mut stream = block_on(db.subscribe_relation_query(&query, ReadOpts::default())).unwrap();
    assert!(opened_rows(stream.try_next_event().expect("opened event")).is_empty());

    let mut root = row(0x01);
    let mut middle = row(0x02);
    let mut leaf = row(0x03);
    db.insert_with_id(
        "teams",
        root,
    ...
    let mut changed = opened_rows(stream.try_next_event().expect("gathered rows event"));
    assert_eq!(
    ...
    let mut filtered_query = RelationQuery {
    ...
    let mut filtered =
        block_on(db.all_relation_query(&filtered_query, ReadOpts::default())).unwrap();
    assert_eq!(
        row_ids(&filtered.rows).into_iter().collect::<BTreeSet<_>>(),
        BTreeSet::from([root, leaf])
    );

    let mut or_true = RelationQuery {
        rel: RelationExpr::Filter {
            input: Box::new(query.rel.clone()),
    ...
    let mut unfiltered = block_on(db.all_relation_query(&or_true, ReadOpts::default())).unwrap();
    assert_eq!(
    ...
    let mut not_true = RelationQuery {
        rel: RelationExpr::Filter {
            input: Box::new(query.rel.clone()),
            predicate: RelationPredicate::Not(Box::new(RelationPredicate::True)),
        },
    };
    let mut empty = block_on(db.all_relation_query(&not_true, ReadOpts::default())).unwrap();
    assert!(empty.rows.is_empty());

    let mut filter_after_limit = RelationQuery {
        rel: RelationExpr::Filter {
...
fn maintained_subscription_with_two_reference_includes_opens_with_source_coverage() {
    let mut schema = access_edge_include_schema();
    let mut client_author = AuthorId::from_bytes([0xc1; 16]);
    let mut server = open_core(0xee, AuthorId::SYSTEM, &schema);
    server
        .insert_with_id(
            "teams",
            row(0xa1),
    ...
    let mut query = Query::from("team_access_edges")
        .include("resource_id")
        .include("team_id");
    ...
    let mut message = try_recv_subscriber_payload(client_transport.as_mut())
    ...
    let mut tables = result_member_adds
        .iter()
...
fn relation_snapshot_reverse_array_skips_deleted_children_with_camel_case_ref() {
    ...
    let mut joined_query = Query::from("users").join_via_column("todos", "ownerId", "id", []);
    let mut prepared_join = prepared(&mut db, &joined_query);
...
fn relation_snapshot_reverse_array_limit_reads_local_child() {
    ...
    let mut query = Query::from("projects")
...
fn version_bearing_current_source_preserves_provenance_timestamps() {
    ...
    .unwrap();
    {
        ...
        let mut table = node.table("todos").unwrap().clone();
        let mut rows = node
            .test_content_current_with_version(&table, DurabilityTier::Local)
            .unwrap();
        let mut created_at = rows.descriptor.field_index("created_at").unwrap();
        let mut record = rows
            .iter()
    ...
    let mut row = rows.iter().find(|row| row.row_uuid() == id).unwrap();
...
fn db_at_reads_historical_cut_and_partial_requires_server() {
    ...
    let mut partial = open_db(0xc1, author, &schema);
    let mut todo = row(0x42);
    ...
    let mut at_first = core.at(GlobalSeq(1), &Query::from("todos")).unwrap();
    assert_eq!(at_first.len(), 1);
    assert_eq!(
        at_first[0].cell(table, "title"),
        Some(Value::String("draft".to_owned()))
    );
    let mut at_second = core.at(GlobalSeq(2), &Query::from("todos")).unwrap();
    assert_eq!(
        at_second[0].cell(table, "title"),
        Some(Value::String("final".to_owned()))
    );

    let mut partial_todos = partial.prepare_query(&Query::from("todos")).unwrap();
    let mut err = partial.at(GlobalSeq(1), &partial_todos).unwrap_err();
...
fn db_query_builder_expresses_s1_shaped_filters_and_include_modes() {
    ...
    let mut table = schema
        ...
        .unwrap();
    let mut read_rows = prepared_read(&mut db, &s1_query);
    ...
    let mut all_rows = prepared_all(&mut db, &s1_query, ReadOpts::default());
    assert_eq!(row_ids(&all_rows), vec![row(1)]);

    let mut holes_query = db
        .table("issues")
        .filter(eq(col("assignee"), lit(alice.0)))
        .filter(eq(col("state"), lit("open")))
        .include_with(Include::new("project").join_mode(JoinMode::Holes));
    assert_eq!(
        row_ids(&prepared_read(&mut db, &holes_query)),
        vec![row(1), row(4)]
    );

    let mut require_query = holes_query.clone().include_with(
        Include::new("project")
            .join_mode(JoinMode::Holes)
            .require_includes(),
    );
    assert_eq!(
        row_ids(&prepared_read(&mut db, &require_query)),
        vec![row(1)]
    );
    assert_eq!(
        row_ids(&prepared_all(&mut db, &require_query, ReadOpts::default())),
        vec![row(1)],
    ...
    let mut paged = db
        ...
        .offset(1)
        .limit(1);
    assert_eq!(row_ids(&prepared_read(&mut db, &paged)), vec![row(3)]);
}

#[test]
fn payload_enum_match_filters_one_shot_and_maintained_case_transitions() {
    let mut schema = payload_enum_query_schema();
    let mut db = open_db(0xe7, AuthorId::from_bytes([0xe7; 16]), &schema);
    let mut matching = row(0xe1);
    let mut other_case = row(0xe2);
    db.insert_with_id(
        "events",
        matching,
    ...
    let mut query = Query::from("events").filter(Predicate::EnumMatch {
    ...
    assert_eq!(row_ids(&prepared_read(&mut db, &query)), vec![matching]);
...
fn projected_relation_edge_removal_removes_only_its_renamed_related_row() {
    let mut root = row(0x91);
    let mut target = row(0x92);
    let mut projected_edge = RelationEdge {
...
fn permission_introspection_magic_columns_fail_closed_on_prepare_query() {
    ...
    let mut query = db.table("todos").select(["$canRead"]);
    let mut error = expect_error(db.prepare_query(&query));
    assert_eq!(error.code, ErrorCode::Query);
    assert!(
    ...
    let mut provenance_query = db.table("todos").select(["$createdAt", "$createdBy"]);
...
fn single_branch_read_view_uses_query_engine_branch_source_for_one_shot_reads() {
    ...
    let mut prepared_query = prepared(&mut db, &query);
    let mut opts = branch_read_opts();

    let mut rows = doctest_support::block_on(db.all(&prepared_query, opts.clone())).unwrap();
    ...
    let mut deleted = doctest_support::block_on(db.all(&prepared_query, opts.clone())).unwrap();
    assert!(
        deleted.is_empty(),
    ...
    let mut restored = doctest_support::block_on(db.all(&prepared_query, opts.clone())).unwrap();
    assert_eq!(
        row_ids(&restored),
        vec![row(0x42)],
    ...
    let mut local_subscription_opts = ReadOpts {
        propagation: Propagation::LocalOnly,
        ..opts.clone()
    };
...
fn immediate_local_visibility_is_complete_before_write_returns() {
    ...
    let mut rows = doctest_support::block_on(db.all(&prepared, opts)).unwrap();
...
fn edge_read_opts_and_wait_honor_edge_durability() {
    ...
    let mut not_observed = doctest_support::block_on(write.wait(DurabilityTier::Edge)).unwrap_err();
