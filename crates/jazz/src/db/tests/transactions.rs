...
fn attached_schema_mergeable_batch_is_queryable_after_owner_commit() {
    ...
    let view = block_on(owner.register_schema_view(schema.clone())).unwrap();
    ...
    let (prepared, overlay_rows) = {
        ...
        let prepared = attached
            ...
            .unwrap();
        let rows = block_on(attached.transaction_all(overlay_open, &prepared, ReadOpts::default()))
            .unwrap();
        (prepared, rows)
    };
...
fn mergeable_overlay_uses_staged_provenance_and_preserves_it_at_commit() {
    ...
    let author = db.identity.author;
    let tx = block_on(db.begin_mergeable()).unwrap();
    let view_token = db.default_view();
    ...
    let overlay = block_on(view.transaction_all(tx, &query, ReadOpts::default())).unwrap();
    let repeated = block_on(view.transaction_all(tx, &query, ReadOpts::default())).unwrap();
    ...
    let committed = block_on(db.read(&query)).unwrap();
...
fn exclusive_overlay_reserves_stable_provenance_for_insert_and_update() {
    ...
    let tx = block_on(db.begin_exclusive()).unwrap();
    ...
    let query = db
        ...
        .unwrap();
    let overlay = block_on(db.transaction_all(tx, &query, ReadOpts::default())).unwrap();
    let repeated = block_on(db.transaction_all(tx, &query, ReadOpts::default())).unwrap();
...
fn upsert_merges_existing_rows_but_writes_absent_rows_directly() {
    ...
    let rows = prepared_read(&mut db, &query)
        .into_iter()
...
fn mergeable_tx_commits_multiple_writes_under_one_tx_id() {
    ...
    let rows = prepared_read(&mut db, &Query::from("todos"))
        .into_iter()
        .map(|row| (row.row_uuid(), row))
        .collect::<BTreeMap<_, _>>();
    assert_eq!(
...
fn mergeable_tx_coalesces_insert_then_update_for_same_row() {
    ...
    let row_after = prepared_one(&mut db, &Query::from("todos")).unwrap();
...
fn mergeable_tx_and_ref_have_identical_restore_and_reinsert_results() {
    ...
    let builder_state = read_state(&mut builder);
    let handle_state = read_state(&mut handle);
...
fn exclusive_tx_ref_survives_handle_reconstruction_until_explicit_commit() {
    ...
    let current = prepared_one(&mut db, &Query::from("todos")).unwrap();
    assert_eq!(
