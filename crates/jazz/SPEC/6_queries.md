...
- `INV-QUERY-16`: Same-drain result churn MUST be folded by net output-member outcome: enter-then-leave sends no stale add, leave-then-reenter replaces the old entry, and non-consolidated positive/retraction deltas publish only the net membership transition.
