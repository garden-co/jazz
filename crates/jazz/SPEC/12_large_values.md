...
> `large_blob_values_follow_ordinary_row_permissions` pins this end to end,
> asserting four things: (1) an authorized owner reading a
...
> permission is rejected by the authority when it attempts a row spoofing
...
### Rejected research -- text storage after windowed encoding

The proposed window-seal simplifications below were contingent on the retired storage window codec and are not implemented. The historical compression and checkpoint observations are retained as rejected research only; current text storage continues to use the established row, extent, and checkpoint mechanisms. Any replacement requires a new row-compatible design and explicit evidence.
