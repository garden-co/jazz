...
use jazz_tools::sync::sync_tracer::SyncTracer;
use jazz_tools::sync::vocabulary::SyncPayload;
...
async fn alice_write_bob_read() {
    ...
    let bob_received = tracer
        ...
        .into_iter()
        .filter(|message| {
            message.side == jazz_tools::sync::sync_tracer::Side::Recv
    ...
    let bob_sent_from_server = tracer
        ...
        .into_iter()
        .filter(|message| {
            message.side == jazz_tools::sync::sync_tracer::Side::Send
                && message.from.name() == "server"
                && message.to.name() == "bob"
        })
        .collect::<Vec<_>>();
