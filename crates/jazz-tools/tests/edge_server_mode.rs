...
async fn seed_boredm_rows(admin: &JazzClient) -> BoredmSeedRows {
...
async fn assert_boredm_member_rows(member: &JazzClient, rows: &BoredmSeedRows) {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let admin = TestingClient::builder()
                ...
                .connect()
                .await;
            let rows = seed_boredm_rows(&admin).await;
            ...
            let member = TestingClient::builder()
                ...
                .connect()
                .await;
            assert_boredm_member_rows(&member, &rows).await;
...
#[tokio::test(flavor = "current_thread")]
async fn fixed_schema_data_dir_reopen_bootstraps_boredm_policy_serving_state() {
    tokio::task::LocalSet::new()
        .run_until(async {
