...
use super::graph_nodes::sort::{SortDirection, SortNode};
...
fn build_execution_query_from_relation(query: &Query, relation: &RelExpr) -> Option<Query> {
...
fn build_execution_query_from_relation_ir(query: &Query) -> Option<Query> {
    let relation = query.relation_ir.as_ref()?;
    build_execution_query_from_relation(query, relation)
}
...
impl QueryGraph {
    ...
    pub fn compile_relation_ir(
        ...
        let query = build_execution_query_from_relation(&template, relation)?;
    ...
    pub fn compile_with_session(
        ...
        let relation_query = if query.has_relation_ir() {
            Some(build_execution_query_from_relation_ir(query)?)
        } else {
            None
        };
