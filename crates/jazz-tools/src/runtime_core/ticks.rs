...
impl<S: Storage, Sch: Scheduler> RuntimeCore<S, Sch> {
    ...
    /// 1. Drains inbound transport events
    ...
    pub fn batched_tick(&mut self) {
        ...
        // 1. Drain inbound transport events into a local buffer to avoid borrow conflicts.
        let transport_events: Vec<(
            crate::sync_manager::ServerId,
            crate::transport_manager::TransportInbound,
        )> = if let Some(ref mut transport) = self.transport {
            let mut events = Vec::new();
            while let Ok(event) = transport.inbound_rx.try_recv() {
                events.push((transport.server_id, event));
            }
            events
        } else {
            Vec::new()
        ...
        for (server_id, event) in transport_events {
            ...
            match event {
                ...
                TransportInbound::Sync { entry, sequence } => {
                    if let Some(observer) = self.inbox_observer() {
                        observer(&entry);
                    }
                    let entry = *entry;
                ...
                TransportInbound::AuthFailure { reason } => {
                    self.pending_auth_failure = Some(reason);
                    self.remove_server(server_id);
                }
            }
        }
    ...
    /// Route outbox entries to the appropriate transport or peer sender.
    fn flush_outbox(&self, outbox: Vec<OutboxEntry>) {
        use crate::sync_manager::types::Destination;
        for msg in outbox {
            if let Some(observer) = self.outbox_observer() {
                observer(&msg);
            }
            match &msg.destination {
                Destination::Server(_) => {
                    if let Some(ref t) = self.transport {
                        let _ = t.outbox_tx.unbounded_send(msg);
                    } else if let Some(ref s) = self.peer_sender {
                        // Fallback for tests / in-memory multi-node topologies: route
                        // server-destined messages through peer_sender when no WS transport
                        // is connected.
                        s.send_sync_message(msg);
                    } else {
                        tracing::warn!(
                            payload = msg.payload.variant_name(),
                            "outbox entry for server dropped: no transport connected"
                        );
                    }
                }
                Destination::Client(_) => {
                    if let Some(ref s) = self.peer_sender {
                        s.send_sync_message(msg);
                    } else {
                        tracing::warn!(
                            payload = msg.payload.variant_name(),
                            "outbox entry for client dropped: no peer sender configured"
                        );
                    }
                }
            }
