...
/// Positive coverage for the cold-start contract introduced by
/// `.changeset/disable-rejected-fate-replay.md`: persisted `BatchFate::Rejected`
/// records must NOT be replayed on restart. The previous behavior re-emitted
/// `MutationErrorEvent`s and retracted lingering visible rows from stored
/// rejection settlements at startup; that path is now intentionally disabled,
/// and the three superseded tests below (`rc_rejected_batch_survives_restart_until_acknowledged`,
/// `rc_restart_retracts_visible_rows_with_stored_rejected_settlement`,
/// `rc_persisting_invalid_multibranch_sealed_batch_submission_fails`) have
/// been removed.
#[test]
fn rc_cold_start_does_not_replay_persisted_rejection() {
    let schema = test_schema();
    let mut alice = create_runtime_with_schema(schema.clone(), "cold-start-no-replay-test");
    ...
    // Persist a Rejected fate as if a previous session had recorded the
    // rejection settlement but crashed before retracting the visible row.
    alice
        .storage_mut()
        .upsert_authoritative_batch_fate(&crate::batch_fate::BatchFate::Rejected {
            batch_id,
            code: "permission_denied".to_string(),
            reason: "simulated previous-session rejection".to_string(),
        })
        .unwrap();

    let storage = alice.into_storage();
    let mut restarted = create_runtime_with_storage(schema, "cold-start-no-replay-test", storage);

    // No mutation-error event should be emitted on cold start.
    assert!(
        restarted.drain_mutation_error_events().is_empty(),
        "cold start must not re-emit MutationErrorEvent from a stored Rejected fate"
    );

    // The visible row stays visible — the runtime no longer applies stored
    // rejection fates to the visible region on startup. (Any subsequent
    // network-delivered re-rejection still retracts it via the normal
    // BatchFate path.)
    assert_eq!(
        restarted
            .storage()
            .load_visible_region_row("users", branch_name.as_str(), row_id)
            .unwrap(),
        Some(visible_row_before),
        "cold start must not retract a visible row from a stored Rejected fate"
    );
}
