...
fn pump_client_messages_to_server(
    ...
    client.batched_tick();
    for entry in client.take_peer_messages() {
...
fn pump_server_with_three_clients(
    ...
    for _ in 0..10 {
        ...
        writer.batched_tick();
        for entry in writer.take_peer_messages() {
        ...
        for entry in alice_reader.take_peer_messages() {
        ...
        for entry in bob_reader.take_peer_messages() {
        ...
        server.batched_tick();
        let server_out = server.take_peer_messages();
...
fn add_server_rehydrates_visible_rows_from_storage_after_restart() {
    ...
    let messages = restarted.take_peer_messages();
...
fn rc_user_inserted_row_stays_hidden_from_other_sessions() {
    ...
    client.batched_tick();
    server.batched_tick();
    client.take_peer_messages();
    server.take_peer_messages();
...
fn create_3tier_rc() -> ThreeTierRC {
    ...
    a.set_peer_sender(Box::new(VecSyncSender::new()));
    ...
    b.set_peer_sender(Box::new(VecSyncSender::new()));
    ...
    c.set_peer_sender(Box::new(VecSyncSender::new()));
...
fn pump_3tier(s: &mut ThreeTierRC) {
    for _ in 0..10 {
        ...
        // A outbox → B
        s.a.batched_tick();
        let a_out = s.a.take_peer_messages();
        for entry in a_out {
        ...
        s.b.immediate_tick();
        s.b.batched_tick();
        let b_out = s.b.take_peer_messages();
        for entry in b_out {
            match &entry.destination {
        ...
        s.c.immediate_tick();
        s.c.batched_tick();
        let c_out = s.c.take_peer_messages();
        for entry in c_out {
...
fn rc_replays_downstream_query_when_upstream_added_late() {
    ...
    a.batched_tick();
    for entry in a.take_peer_messages() {
...
fn rc_replays_active_queries_on_upstream_reconnect() {
    ...
    let initial_forwarded = s.b.take_peer_messages();
    assert!(
    ...
    let replayed_forwarded = s.b.take_peer_messages();
    assert!(
...
fn rc_does_not_replay_unsubscribed_queries_on_upstream_reconnect() {
    ...
    pump_a_to_b(&mut s);
    s.b.take_peer_messages(); // Drain unsubscription forwarding and unrelated traffic.
...
fn rc_insert_syncs_exact_row_version_without_row_region_reads() {
    ...
    let messages = core.take_peer_messages();
