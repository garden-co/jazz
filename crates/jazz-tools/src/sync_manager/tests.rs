...
fn client_receives_existing_catalogue_on_connect() {
    ...
    match &outbox[0] {
        ...
        _ => panic!("Expected catalogue DocUpdated to client"),
    }
}
...
fn live_catalogue_updates_broadcast_without_query_scope() {
    ...
    assert!(outbox.iter().all(|entry| matches!(
        ...
        OutboxEntry {
            ...
            payload: SyncPayload::DocUpdated { doc_id, .. },
        } if *doc_id == object_id
    )));
}
...
fn remotely_received_catalogue_replays_to_later_clients() {
    ...
    // Create a Yrs doc with content and use DocUpdated to ingest it
    {
        use yrs::{Doc, Map, Transact};
        let doc = Doc::new();
        let root = doc.get_or_insert_map("root");
        let mut txn = doc.transact_mut();
        root.insert(&mut txn, "content", b"schema".as_slice());
    ...
    assert!(matches!(
        ...
        } if *id == client_id && *doc_id == object_id
    ));
}
...
fn server_update_forwarded_to_matching_clients() {
    ...
    // Server sends DocUpdated
    let update = {
        use yrs::{Doc, Map, Transact};
        let doc = Doc::new();
        let root = doc.get_or_insert_map("root");
        let mut txn = doc.transact_mut();
        root.insert(&mut txn, "content", b"from server".as_slice());
    ...
    match &outbox[0] {
        OutboxEntry {
            ...
            assert_eq!(*id, client_id);
            assert_eq!(*doc_id, obj_id);
            assert!(!fwd_update.is_empty());
        }
        _ => panic!("Expected DocUpdated to client"),
    }
}
