    ...
    fn tree_scan_ordered(
        ...
        self.with_tree_mut(|tree| {
            ...
            match scan.direction {
                IndexScanDirection::Ascending => tree
                    .scan_range(start_key.as_bytes(), end_key.as_bytes(), |key| {
                        let key = std::str::from_utf8(key)
                            .map_err(|e| BTreeError::Corrupt(format!("invalid key utf8: {}", e)))?;
                        Ok(collector.consume_key(key))
                    })
                    .map_err(map_storage_err)?,
                IndexScanDirection::Descending => tree
                    .scan_range_reverse(start_key.as_bytes(), end_key.as_bytes(), |key| {
                        let key = std::str::from_utf8(key)
                            .map_err(|e| BTreeError::Corrupt(format!("invalid key utf8: {}", e)))?;
                        Ok(collector.consume_key(key))
                    })
