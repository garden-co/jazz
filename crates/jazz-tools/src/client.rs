...
pub struct JazzClient {
    ...
    /// Whether a server URL was provided at connect time.
    has_server_url: bool,
...
impl JazzClient {
    ...
    pub async fn connect(context: AppContext) -> Result<Self> {
        ...
        // Wire SyncTracer observers for outgoing and incoming messages if a
        // tracer is configured. These observers capture payloads flowing
        // through this client so integration tests can assert on
        // human-readable traces like `alice => server RowVersionCreated`.
        if let Some((ref tracer, ref name)) = context.sync_tracer {
            let tracer_out = tracer.clone();
            let name_out = name.clone();
            let outbox_observer: crate::runtime_core::OutboxObserverBox = Box::new(move |entry| {
                tracer_out.record_outgoing(&name_out, &entry.destination, &entry.payload);
            });
            runtime
                .set_outbox_observer(outbox_observer)
                .map_err(|e| JazzError::Connection(e.to_string()))?;

            let tracer_in = tracer.clone();
            let name_in = name.clone();
            let inbox_observer: crate::runtime_core::InboxObserverBox = Box::new(move |entry| {
                tracer_in.record_incoming(&entry.source, &name_in, &entry.payload);
            });
            runtime
                .set_inbox_observer(inbox_observer)
                .map_err(|e| JazzError::Connection(e.to_string()))?;
        }
        ...
        let has_server_url = !context.server_url.is_empty();
        if has_server_url {
            let ws_url = build_ws_url(&context.server_url)?;
            ...
            let transport_client_id = runtime
                .connect(ws_url, auth_config)
                .map_err(|e| JazzError::Connection(e.to_string()))?;

            // Register the transport's actual client ID with the tracer so
            // human-readable names resolve correctly in test traces.
            if let Some((ref tracer, ref name)) = context.sync_tracer {
                tracer.register_client(transport_client_id, name);
            }

            // Wait for the transport to complete its first auth handshake
            // before returning. Matches the pre-transport-rewrite contract
            // where `JazzClient::connect` would block until the stream was
            // live, so callers can immediately use server-durability tiers
            // without racing the background transport task.
            let _ = runtime
