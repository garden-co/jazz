...
/// First message a client sends after WebSocket upgrade.
/// Carries authentication and client identity in a single frame,
/// replacing the HTTP headers used by the SSE + POST transport.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuthHandshake {
    /// Client ID. Generated by server if not provided.
    pub client_id: Option<String>,
    /// Authentication payload.
    pub auth: AuthHandshakePayload,
    /// Admin secret for catalogue sync (schema, lens, permissions).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub admin_secret: Option<String>,
    /// Client's current catalogue state hash for delta sync.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub catalogue_state_hash: Option<String>,
}

/// Authentication variants for the WebSocket handshake.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "type")]
pub enum AuthHandshakePayload {
    /// Frontend JWT authentication.
    #[serde(rename = "jwt")]
    Jwt { token: String },
    /// Backend impersonation with secret + session.
    #[serde(rename = "backend")]
    Backend { secret: String, session: String },
    /// Local development auth (anonymous or demo mode).
    #[serde(rename = "local")]
