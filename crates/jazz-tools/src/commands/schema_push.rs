//! Schema push command implementation.
//!
//! Pushes versioned schema and lens catalogue objects to a sync server using the
//! same runtime sync callback pattern used by the client.
...
impl SyncServerClient {
    ...
    async fn push_sync(
        ...
        let payload_bytes = payload.to_rkyv_bytes()?;
        let sync_url = format!(
            "{}{}{}?client_id={}",
...
pub async fn run(
    ...
    let mut pushed_schema_count = 0usize;
    for schema_info in &schema_versions {
        ...
        pushed_schema_count += 1;
        println!(
            "Pushed schema v{} ({})",
            schema_info.version, schema_info.hash
        );
    }

    let forward_migrations = collect_forward_migration_files(&schema_directory)?;
    let mut pushed_lens_count = 0usize;
    ...
    println!(
        "Done. Pushed {} schemas, {} lenses.",
        pushed_schema_count, pushed_lens_count
    );
    Ok(())
}
