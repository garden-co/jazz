//! jazz-wasm - WebAssembly bindings for the Jazz database engine.
//!
//! This crate provides JavaScript bindings for the Jazz local-first database engine,
//! enabling TypeScript/JavaScript applications to use Jazz with custom storage backends.
//!
//! # Architecture
//!
//! - **WasmRuntime**: Main entry point that wraps SchemaManager and provides CRUD operations
//! - **WasmQueryBuilder**: Fluent query builder exposed to JavaScript
//! - **JsStorageDriver**: Interface for JavaScript storage implementations (IndexedDB, node:sqlite)
//! - **Type bridges**: Serialization between Rust and JavaScript types
//!
//! # Usage
//!
//! ```javascript
//! import { WasmRuntime, WasmQueryBuilder } from 'jazz-wasm';
//!
//! // Create a storage driver (e.g., IndexedDB)
//! const driver = {
//!   async process(requests) {
//!     // Handle storage requests
//!     return responses;
//!   }
//! };
//!
//! // Create runtime
//! const schema = { todos: { columns: [...] } };
//! const runtime = new WasmRuntime(driver, JSON.stringify(schema), 'my-app', 'dev', 'main');
//!
//! // Insert a row
//! const id = await runtime.insert('todos', {
//!   title: { type: 'Text', value: 'Buy milk' },
//!   done: { type: 'Boolean', value: false },
//! });
//!
//! // Query with builder
//! const query = new WasmQueryBuilder('todos').branch('main').build();
//! const results = await runtime.query(query);
//!
//! // Tick must be called periodically
//! setInterval(() => runtime.tick(), 100);
//! ```

#![allow(clippy::new_without_default)]

pub mod driver_bridge;
pub mod query;
pub mod runtime;
pub mod types;
#[cfg(target_arch = "wasm32")]
pub mod worker_bridge;
#[cfg(target_arch = "wasm32")]
pub mod worker_host;
pub mod worker_protocol;
#[cfg(target_arch = "wasm32")]
pub mod ws_stream;

// Re-export main types for JavaScript
pub use driver_bridge::JsStorageDriver;
pub use query::WasmQueryBuilder;
pub use runtime::WasmRuntime;
#[cfg(target_arch = "wasm32")]
pub use worker_bridge::WasmWorkerBridge;
#[cfg(target_arch = "wasm32")]
pub use worker_host::run_as_worker;

use wasm_bindgen::prelude::*;

/// Initialize the WASM module.
///
/// Sets up panic hook for better error messages in the browser console.
#[wasm_bindgen(start)]
pub fn init() {
    #[cfg(feature = "console_error_panic_hook")]
    console_error_panic_hook::set_once();
}

/// Parse a schema from JSON string.
///
/// Returns the schema as a JsValue for inspection.
#[wasm_bindgen(js_name = parseSchema)]
pub fn parse_schema(json: &str) -> Result<JsValue, JsError> {
    let schema: types::Schema =
        serde_json::from_str(json).map_err(|e| JsError::new(&format!("Parse error: {}", e)))?;
    Ok(serde_wasm_bindgen::to_value(&schema)?)
}

/// Generate a new UUID v7 (time-ordered).
///
/// Useful when a caller wants the default generated row-id shape.
#[wasm_bindgen(js_name = generateId)]
pub fn generate_id() -> String {
    uuid::Uuid::now_v7().to_string()
}

/// Get the current timestamp in microseconds since Unix epoch.
#[wasm_bindgen(js_name = currentTimestamp)]
pub fn current_timestamp() -> u64 {
    use web_time::{SystemTime, UNIX_EPOCH};
    SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|d| d.as_micros() as u64)
        .unwrap_or(0)
}

/// Scan a persisted opfs-btree file snapshot into raw key/value entries.
#[cfg(target_arch = "wasm32")]
#[wasm_bindgen(js_name = scanOpfsBTreeEntriesFromFileBytes)]
pub fn scan_opfs_btree_entries_from_file_bytes(
    file_bytes: &js_sys::Uint8Array,
) -> Result<JsValue, JsError> {
    let file = opfs_btree::MemoryFile::from_bytes(file_bytes.to_vec());
    let mut tree = opfs_btree::OpfsBTree::open(file, opfs_btree::BTreeOptions::default())
        .map_err(|err| JsError::new(&format!("open opfs-btree bytes: {err}")))?;
    let entries = tree
        .raw_entries(usize::MAX)
        .map_err(|err| JsError::new(&format!("scan opfs-btree entries: {err}")))?;

    raw_entries_to_js(entries).map(Into::into)
}

/// Incrementally scan a persisted opfs-btree file snapshot into raw key/value entries.
#[cfg(target_arch = "wasm32")]
#[wasm_bindgen(js_name = OpfsBTreeEntryScanner)]
pub struct WasmOpfsBTreeEntryScanner {
    tree: opfs_btree::OpfsBTree<opfs_btree::MemoryFile>,
    cursor: Option<opfs_btree::RawEntryCursor>,
    done: bool,
}

#[cfg(target_arch = "wasm32")]
#[wasm_bindgen(js_class = OpfsBTreeEntryScanner)]
impl WasmOpfsBTreeEntryScanner {
    #[wasm_bindgen(constructor)]
    pub fn new(file_bytes: &js_sys::Uint8Array) -> Result<WasmOpfsBTreeEntryScanner, JsError> {
        let file = opfs_btree::MemoryFile::from_bytes(file_bytes.to_vec());
        let tree = opfs_btree::OpfsBTree::open(file, opfs_btree::BTreeOptions::default())
            .map_err(|err| JsError::new(&format!("open opfs-btree bytes: {err}")))?;
        Ok(Self {
            tree,
            cursor: None,
            done: false,
        })
    }

    #[wasm_bindgen(js_name = nextBatch)]
    pub fn next_batch(&mut self, limit: usize) -> Result<JsValue, JsError> {
        if self.done {
            return raw_entry_batch_to_js(Vec::new(), true);
        }

        let batch = self
            .tree
            .raw_entries_batch(self.cursor, limit.max(1))
            .map_err(|err| JsError::new(&format!("scan opfs-btree entries: {err}")))?;

        self.cursor = batch.next_cursor;
        self.done = batch.done;
        raw_entry_batch_to_js(batch.entries, batch.done)
    }
}

#[cfg(target_arch = "wasm32")]
fn raw_entry_batch_to_js(entries: Vec<(Vec<u8>, Vec<u8>)>, done: bool) -> Result<JsValue, JsError> {
    let batch = js_sys::Object::new();
    js_sys::Reflect::set(
        &batch,
        &JsValue::from_str("entries"),
        &raw_entries_to_js(entries)?.into(),
    )
    .map_err(|_| JsError::new("set entries failed"))?;
    js_sys::Reflect::set(
        &batch,
        &JsValue::from_str("done"),
        &JsValue::from_bool(done),
    )
    .map_err(|_| JsError::new("set done failed"))?;
    Ok(batch.into())
}

#[cfg(target_arch = "wasm32")]
fn raw_entries_to_js(entries: Vec<(Vec<u8>, Vec<u8>)>) -> Result<js_sys::Array, JsError> {
    let out = js_sys::Array::new();
    for (key, value) in entries {
        let entry = js_sys::Object::new();
        js_sys::Reflect::set(
            &entry,
            &JsValue::from_str("key"),
            &JsValue::from_str(&String::from_utf8_lossy(&key)),
        )
        .map_err(|_| JsError::new("set key failed"))?;
        js_sys::Reflect::set(
            &entry,
            &JsValue::from_str("keyBytes"),
            &js_sys::Uint8Array::from(key.as_slice()).into(),
        )
        .map_err(|_| JsError::new("set keyBytes failed"))?;
        js_sys::Reflect::set(
            &entry,
            &JsValue::from_str("value"),
            &js_sys::Uint8Array::from(value.as_slice()).into(),
        )
        .map_err(|_| JsError::new("set value failed"))?;
        out.push(&entry);
    }

    Ok(out)
}
