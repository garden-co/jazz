...
fn insert_todo(runtime: &WasmRuntime, title: &str, completed: bool) -> String {
    ...
    named_values.insert("title".to_string(), Value::Text(title.to_string()));
    named_values.insert("completed".to_string(), Value::Boolean(completed));
    let values = serde_wasm_bindgen::to_value(&named_values).unwrap();
...
fn find_value_for_column<'a>(row: &'a serde_json::Value, column: &str) -> &'a serde_json::Value {
    let values = row["values"].as_array().expect("row.values is array");
    for entry in values {
        if entry["column"] == column {
            return &entry["value"];
        }
    }
    panic!("column `{column}` not found in row {row:?}");
}
...
async fn opfs_crud_round_trip() {
    ...
    assert_eq!(rows.len(), 1, "expected 1 todo, got {:?}", rows);
    assert_eq!(find_value_for_column(&rows[0], "title"), "Buy milk");
    assert_eq!(find_value_for_column(&rows[0], "completed"), false);
...
async fn opfs_index_operations() {
    ...
    for row in &rows {
        assert_eq!(
            find_value_for_column(row, "completed"),
            true,
...
async fn opfs_runtime_core_e2e() {
    ...
    assert_eq!(find_value_for_column(&rows[0], "title"), "Original");
    assert_eq!(find_value_for_column(&rows[0], "completed"), false);
    ...
    assert_eq!(rows.len(), 1);
    assert_eq!(find_value_for_column(&rows[0], "title"), "Original");
    assert_eq!(find_value_for_column(&rows[0], "completed"), true);
