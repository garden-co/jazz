//! Async B-tree over an opaque [`AsyncPageStore`].
//!
//! It supports create/open, put/get/delete/forward range, and checkpoint.
//! Page rebalancing and overflow values remain on the synchronous production
//! path and must land before this becomes the default browser tree.
...
pub struct AsyncPageBTreeOptions {
    pub page_size: usize,
    pub cache_pages: usize,
}
impl Default for AsyncPageBTreeOptions {
    fn default() -> Self {
        Self {
            page_size: 16 * 1024,
            cache_pages: 256,
...
pub struct AsyncPageBTree<S: AsyncPageStore> {
    store: S,
    options: AsyncPageBTreeOptions,
    root: Option<u64>,
    total_pages: u64,
    generation: u64,
...
mod tests {
    ...
    fn async_tree_splits_ranges_and_reopens() {
        futures::executor::block_on(async {
            let options = AsyncPageBTreeOptions {
                page_size: 4096,
                cache_pages: 3,
            };
            let mut tree = AsyncPageBTree::open(Store::default(), options)
                .await
                .unwrap();
            for i in 0..900u32 {
            ...
            let mut reopened = AsyncPageBTree::open(store, options).await.unwrap();
            assert_eq!(
            ...
            assert!(reopened.delete(b"k0420").await.unwrap());
            assert_eq!(reopened.get(b"k0420").await.unwrap(), None);
            reopened.checkpoint().await.unwrap();
            let store = reopened.into_store();
            let mut after_delete = AsyncPageBTree::open(store, options).await.unwrap();
            assert_eq!(after_delete.get(b"k0420").await.unwrap(), None);
            assert_eq!(
                after_delete.get(b"k0421").await.unwrap(),
                Some(b"v0421".to_vec())
            );
        });
    }
...
impl<S: AsyncPageStore> AsyncPageBTree<S> {
    pub async fn open(store: S, options: AsyncPageBTreeOptions) -> Result<Self, BTreeError> {
        let mut tree = Self {
            store,
            options,
            root: None,
    ...
    /// Delete one key from the resident tree.
    ///
    /// Empty pages are deliberately retained in this first async port. That
    /// keeps lookup correctness and checkpoint atomicity independent of page
    /// rebalancing; compaction can reclaim them later without changing the
    /// page-store contract.
    pub async fn delete(&mut self, key: &[u8]) -> Result<bool, BTreeError> {
        ...
        loop {
            match decode_page(&self.page(id).await?, self.options.page_size)? {
                Page::Leaf { mut entries, next } => {
                    let at = entries.partition_point(|(candidate, _)| candidate.as_slice() < key);
                    if entries
                        .get(at)
                        .is_none_or(|(candidate, _)| candidate.as_slice() != key)
                    {
                        return Ok(false);
                    }
                    entries.remove(at);
                    self.write(id, Page::Leaf { entries, next })?;
                    return Ok(true);
                }
                Page::Internal { keys, children } => {
                    id = children[keys.partition_point(|candidate| candidate.as_slice() <= key)];
                }
                _ => return Err(BTreeError::Corrupt("non-tree page in delete".into())),
            }
        }
    }
    async fn insert(
