...
- **Cache Key**: A unique identifier generated from the resolve query and root CoValue ID
...
**User Story:** As a developer, I want SubscriptionScope to generate cache keys from resolve queries and root CoValue IDs, so that queries can be uniquely identified for caching.

#### Acceptance Criteria

1. WHEN a SubscriptionScope is marked as cacheable THEN the SubscriptionScope SHALL generate a cache key from the resolve query and the root CoValue ID
2. WHEN generating a cache key THEN the SubscriptionScope SHALL combine the resolve query and root CoValue ID into a unique identifier
3. WHEN a SubscriptionScope is not marked as cacheable THEN the SubscriptionScope SHALL not generate a cache key
...
1. WHEN the storage layer calls loadCache with a cache key THEN the storage layer SHALL retrieve all CoValues associated with the cache bucket
2. WHEN loadCache retrieves cached CoValues THEN the storage layer SHALL make the cached data available for subsequent operations
3. WHEN a cache key has no associated cache bucket THEN loadCache SHALL complete without loading any data
...
1. WHEN a CoValue is stored and has cache keys THEN the storage layer SHALL call storeCache for each cache key with the CoValue data
2. WHEN storeCache is called with a cache key and CoValue THEN the storage layer SHALL update the cache bucket content with the CoValue data
3. WHEN a CoValue has no cache keys THEN the storage layer SHALL not call storeCache during CoValue storage
