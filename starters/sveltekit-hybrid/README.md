...
path. New visitors use a device-local identity, and signing up preserves
...
src/
  ...
  routes/
    +layout.svelte                 ← switches between local-first and JWT Jazz clients
...
instance (from `jazz-tools/svelte`) for the local-first path and fetches
...
interact with a stale local-first client during a sign-up → authenticated
transition.
...
5. All subsequent JWTs carry `sub: <jazz-user-id>`, so the server sees
   the same principal that created the local-first todos. Same Jazz id,
   same data.
...
briefly rendering the stale local-first one.
...
new local-first Jazz user id `Y`), creates todos on device B, and then
