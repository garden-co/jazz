...
async function restorePhrase(page: Page, phrase: string) {
  ...
  await page.waitForFunction((input) => !input.isConnected, outgoingInput, { timeout: TIMEOUT });
...
test("recovery phrase round-trips the local-first identity", async ({ page }) => {
  ...
  const retainedInput = await page.getByLabel(TODO_INPUT_LABEL).elementHandle();
  if (!retainedInput) throw new Error("The restored client did not render its todo input");
  await openBackup(page);
