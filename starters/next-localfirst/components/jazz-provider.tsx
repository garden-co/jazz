...
export function useLocalAccount() {
  ...
  if (!value) throw new Error("useLocalAccount must be used inside JazzProvider");
  return value;
}
...
export function JazzProvider({ children }: React.PropsWithChildren) {
  ...
  return <AccountRuntime accounts={accounts}>{children}</AccountRuntime>;
}

function AccountRuntime({ accounts, children }: React.PropsWithChildren<{ accounts: Accounts }>) {
  const [runtime, setRuntime] = useState({ account: accounts.getLoggedIn()!, generation: 0 });
  ...
  function restore(secret: string): Promise<void> {
    const transition = restoreQueue.current.then(async () => {
      ...
      // A failed sync barrier leaves the old client usable.
  ...
  return (
    <AccountContext.Provider value={{ accounts, account: runtime.account, restore }}>
