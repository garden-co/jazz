...
| `CB-003` | High     | fixed locally     | `SubscriptionStream` drop-time cleanup synchronously borrows the async node. Dropping during a cold node operation can panic. Local and upstream cleanup paths share this lifecycle risk.                                                                                                                    | `fix/async-lifecycle-review` queues state-identified retirement, makes explicit close terminal, closes shutdown admission before storage awaits, and retires the full connection/runtime shell.                                          | Land the mutation-sensitive contention, successor-rollover, terminal-stream, and concurrent-close receipts; `Drop` must never block, spawn, leak, or acquire the async node lock, and local/upstream cleanup must drain under normal node ownership. |
...
| `CB-011` | High     | in review         | A deployed policy comparing `$createdBy` or `$updatedBy` with `session.user_id` bound a UUID provenance column to the raw external principal string, so the first authority receipt failed with `Query: operand type mismatch`. This blocks secure built-in ownership policies for opaque provider subjects. | `fix/magic-provenance-policy-types` lowers only these comparisons through a protected authenticated-author binding and preserves stored provenance when reconstructing prior policy rows.                                                | Land mutation-sensitive receipts proving creator insert/read/update/delete success, another session's rejection, `$updatedBy` coverage, and that ordinary app string ownership remains raw-principal based.                                          |
