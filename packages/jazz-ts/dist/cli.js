...
async function generateSqlForSchemaFile(tsFile) {
...
async function runJazzBuild(jazzBin, schemaDir) {
    return new Promise((resolve, reject) => {
        ...
        child.on("error", (err) => {
            if (err.code === "ENOENT") {
                reject(new Error(`jazz binary not found at '${jazzBin}'. Use --jazz-bin to specify the path.`));
            }
            else {
                reject(err);
            }
