...
const mockGetActiveSyntheticAuth = vi.fn().mockReturnValue({
    ...
    profile: { id: '1', name: 'User 1', mode: 'anonymous', token: 'fallback-token' }
});
...
describe('useLinkExternalIdentity', () => {
    ...
    it('calls linkExternalIdentity with provided auth fields', async () => {
        ...
        expect(mockLinkExternalIdentity).toHaveBeenCalledWith('https://jazz.example.com', {
            jwtToken: 'jwt-abc',
            localAuthMode: 'demo',
            localAuthToken: 'explicit-token',
    ...
    it('falls back to synthetic user auth when local auth not provided', async () => {
        ...
        expect(mockLinkExternalIdentity).toHaveBeenCalledWith('https://jazz.example.com', expect.objectContaining({
            localAuthMode: 'anonymous',
            localAuthToken: 'fallback-token'
        }), undefined);
    });
    ...
    it('passes serverPathPrefix and logPrefix through', async () => {
        ...
        expect(mockLinkExternalIdentity).toHaveBeenCalledWith('https://jazz.example.com', expect.objectContaining({ pathPrefix: '/apps/test' }), '[test]');
    });
    it('throws when no local auth can be resolved', async () => {
        ...
        await expect(link({ jwtToken: 'jwt' })).rejects.toThrow('Local auth mode and token are required');
    });
});
