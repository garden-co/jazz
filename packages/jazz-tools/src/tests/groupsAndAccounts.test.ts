...
describe("Group inheritance", () => {
  ...
  test("Revoked group extensions", async () => {
    const me = await createJazzTestAccount({
      isCurrentActiveAccount: true,
    });
    ...
    const parentGroup = Group.create({ owner: me });
    const alice = await Account.createAs(me, {
    ...
    const bob = await Account.createAs(me, {
    ...
    // `childGroup`'s members now include `bob`
    expect(
      childGroup.members.map((member) => member.account?.id),
    ).toContainEqual(bob.id);

    // `childGroup` has `parentGroup`'s members (in this case, `alice` as a writer)
    childGroup.extend(parentGroup);
    expect(childGroup.getRoleOf(alice.id)).toBe("writer");

    // Verify that `childGroup` inherits `alice` as a member
    expect(
      childGroup.members.map((member) => member.account?.id),
    ).toContainEqual(alice.id);

    // `childGroup` no longer has `parentGroup`'s members
    childGroup.revokeExtend(parentGroup);
    expect(childGroup.getRoleOf(bob.id)).toBe("reader");
    // Verify that `childGroup` no longer inherits `alice` as a member
    expect(
      childGroup.members.map((member) => member.account?.id),
    ).not.toContainEqual(alice.id);
    expect(childGroup.getRoleOf(alice.id)).toBe(undefined);
  });
...
describe("Account permissions", () => {
  ...
  test("permissions for unextended group's members", async () => {
    // Create test accounts
    const me = await createJazzTestAccount({
      isCurrentActiveAccount: true,
    });

    // Create child group and members
    const childGroup = Group.create({ owner: me });
    const childWriter = await Account.createAs(me, {
      creationProps: { name: "ChildWriter" },
    });
    const childReader = await Account.createAs(me, {
      creationProps: { name: "ChildReader" },
    });
    const childWriteOnly = await Account.createAs(me, {
      creationProps: { name: "ChildWriteOnly" },
    });
    childGroup.addMember(childWriter, "writer");
    childGroup.addMember(childReader, "reader");
    childGroup.addMember(childWriteOnly, "writeOnly");

    // Create parent group and members
    const parentGroup = Group.create({ owner: me });
    const parentWriter = await Account.createAs(me, {
      creationProps: { name: "ParentWriter" },
    });
    const parentReader = await Account.createAs(me, {
      creationProps: { name: "ParentReader" },
    });
    const parentWriteOnly = await Account.createAs(me, {
      creationProps: { name: "ParentWriteOnly" },
    });
    parentGroup.addMember(parentWriter, "writer");
    parentGroup.addMember(parentReader, "reader");
    parentGroup.addMember(parentWriteOnly, "writeOnly");

    // Create test object
    const testObject = CoMap.create({}, { owner: childGroup });

    // Setup group inheritance
    childGroup.extend(parentGroup);

    // Test `canRead` permissions
    expect(me.canRead(testObject)).toBe(true);
    expect(childWriter.canRead(testObject)).toBe(true);
    expect(childReader.canRead(testObject)).toBe(true);
    // `writeOnly` can read self's changes
    expect(childWriteOnly.canRead(testObject)).toBe(true);
    expect(parentWriter.canRead(testObject)).toBe(true);
    expect(parentReader.canRead(testObject)).toBe(true);
    // When extending groups, only `admin`, `writer` and `reader` roles are inherited.
    expect(parentWriteOnly.canRead(testObject)).toBe(false);

    // Test `canWrite` permissions
    expect(me.canWrite(testObject)).toBe(true);
    expect(childWriter.canWrite(testObject)).toBe(true);
    expect(childReader.canWrite(testObject)).toBe(false);
    expect(childWriteOnly.canWrite(testObject)).toBe(true);
    expect(parentWriter.canWrite(testObject)).toBe(true);
    expect(parentReader.canWrite(testObject)).toBe(false);
    expect(parentWriteOnly.canWrite(testObject)).toBe(false);

    // Test `canAdmin` permissions
    expect(me.canAdmin(testObject)).toBe(true);
    expect(childWriter.canAdmin(testObject)).toBe(false);
    expect(childReader.canAdmin(testObject)).toBe(false);
    expect(childWriteOnly.canAdmin(testObject)).toBe(false);
    expect(parentWriter.canAdmin(testObject)).toBe(false);
    expect(parentReader.canAdmin(testObject)).toBe(false);
    expect(parentWriteOnly.canAdmin(testObject)).toBe(false);

    // Revoke `parentGroup` access to `childGroup`
    childGroup.revokeExtend(parentGroup);
    ...
    expect(childReader.canRead(testObject)).toBe(true);
    expect(childWriteOnly.canRead(testObject)).toBe(true);
    expect(parentWriter.canRead(testObject)).toBe(false);
    expect(parentReader.canRead(testObject)).toBe(false);
    expect(parentWriteOnly.canRead(testObject)).toBe(false);
    ...
    expect(childReader.canWrite(testObject)).toBe(false);
    expect(childWriteOnly.canWrite(testObject)).toBe(true);
    expect(parentWriter.canWrite(testObject)).toBe(false);
    expect(parentReader.canWrite(testObject)).toBe(false);
    expect(parentWriteOnly.canWrite(testObject)).toBe(false);
