...
function handleHello(port: MessagePort, message: BrowserBrokerTabMessage): string | null {
  ...
  const interruptedResetLeader =
    resetState !== null &&
    leader?.tabId === message.tabId &&
    !leader.ready &&
    resetState.promotedLeadershipId === leader.leadershipId
      ? { reset: resetState, leadershipId: leader.leadershipId }
      : null;
  ...
  if (interruptedResetLeader) {
    // Reconnecting replaces the control port that received become-leader. A
    // not-yet-ready reset leader can no longer complete that promotion, so
    // immediately move the reset to another candidate instead of leaving it
    // stranded until an unrelated heartbeat happens to evict this tab.
    markLeaderCandidateFailed(message.tabId);
    clearLeader(interruptedResetLeader.leadershipId, {
      demoteLeader: false,
      removeLeaderTab: false,
    });
    removeTabFromActiveReset(message.tabId);
    interruptedResetLeader.reset.promotedLeadershipId = null;
    void promoteResetLeader(interruptedResetLeader.reset);
