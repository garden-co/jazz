...
import {
  createReactNativeCoreUnsupportedError,
  REACT_NATIVE_CORE_UNSUPPORTED_MESSAGE,
...
describe("react-native react-native core support", () => {
  it("fails fast instead of routing through a legacy bridge", async () => {
    expect(createReactNativeCoreUnsupportedError().message).toBe(
      REACT_NATIVE_CORE_UNSUPPORTED_MESSAGE,
    );
    ...
    await expect(loadJazzRn()).rejects.toThrow(REACT_NATIVE_CORE_UNSUPPORTED_MESSAGE);
  });
  ...
  it("does not mint tokens through the old jazz-rn native module", () => {
    ...
    ).toThrow(REACT_NATIVE_CORE_UNSUPPORTED_MESSAGE);
    expect(() =>
