...
it("accepts one competing group reconciliation and converges the space on its accepted epoch", async () => {
  ...
  try {
    ...
    expect(
      groupDeliveries
        .filter((row) => row.epochId === groupSuccessors[0]!.epochId)
        .map((row) => row.recipientAccountId)
        .sort(),
    ).toEqual(
      accounts
        .slice(0, 2)
