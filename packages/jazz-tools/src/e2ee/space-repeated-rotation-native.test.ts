...
it("replays native Node space rotations and delivers complete history to a newly approved device", async () => {
  ...
  try {
    ...
    const open = async (accountStore: ReturnType<typeof store>) => {
    ...
    const creator = await open(aliceStore);
    const removed = await open(bobStore);
    const alice = { account: sessions[0]!.getSnapshot().account! };
    const bob = { account: sessions[1]!.getSnapshot().account! };
    const [first] = await creator.e2ee.devices.list();
    const [bobDevice] = await removed.e2ee.devices.list();
    const remaining = await open(aliceStore);
    expect(sessions[2]!.getSnapshot().account!.id).toBe(alice.account.id);
    ...
    const newcomer = await open(aliceStore);
    expect(sessions[3]!.getSnapshot().account!.id).toBe(alice.account.id);
    const listed = await newcomer.e2ee.devices.list();
    expect(
      listed
    ...
    expect(listed.filter((device) => device.state === "pending")).toHaveLength(1);
    expect(
      listed.every((device) => "keyReadiness" in device && device.keyReadiness === "not-verified"),
    ).toBe(true);
    const newRequest = listed.find((device) => device.state === "pending")!;
