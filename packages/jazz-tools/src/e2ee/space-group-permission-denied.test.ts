...
it("keeps a space in maintenance when Jazz denies recipient-group rotation", async () => {
  ...
  const policies = definePermissions(app, ({ policy, session }) => {
    ...
    // No group-successor insert permission: key possession cannot override Jazz.
