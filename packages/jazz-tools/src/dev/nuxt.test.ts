...
import { jazzNuxt, __resetJazzNuxtPluginForTests } from "./nuxt.js";
import type { ViteDevServer } from "./vite.js";

const tempRoots = createTempRootTracker();
const originalJazzAppId = process.env.NUXT_PUBLIC_JAZZ_APP_ID;
const originalJazzServerUrl = process.env.NUXT_PUBLIC_JAZZ_SERVER_URL;
const originalBackendSecret = process.env.BACKEND_SECRET;

function makeViteServer(command: "serve" | "build", root = "/tmp/jazz-nuxt-test"): ViteDevServer {
  return {
...
beforeEach(() => {
  delete process.env.NUXT_PUBLIC_JAZZ_APP_ID;
  delete process.env.NUXT_PUBLIC_JAZZ_SERVER_URL;
  delete process.env.JAZZ_ADMIN_SECRET;
  delete process.env.BACKEND_SECRET;
});

afterEach(async () => {
  await __resetJazzNuxtPluginForTests();
  await tempRoots.cleanup();
  await rm(join("/tmp/jazz-nuxt-test", ".env"), { force: true }).catch(() => undefined);
  vi.restoreAllMocks();

  if (originalJazzAppId === undefined) {
    delete process.env.NUXT_PUBLIC_JAZZ_APP_ID;
  } else {
    process.env.NUXT_PUBLIC_JAZZ_APP_ID = originalJazzAppId;
  }

  if (originalJazzServerUrl === undefined) {
    delete process.env.NUXT_PUBLIC_JAZZ_SERVER_URL;
  } else {
    process.env.NUXT_PUBLIC_JAZZ_SERVER_URL = originalJazzServerUrl;
  }
...
describe("jazzNuxt", () => {
  it("starts a local server in dev and injects NUXT_PUBLIC_JAZZ_* env vars", async () => {
    const port = await getAvailablePort();
    const root = await tempRoots.create("jazz-nuxt-test-");
    await mkdir(join(root, "server"), { recursive: true });
    await writeFile(join(root, "schema.ts"), todoSchema());

    const plugin = jazzNuxt({ server: { port, adminSecret: "nuxt-test-admin" } });
    ...
    const schemasResponse = await fetch(
      `http://127.0.0.1:${port}/apps/${viteServer.config.env!.NUXT_PUBLIC_JAZZ_APP_ID}/schemas`,
      { headers: { "X-Jazz-Admin-Secret": "nuxt-test-admin" } },
    );
    ...
    expect(viteServer.config.env!.NUXT_PUBLIC_JAZZ_APP_ID).toBeTruthy();
    expect(viteServer.config.env!.NUXT_PUBLIC_JAZZ_SERVER_URL).toBe(`http://127.0.0.1:${port}`);
    expect(process.env.NUXT_PUBLIC_JAZZ_APP_ID).toBe(
      viteServer.config.env!.NUXT_PUBLIC_JAZZ_APP_ID,
    );
    expect(process.env.NUXT_PUBLIC_JAZZ_SERVER_URL).toBe(`http://127.0.0.1:${port}`);
  }, 30_000);

  it("does not start a server during build", async () => {
    const spy = vi.spyOn(devServer, "startLocalJazzServer");

    const plugin = jazzNuxt({ server: { port: 19999, adminSecret: "build-admin" } });
    await (plugin.configureServer as (s: ViteDevServer) => Promise<void>)(makeViteServer("build"));

    expect(spy).not.toHaveBeenCalled();
    expect(process.env.NUXT_PUBLIC_JAZZ_APP_ID).toBeUndefined();
  });

  it("does not start a server when server:false", async () => {
    const spy = vi.spyOn(devServer, "startLocalJazzServer");

    const plugin = jazzNuxt({ server: false });
  ...
  it("passes BACKEND_SECRET from process.env when not set in server options", async () => {
    process.env.BACKEND_SECRET = "env-backend-secret";
    ...
    const plugin = jazzNuxt({ server: { port: 19993, adminSecret: "be-secret-admin" } });
    await (plugin.configureServer as (s: ViteDevServer) => Promise<void>)(makeViteServer("serve"));

    expect(startSpy).toHaveBeenCalledWith(
      expect.objectContaining({ backendSecret: "env-backend-secret" }),
    );
  });
  ...
  it("injects BACKEND_SECRET from the server handle", async () => {
    ...
    const plugin = jazzNuxt({ server: { port: 19998, adminSecret: "backend-secret-admin" } });
    await (plugin.configureServer as (s: ViteDevServer) => Promise<void>)(makeViteServer("serve"));
  ...
  it("builds jwksUrl from Vite's configured host and port", async () => {
    ...
    const root = await tempRoots.create("jazz-nuxt-jwks-test-");
    const plugin = jazzNuxt({ server: { port: 19995, adminSecret: "jwks-admin" } });
  ...
  it("respects APP_ORIGIN when set, over Vite's configured port", async () => {
    ...
    try {
      const root = await tempRoots.create("jazz-nuxt-apporigin-test-");
      const plugin = jazzNuxt({ server: { port: 19994, adminSecret: "app-origin-admin" } });
      const viteServer: ViteDevServer = {
        config: { root, command: "serve", env: {}, server: { port: 3000 } },
        httpServer: { once() {} },
        ws: { send() {} },
  ...
  it("connects to an existing server via NUXT_PUBLIC_JAZZ_SERVER_URL env var", async () => {
    process.env.NUXT_PUBLIC_JAZZ_SERVER_URL = "http://jazz-test-server:4000";
    process.env.NUXT_PUBLIC_JAZZ_APP_ID = "00000000-0000-0000-0000-000000000010";

    vi.spyOn(devServer, "startLocalJazzServer");
    vi.spyOn(devServer, "pushSchemaCatalogue").mockResolvedValue({ hash: "abc" });
    vi.spyOn(schemaWatcher, "watchSchema").mockReturnValue({ close: vi.fn() });

    const plugin = jazzNuxt({ adminSecret: "env-test-admin" });
    ...
    expect(viteServer.config.env!.NUXT_PUBLIC_JAZZ_SERVER_URL).toBe("http://jazz-test-server:4000");
  });
  ...
  it("throws when connecting to an existing server without appId", async () => {
    ...
    const plugin = jazzNuxt({ adminSecret: "admin" });
    await expect(
      (plugin.configureServer as (s: ViteDevServer) => Promise<void>)(makeViteServer("serve")),
  ...
  it("stops the server and closes the watcher when the close hook fires", async () => {
    ...
    const root = await tempRoots.create("jazz-nuxt-close-test-");
    ...
    const plugin = jazzNuxt({ server: { port: 19997, adminSecret: "close-hook-admin" } });
    await (plugin.configureServer as (s: ViteDevServer) => Promise<void>)(viteServer);
  ...
  it("surfaces schema push failures as HMR errors", async () => {
    ...
    const root = await tempRoots.create("jazz-nuxt-hmr-test-");
    const wsSend = vi.fn();
    const viteServer: ViteDevServer = {
      config: { root, command: "serve", env: {} },
      httpServer: { once() {} },
    ...
    const plugin = jazzNuxt({ server: { port: 19996, adminSecret: "hmr-error-admin" } });
...
describe("jazzNuxt config hook", () => {
  it("adds jazz-napi to ssr.external", () => {
    const plugin = jazzNuxt();
  ...
  it("preserves existing ssr.external entries", () => {
    const plugin = jazzNuxt();
    const config = (plugin as { config?: (c: Record<string, unknown>) => unknown }).config;
...
describe("dev barrel", () => {
  it("exposes jazzNuxt", async () => {
    const dev = await import("./index.js");
    expect((dev as Record<string, unknown>).jazzNuxt).toBeDefined();
  });
});
