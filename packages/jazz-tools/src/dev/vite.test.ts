...
const originalTelemetryCollectorUrl = process.env.VITE_JAZZ_TELEMETRY_COLLECTOR_URL;
...
afterEach(async () => {
  ...
  if (originalTelemetryCollectorUrl === undefined) {
    ...
    process.env.VITE_JAZZ_TELEMETRY_COLLECTOR_URL = originalTelemetryCollectorUrl;
  }
...
describe("jazzPlugin", () => {
  ...
  it("passes server telemetry through the managed runtime", async () => {
    ...
    expect(process.env.VITE_JAZZ_TELEMETRY_COLLECTOR_URL).toBe("http://localhost:4317");
  });
