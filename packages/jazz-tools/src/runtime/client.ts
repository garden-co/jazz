...
export interface Runtime {
  ...
  /**
   * Start the WASM-internal WebSocket transport (WasmRuntime only).
   * When present, the runtime manages the WebSocket connection itself instead
   * of relying on the TS-layer HTTP streaming transport.
   */
  connect?(wsUrl: string, authJson: string): void;
  /** Stop the WASM-internal WebSocket transport. */
  disconnect?(): void;
  /**
   * Consume the most recent transport auth-rejection reason, if any.
   * After it returns a string the transport is permanently stopped — the
   * host must refresh credentials and call `connect()` again.
   */
...
export interface ConnectSyncRuntimeOptions {
  ...
  /**
   * Use backend-secret auth for the WebSocket transport.
   * When true, `context.backendSecret` is sent in the WS handshake instead
   * of end-user auth. Pass true from server-side contexts that always
   * authenticate as a backend (e.g. JazzContext with backendSecret).
   */
  useBackendSyncAuth?: boolean;
}
...
export class JazzClient {
  ...
  private authFailurePoller: ReturnType<typeof setInterval> | null = null;
  ...
  private constructor(
    ...
    this.startAuthFailurePoller();
  }

  /**
   * Poll the runtime for transport-side auth rejections and surface them
   * via `onAuthFailure`. The Rust transport stops reconnecting permanently
   * when it sees a 401/403 from the server, so the host needs a way to
   * find out and refresh credentials. Polling is cheap (a single atomic
   * read in Rust) and avoids adding a cross-language callback channel.
   */
  private startAuthFailurePoller(): void {
    if (!this.runtime.takeAuthFailure || !this.onAuthFailure) return;
    if (this.authFailurePoller !== null) return;
    this.authFailurePoller = setInterval(() => {
      const reason = this.runtime.takeAuthFailure?.() ?? null;
      if (reason) {
        this.onAuthFailure?.(reason as AuthFailureReason);
      }
    }, 500);
  }

  private stopAuthFailurePoller(): void {
    if (this.authFailurePoller !== null) {
      clearInterval(this.authFailurePoller);
      this.authFailurePoller = null;
    }
  }
  ...
  static connectWithRuntime(
    ...
    // Apply backend auth BEFORE setupSync so the WS transport connects with correct credentials.
    if (runtimeOptions?.useBackendSyncAuth) {
      client.useBackendSyncAuth = true;
    }
  ...
  async shutdown(): Promise<void> {
    this.stopAuthFailurePoller();
    // Tear down the WS transport if the runtime owns it. This also runs
    // remove_server() on the runtime side so we don't leak upstream state
    // for this server_id. (Mirrored in close() below for WASM, which
    // teardowns transport internally.)
    if (this.runtime.disconnect) {
      this.runtime.disconnect();
    }
    // Close runtime if it supports explicit shutdown (e.g., NapiRuntime).
    // For WASM runtimes, close() handles WebSocket teardown internally.
  ...
  /**
   * Build the auth JSON payload for `runtime.connect()`.
   *
   * Maps the supported auth modes to the Rust `AuthConfig` struct shape:
   *   { jwt_token, backend_secret, admin_secret, backend_session }
   */
  private buildWsAuthJson(): string {
    const auth = this.getSyncAuth();
    return JSON.stringify({
      jwt_token: auth.jwtToken ?? null,
      backend_secret: auth.backendSecret ?? null,
      admin_secret: null,
