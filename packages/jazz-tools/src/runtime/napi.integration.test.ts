...
describe("NAPI integration", () => {
  ...
  it("subscribes to the the pilot customer team access-edge holder through the local server route", async () => {
    const appId = randomUUID();
    ...
    const schemaDir = await createTempDir("jazz-napi-pilot-app-holder-schema-");
    ...
    const pilot-appSchema = (await loadCompiledSchema(schemaDir)).wasmSchema;
