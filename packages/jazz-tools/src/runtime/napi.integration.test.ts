...
async function createBoredmRealSchemaDir(options?: {
  ...
  const schemaDir = await createTempDir("jazz-napi-boredm-real-schema-");
  const fixtureJsonPath = fileURLToPath(new URL("schema-source.json", BOREDM_REAL_FIXTURE_DIR));
  await writeFile(
...
describe("NAPI integration", () => {
  ...
  it("resolves global waits after deploying the BoreDM real schema fixture", async () => {
    const appId = randomUUID();
    const backendSecret = "napi-boredm-global-wait-deploy-secret";
    const adminSecret = "napi-boredm-global-wait-deploy-admin-secret";
    ...
    try {
      ...
      runtimeData = await createTempRuntimeData("jazz-napi-boredm-global-wait-deploy-runtime-");
      context = createJazzContext({
        appId,
        app: { wasmSchema: boredmSchema },
        permissions: {},
      ...
      const createdLocation = await withTimeout(
        backendDb
          .insert(
            locationTable,
            {
              display_address: "BoreDM deploy schemaDir global wait",
            },
            { id: randomUUID() },
          )
          .wait({ tier: "global" }),
        90_000,
        "BoreDM deploy schemaDir backend global insert wait timed out",
      );
  ...
  it("publishes inherited seeded-reachability permissions through the local server route", async () => {
    const appId = randomUUID();
    const backendSecret = "napi-boredm-permissions-secret";
    const adminSecret = "napi-boredm-permissions-admin-secret";
    const schemaDir = await createTempDir("jazz-napi-boredm-permissions-schema-");
    const server = await startLocalJazzServer({
      appId,
      backendSecret,
      adminSecret,
