...
async function expectProvenancePredicateUsesCoreMilliseconds(
  ...
  runtime.insert(
    ...
    // Native runtime mutation contexts use the public microsecond write
    // representation, which the adapter validates and converts to core's
    // physical-millisecond CurrentRow provenance.
    JSON.stringify({ updated_at: updatedAtMs * 1_000 }),
  );
