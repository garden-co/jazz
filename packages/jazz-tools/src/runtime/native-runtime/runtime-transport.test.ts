...
describe("NativeRuntimeAdapter server transport", () => {
  ...
  it("wakes downstream peer transports after receiving server frames", async () => {
    const sockets: FakeWebSocket[] = [];
    globalThis.WebSocket = class extends FakeWebSocket {
      constructor(url: string) {
        super(url);
    ...
    let peerTransportWakes = 0;
    runtime.onPeerTransportWork(() => {
      peerTransportWakes += 1;
    });

    runtime.connect("ws://127.0.0.1:4200/apps/app-a/ws", "{}");
    await waitForFakeWebSocketNegotiation();
    await waitForServerPumpTimer();
    peerTransportWakes = 0;

    sockets[0]!.emitMessage(encodeWebSocketFrameBatch([Uint8Array.from([1, 42])]));
    await waitForServerPumpTimer();

    expect(peerTransportWakes).toBeGreaterThan(0);
  });
});
