...
type AccountConfigBinding = { accountId?: string; accountRegistryAuthority?: string };
const admitted = new WeakMap<object, Readonly<{ id: string; registry: string }>>();
...
export function assertAccountConfig(config: AccountConfigBinding): void {
  ...
  if (
    (config.accountId !== undefined || config.accountRegistryAuthority !== undefined) &&
    (!binding ||
