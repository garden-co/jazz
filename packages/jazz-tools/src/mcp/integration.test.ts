 ...
 *
 * Spawns `node packages/jazz-run/dist/index.js mcp` as a real child process,
 * communicates over stdio with newline-delimited JSON-RPC, and exercises the
 * full lifecycle and all three tools against the committed index artefacts.
 *
 ...
 *
 * Requires: `pnpm build` before running (dist/mcp/server.js and
 * packages/jazz-run/dist/index.js must exist, and bin/docs-index.db
 * must have been generated via `node dist/mcp/build-index.js`).
 */
...
// Relative to this source file: src/mcp/ → ../../.. → packages/ → jazz-run/dist/
const shimPath = join(here, "../../../jazz-run/dist/index.js");
...
describe("MCP integration: full lifecycle", () => {
  ...
  it("tools/list → three tools with correct names", async () => {
    ...
    const res = await server.recv();
    const tools =
      ((res.result as Record<string, unknown>).tools as Array<{
        name: string;
      }>) ?? [];
  ...
  it("search_docs → returns results with title and slug for a known term", async () => {
    ...
    const res = await server.recv();
    const content = (
      (res.result as Record<string, unknown>).content as Array<{
        type: string;
        text: string;
      }>
    )[0]!;
  ...
  it("get_doc → markdown with title heading and body content", async () => {
    ...
    const listText = (
      (listRes.result as Record<string, unknown>).content as Array<{
        text: string;
      }>
    )[0]!.text;
  ...
  it("list_pages → markdown list with title, slug, description per page", async () => {
    ...
    expect(text.length).toBeGreaterThan(0);
    // The quickstart slug appears in every jazz1 docs index
