...
describe("vue/createJazzClient runtime dedup", () => {
  beforeEach(() => {
    mocks.reset();
  });

  it("collapses two same-identity clients onto a single runtime", async () => {
    const config: DbConfig = { appId: "vue-dedup-shared", serverUrl: "https://jazz.example.com" };
    mocks.createDb.mockResolvedValue(mockDb("vue-dedup-shared"));
  ...
  it("keeps distinct identities on separate runtimes (single-screen multi-principal)", async () => {
    mocks.createDb.mockImplementation(async () => mockDb("vue-dedup-multi"));

    await createJazzClient({ appId: "vue-dedup-multi", secret: "principal-A" } as DbConfig);
    await createJazzClient({ appId: "vue-dedup-multi", secret: "principal-B" } as DbConfig);
