  ...
  if (coValue.verified.header.ruleset.type === "group") {
    ...
    for (const tx of coValue.verifiedTransactions) {
      ...
      if (
        ...
        transactorRoleAtTxTime !== "super-admin" &&
...
function isHigherRole(a: Role, b: Role | undefined) {
  ...
  if (b === "super-admin") return false;
  if (a === "super-admin") return true;
...
function determineValidTransactionsForGroup(
  ...
  for (const transaction of coValue.verifiedTransactions) {
    ...
    if (
      ...
      (assignedRole === "admin" || assignedRole === "super-admin")
    ) {
    ...
    /**
     * Super admins can do anything except demote other super-admins
     */
    if (transactorRole === "super-admin") {
      if (
        affectedMember !== transactor &&
        affectedMemberRole === "super-admin" &&
        assignedRole !== "super-admin"
      ) {
        markTransactionAsInvalid("Super-admins can only demote themselves.");
        continue;
      }
    ...
    if (transactorRole === "admin") {
      ...
      if (affectedMemberRole === "super-admin") {
    ...
    if (transactorRole === "adminInvite") {
      ...
      // TODO: Add superAdminInvite logic - superAdminInvite should only create super-admin
