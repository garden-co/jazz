  ...
  test("loading a coValue created on a different edge", async () => {
    ...
    expect(
      ...
      [
        ...
        "edge-italy -> storage | LAZY_LOAD Map",
        "storage -> edge-italy | LAZY_LOAD_RESULT Map sessions: empty",
  ...
  test("large coValue streaming from an edge to the core server and a client at the same time", async () => {
    ...
    expect(
      ...
      [
        ...
        "storage -> edge | LAZY_LOAD_RESULT Map sessions: header/100",
        ...
        "core -> storage | LAZY_LOAD Group",
        "storage -> core | LAZY_LOAD_RESULT Group sessions: empty",
        "core -> edge | KNOWN Group sessions: empty",
        "core -> storage | LAZY_LOAD Map",
        "storage -> core | LAZY_LOAD_RESULT Map sessions: empty",
