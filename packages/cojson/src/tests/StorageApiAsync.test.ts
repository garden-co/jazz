...
describe("StorageApiAsync", () => {
  ...
  describe("store", () => {
    test("should replace session history and update subsequent loads", async () => {
      ...
      await storage.store(fullContent, vi.fn());
      ...
      await storage.store(replacement, vi.fn());
    ...
    test("should replace multi-signature session history and stream on load", async () => {
      ...
      await storage.store(groupContent, vi.fn());

      const mapContent = getNewContentSince(map.core, emptyKnownState(map.id));
      await storage.store(mapContent, vi.fn());
      await storage.waitForSync(map.id, map.core);
      ...
      for (const msg of after) {
        checkNode.node.syncManager.handleNewContent(msg, "storage");
      }
