...
export class SyncManager {
  ...
  async sendNewContent(
    forKnown: CojsonInternalTypes.CoValueKnownState,
  ): Promise<void> {
    const { knownMessagesPool, contentMessagesPool } =
      await this.collectCoValueData(forKnown, {}, {});

    // reverse to send the top level id the last
    const collectedIds = Object.keys(knownMessagesPool).reverse();
    collectedIds.forEach((coId) => {
      this.sendStateMessage(knownMessagesPool[coId as RawCoID]);

      const contentMessages = contentMessagesPool[coId as RawCoID] || [];
  ...
  private async collectCoValueData(
    ...
    knownMessagesPool: Record<RawCoID, KnownStateMessage>,
    contentMessagesPool: Record<RawCoID, NewContentMessage[]>,
    asDependencyOf?: CojsonInternalTypes.RawCoID,
  ) {
    if (knownMessagesPool[coValueKnownState.id]) {
      return { knownMessagesPool, contentMessagesPool };
    }
    ...
    if (!coValueRow) {
      ...
      knownMessagesPool[coValueKnownState.id] = emptyKnownMessage;
      return { knownMessagesPool, contentMessagesPool };
    }
    ...
    const knownMessage: KnownStateMessage = { action: "known", ...ourKnown };
    asDependencyOf && (knownMessage.asDependencyOf = asDependencyOf);
    knownMessagesPool[ourKnown.id] = knownMessage;
    contentMessagesPool[ourKnown.id] = nonEmptyContentMessages;
