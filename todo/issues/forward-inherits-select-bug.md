...
- Where: `crates/jazz-tools/tests/policies_integration/inherited_policies.rs`
- Steps to reproduce:
...
- Expected: `alice` sees the child row via the parent-granted inherited SELECT path.
- Actual: the child row stays hidden, and follow-on scenarios that depend on inherited child visibility fail or time out.
