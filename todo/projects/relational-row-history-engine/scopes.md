...
## Row-History Engine Replacement — replace today's engine with row histories

- [ ] Add row-history storage primitives and key layout to `Storage`, with one physical keyed table space per user table, a visible region keyed by `($branch, $row_id)`, and a history region keyed by `($row_id, $version_id)`
...
- [ ] Replace any remaining production row-state mirrors with storage-backed row-apply / row-patch helpers plus a tiny monotonic clock in `RuntimeCore`
...
- [ ] Implement the new row-history storage path in `MemoryStorage`
- [ ] Implement the same row-history storage path in `FjallStorage`
...
- [ ] After those tests and benchmarks validate the direction, adapt the remaining storage backends to the same row-history model
...
- [ ] Keep any remaining test-only row cache helpers out of production surfaces entirely rather than just renaming them
...
- [ ] Introduce cross-row transactions as the write unit across multiple rows while keeping the row-history storage model unchanged
