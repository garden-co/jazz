...
Replace SSE + HTTP POST with **WebSocket** (ships now) behind a **TransportProvider** abstraction that keeps **WebTransport** open as a future upgrade path. Both provide bidirectional ordered streams over a single connection — WebSocket solves the ordering bugs today, WebTransport adds performance later.
...
The `TransportProvider` abstraction makes **single-stream WebTransport** a near drop-in upgrade when the time is right. The upgrade path:
...
For the first WebTransport step, sync logic and tests stay the same and the wire format stays the same. The main change is the underlying stream adapter. If we later want **multi-stream** WebTransport (separating sync control from large blob/file transfer), that becomes a second phase: `TransportManager` gains stream routing, but `RuntimeCore` and the outbox/inbox contract stay the same.
...
See **[WebSocket Transport Spec](../../specs/todo/a_mvp/websocket.md)** for full flow definitions.
...
│                                                      │
│  TransportHandle                                     │
│  ├── server_id  (stable across reconnects)           │
...
│  Recv loop: ws.recv() → decode event/control frame   │
...
**What gets deleted:** `JsSyncSender`, `NapiSyncSender`, `RnSyncSender`, `SyncSender` trait, the network-facing JS transport path in `sync-transport.ts`, `StreamController`, `sendSyncPayloadBatch()`, `readBinaryFrames()`, `reqwest`-based transport.

**What does not disappear:** the browser worker bridge still exists. Main thread ↔ worker sync still uses `postMessage`, `onSyncMessageReceived()`, and `onSyncMessageToSend()` for inter-runtime delivery. The simplification is that JS stops owning the **server** connection; it does not mean every sync-related JS helper vanishes.

**Single bidirectional connection for sync (both transports):**

Both transports use the same binary framing protocol Jazz already has (4-byte length-prefixed frames with JSON payloads). The core sync payloads stay the same, and the server-to-client sync data continues to flow through `ServerEvent` frames. The transport adds a WebSocket auth handshake and explicit connection-control handling, but the actual sync payload semantics do not change.
...
The TypeScript **network transport layer** (`sync-transport.ts`, 650+ LOC) gets removed or split apart. Transport logic moves to Rust's `TransportManager`. The JS API shrinks to:
...
No more `StreamController`, `sendSyncPayloadBatch()`, or `readBinaryFrames()` for server sync. JS calls `connect()` and Rust handles the server connection. Browser worker-bridge callbacks such as `onSyncMessageToSend()` still exist for main-thread ↔ worker delivery.
