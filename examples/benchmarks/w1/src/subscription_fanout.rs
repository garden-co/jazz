...
impl FanoutFixture {
    ...
    // Historical-backfill adaptation: released engines expose no Db-level
    // query-program compilation counter. This diagnostic-only receipt field is
    // reported as zero; it never influenced timing or the settled condition.
