...
const ISSUER = "https://band-binder.test";
const user = (subject: string) => JSON.stringify([ISSUER, subject]);
...
describe("BandBinder workspace roles", () => {
  it(
    ...
    async () => {
      ...
      const workspace = await owner
        .insert(app.workspaces, { name: "World tour", ownerSubject: user("owner") })
        .wait({ tier: "edge" });
      await owner
        .insert(app.members, { workspaceId: workspace.id, subject: user("owner"), role: "owner" })
        .wait({ tier: "edge" });
      ...
      const memberMembership = await owner
        .insert(app.members, { workspaceId: workspace.id, subject: user("member"), role: "member" })
        .wait({ tier: "edge" });
      ...
      expect(
        await manager.all(
          app.members.where({ id: managerMembership.id, subject: user("manager") }),
          {
            tier: "edge",
          },
        ),
      ...
      expect(
        await manager.all(app.workspaces.where({ id: workspace.id }), { tier: "edge" }),
      ).toEqual([workspace]);
  ...
  it(
    "rejects cross-workspace UUIDs for every recursive and block-dependent relation",
    async () => {
      const owner = testApp.as(session("owner"));
      const workspaceA = await owner
        .insert(app.workspaces, { name: "A", ownerSubject: user("owner") })
        .wait({ tier: "edge" });
      const workspaceB = await owner
        .insert(app.workspaces, { name: "B", ownerSubject: user("owner") })
        .wait({ tier: "edge" });
      for (const workspaceId of [workspaceA.id, workspaceB.id]) {
        await owner
          .insert(app.members, { workspaceId, subject: user("owner"), role: "owner" })
          .wait({ tier: "edge" });
      }
