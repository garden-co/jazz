...
describe("BandBinder workspace roles", () => {
  it(
    ...
    async () => {
      ...
      const workspace = await owner
        ...
        .wait({ tier: "edge" });
      await owner
        .insert(app.members, { workspaceId: workspace.id, author: author("owner"), role: "owner" })
        .wait({ tier: "edge" });
      ...
      expect(
        await manager.all(
          app.members.where({ id: managerMembership.id, author: author("manager") }),
          {
            tier: "edge",
          },
        ),
  ...
  it(
    ...
    async () => {
      const owner = testApp.as(session("owner"));
      const workspaceA = await owner.insert(app.workspaces, { name: "A" }).wait({ tier: "edge" });
      const workspaceB = await owner.insert(app.workspaces, { name: "B" }).wait({ tier: "edge" });
      for (const workspaceId of [workspaceA.id, workspaceB.id]) {
        await owner
          .insert(app.members, { workspaceId, author: author("owner"), role: "owner" })
          .wait({ tier: "edge" });
      }
