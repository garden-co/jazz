...
function BootstrapWorkspace({ subject }: { subject: string }) {
  const db = useDb();
  ...
  const bootstrap = async () => {
    ...
    try {
      const workspace = await db
        .insert(app.workspaces, { name: "World tour", ownerSubject: subject })
        .wait({ tier: "local" });
      await db
        .insert(app.members, { workspaceId: workspace.id, subject, role: "owner" })
        .wait({ tier: "local" });
      const page = await db
