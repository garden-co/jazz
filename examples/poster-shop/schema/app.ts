...
export interface CounterEventInit {
  actor_id: string;
  created_at: string;
}

export interface CounterEventWhereInput {
...
export class CounterEventQueryBuilder<
  I extends Record<string, never> = {},
> implements QueryBuilder<CounterEvent> {
  readonly _table = "counter_events";
  readonly _schema: WasmSchema = wasmSchema;
  declare readonly _rowType: CounterEvent;
  declare readonly _initType: CounterEventInit;
  ...
  where(conditions: CounterEventWhereInput): CounterEventQueryBuilder<I> {
  ...
  orderBy(
    column: keyof CounterEvent,
    direction: "asc" | "desc" = "asc",
  ): CounterEventQueryBuilder<I> {
    const clone = this._clone();
    clone._orderBys.push([column as string, direction]);
    return clone;
  }

  limit(n: number): CounterEventQueryBuilder<I> {
    const clone = this._clone();
    clone._limitVal = n;
    return clone;
  }

  offset(n: number): CounterEventQueryBuilder<I> {
    const clone = this._clone();
    clone._offsetVal = n;
    return clone;
  }
  ...
  private _clone<CloneI extends Record<string, never> = I>(): CounterEventQueryBuilder<CloneI> {
    const clone = new CounterEventQueryBuilder<CloneI>();
...
export interface GeneratedApp {
  counter_events: CounterEventQueryBuilder;
  wasmSchema: WasmSchema;
}

export const app: GeneratedApp = {
  counter_events: new CounterEventQueryBuilder(),
  wasmSchema,
};
