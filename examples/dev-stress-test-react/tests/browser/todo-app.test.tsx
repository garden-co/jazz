...
describe("React Todo App E2E", () => {
  ...
  // -------------------------------------------------------------------------
  // 9. Route-based navigation to generator page
  // -------------------------------------------------------------------------

  it("switches to the fixture generator page and back", async () => {
    const el = await mountApp({ driver: { type: "persistent", dbName: uniqueDbName("routes") } });
    const generatorButton = [...el.querySelectorAll("button")].find(
      (button) => button.textContent === "Generate Fixtures",
    );
    ...
    await waitFor(
      () => el.querySelector("h2")?.textContent === "Fixture Generator",
      1000,
      "Fixture generator should mount",
    );

    expect(el.querySelector("#todo-list")).toBeNull();
    expect(window.location.hash).toBe("#/generate");

    const backToTodosButton = [...el.querySelectorAll("button")].find(
      (button) => button.textContent === "Back to todos",
    );
    expect(backToTodosButton).toBeTruthy();
    await act(async () => {
      backToTodosButton!.click();
    });

    await waitFor(() => el.querySelector("#todo-list") !== null, 1000, "Todos page should remount");
