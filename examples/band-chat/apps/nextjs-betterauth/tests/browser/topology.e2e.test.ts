...
describe("BandChat cross-topology recovery", () => {
  it("converges concurrent messages, reactions, attachments, and an offline replay exactly once", async () => {
    ...
    const owner = await openClient(server, "owner", ownerToken);
    const peer = await openClient(server, "peer", peerToken);
    ...
    const room = (
      await owner.insert(app.rooms, { name: scenario.assertion.visibleText }).wait({ tier: "edge" })
    ).value;
    await owner
    ...
    const [ownerMessage] = await Promise.all([
    ...
    await Promise.all([
      owner
        .insert(app.reactions, {
          messageId: ownerMessage.value.id,
    ...
    expect(
      (await peer.all(app.reactions.where({ messageId: ownerMessage.value.id }), { tier: "edge" }))
