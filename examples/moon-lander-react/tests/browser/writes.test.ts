...
describe("reconcileDeposits", () => {
  ...
  it("inserts missing deposits to reach per-type target", async () => {
    ...
    const existing = [makeDeposit({ fuelType: "circle", occurredAt: 1000 })];
  ...
  it("does nothing when all per-type counts match targets", async () => {
    ...
    for (const ft of FUEL_TYPES) {
      for (let i = 0; i < DEPOSITS_PER_TYPE; i++) {
        existing.push(makeDeposit({ fuelType: ft, occurredAt: 1000 + i }));
      }
    }
  ...
  it("trims excess deposits by marking newest as collected", async () => {
    ...
    // 5 circle deposits with ascending occurredAt
    ...
    // Trimmed deposits should be the two newest (occurredAt 140, 130)
    const trimmedIds = updates.map((u) => u.id);
    expect(trimmedIds).toContain("circle-4"); // occurredAt 140
    expect(trimmedIds).toContain("circle-3"); // occurredAt 130
  ...
  it("handles mixed insert and trim across types", async () => {
    ...
    for (const ft of FUEL_TYPES) {
      if (ft === "circle") {
        // 5 circles — 2 excess
        for (let i = 0; i < 5; i++) {
          existing.push(makeDeposit({ fuelType: ft, id: `c-${i}`, occurredAt: 100 + i }));
        }
      } else if (ft === "triangle") {
        // 1 triangle — 2 short
        existing.push(makeDeposit({ fuelType: ft, occurredAt: 500 }));
      } else {
        // Exactly at target
        for (let i = 0; i < DEPOSITS_PER_TYPE; i++) {
          existing.push(makeDeposit({ fuelType: ft, occurredAt: 1000 + i }));
