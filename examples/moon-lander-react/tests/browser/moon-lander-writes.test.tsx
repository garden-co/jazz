...
import { playerStateChanged, reconcileDeposits, DEPOSITS_PER_TYPE } from "../../src/sync/writes.js";
...
function mockDb() {
  ...
  const updates: Array<{ table: unknown; id: string; data: Record<string, unknown>; tier: string }> = [];

  return {
    db: {
      insertPersisted: vi.fn(async (table: unknown, data: Record<string, unknown>, tier: string) => {
        const id = `new-${inserts.length}`;
        inserts.push({ table, data, tier });
        return id;
      }),
      updatePersisted: vi.fn(async (table: unknown, id: string, data: Record<string, unknown>, tier: string) => {
...
describe("reconcileDeposits", () => {
  ...
  it("inserts deposits with positionX within world bounds", async () => {
    ...
    await reconcileDeposits(db, [], FUEL_TYPES.map(() => DEPOSITS_PER_TYPE));
  ...
  it("does nothing when all targets are zero", async () => {
    ...
    await reconcileDeposits(db, [], FUEL_TYPES.map(() => 0));
