...
import { initPlayerProps, getOrCreatePlayerId } from "./game/player";
...
export const Game = memo(function Game({
  ...
  const playerProps = useRef(initPlayerProps(playerId)).current;
  ...
  return (
    ...
    >
      <canvas ref={canvasRef} data-testid="game-canvas" className={styles.canvas} />
      <Hud
        mode={engine.mode}
        fuel={engine.fuel}
        requiredFuelType={playerProps.requiredFuelType}
