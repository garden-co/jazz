 ...
 *  Each browser context (or Firefox container) gets its own player. */
...
/** Initialise player properties from a player ID.
 *  Name and colour are deterministic (identity). Fuel type is random each session. */
export function initPlayerProps(id: string): PlayerProps {
  const h = hashCode(id);
  return {
