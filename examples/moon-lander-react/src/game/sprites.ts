...
function lighten(hex: string, amount = 0.4): string {
  return adjustColour(hex, (ch) => Math.min(255, Math.round(ch + (255 - ch) * amount)));
}
...
// ---------------------------------------------------------------------------
// Animation clock — per-engine instance state
// ---------------------------------------------------------------------------
...
export function createSpriteAnimationState(): SpriteAnimationState {
  return { thrustPhase: 0, walkPhase: 0, walkTimer: 0 };
}

const WALK_FRAME_INTERVAL = 0.2; // seconds per walk frame

export function tickSpriteAnimation(anim: SpriteAnimationState, dt: number): void {
  anim.thrustPhase += dt * 12; // fast cycling for thrust flicker
  anim.walkTimer += dt;
  if (anim.walkTimer >= WALK_FRAME_INTERVAL) {
    anim.walkTimer -= WALK_FRAME_INTERVAL;
    anim.walkPhase = (anim.walkPhase + 1) % 2;
  }
}

export function getThrustFrame(anim: SpriteAnimationState): number {
  return Math.floor(anim.thrustPhase) % 3;
}

export function getWalkFrame(anim: SpriteAnimationState): number {
  return anim.walkPhase;
}
