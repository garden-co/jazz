import { COLOURS, curveOffset, GROUND_LEVEL, MOON_SURFACE_WIDTH } from "./constants";
...
for (let layer = 0; layer < 3; layer++) {
  for (let i = 0; i < STARS_PER_LAYER; i++) {
    ...
    const starColour = STAR_COLOURS[Math.floor(seededRand(seed + 0.9) * STAR_COLOURS.length)];
    stars.push({
...
// ---------------------------------------------------------------------------
// Unwrapped camera — per-engine instance state
// ---------------------------------------------------------------------------

export interface CameraTrackingState {
  prevRawCameraX: number; // NaN = uninitialised
  unwrappedCameraX: number;
}

export function createCameraTrackingState(): CameraTrackingState {
  return { prevRawCameraX: NaN, unwrappedCameraX: 0 };
}

function trackUnwrappedCamera(cam: CameraTrackingState, cameraX: number): void {
  if (Number.isNaN(cam.prevRawCameraX)) {
    cam.prevRawCameraX = cameraX;
    cam.unwrappedCameraX = cameraX;
    return;
  }
  let delta = cameraX - cam.prevRawCameraX;
  // Detect wrap: if delta jumps by more than half the world, correct it
  if (delta > W / 2) delta -= W;
  if (delta < -W / 2) delta += W;
  cam.unwrappedCameraX += delta;
  cam.prevRawCameraX = cameraX;
}

function drawMountainLayers(
  ctx: CanvasRenderingContext2D,
  cam: CameraTrackingState,
  w: number,
  screenGroundY: number,
) {
  for (const layer of CLIFF_LAYERS) {
    drawCliffLayer(ctx, cam.unwrappedCameraX, screenGroundY, w, layer);
  }
}
...
function drawSurfaceMarks(
  ...
  for (let i = 0; i < craterCount; i++) {
    ...
    const worldX = tile * CRATER_TILE + seededRand(tile * 89 + 59) * CRATER_TILE;
...
function drawGround(
  ...
  drawMountainLayers(ctx, cam, w, screenGroundY);
...
export function drawBackground(
  ...
  if (cam) {
    ...
    trackUnwrappedCamera(cam, cameraX);
    drawStarfield(ctx, cam.unwrappedCameraX, cameraY, w, h, now ?? 0);
    drawGround(ctx, cam, cameraX, cameraY, w, h);
  } else {
    // Fallback for callers without camera state (initial draw)
    drawStarfield(ctx, cameraX, cameraY, w, h, now ?? 0);
    drawGround(ctx, createCameraTrackingState(), cameraX, cameraY, w, h);
  }
}
