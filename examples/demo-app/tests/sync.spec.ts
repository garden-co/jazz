...
test.describe("Demo App Sync", () => {
  ...
  test("app loads with sync enabled and connects", async ({ page }) => {
    ...
    const syncConnected = consoleLogs.some((log) =>
      log.includes("Sync state: connected"),
    );
  ...
  test("two tabs can connect to sync server", async ({ browser }) => {
    ...
    try {
      ...
      const tab1Connected = logs1.some((log) =>
        log.includes("Sync state: connected"),
      );
      const tab2Connected = logs2.some((log) =>
        log.includes("Sync state: connected"),
      );
  ...
  test("issue edit in one tab syncs to another tab list", async ({
    browser,
  }) => {
    const context1 = await browser.newContext();
    const context2 = await browser.newContext();
    ...
    try {
      ...
      const originalTitle = await firstIssueRow
      ...
      const titleSpan = page2
        .locator("span.cursor-pointer.hover\\:text-primary")
        .first();
      ...
      const updatedTitleInTab1 = page1.locator(
        `[data-issue-id="${issueId}"] [data-testid="issue-title"]`,
      );
