...
function defaultConfig(secret: string): DbConfig {
...
function FileBrowser() {
  ...
  async function createFolder() {
    ...
    if (!name) return;
    const write = db.insert(app.folders, { name, owner_id: userId });
  ...
  async function upload(file: File) {
    ...
    setIsUploading(true);
    try {
      // File.stream() is consumed directly by Jazz; this app never buffers the
      // complete file in JavaScript before creating the Bytea value.
  ...
  return (
    <main>
      <header>
        <div>
          <p className="eyebrow">Large-value reference app</p>
          <h1>EpicDrop</h1>
          <p>
            Upload binary files without first loading the complete value into JavaScript memory.
          </p>
        </div>
...
export function App() {
  ...
  return (
    <JazzProvider config={defaultConfig(secret)} fallback={<p>Opening EpicDrop…</p>}>
      <FileBrowser />
    </JazzProvider>
  );
}
