...
export interface OwnedItemInit {
  title: string;
  ownerId: string;
}

export interface OwnedItemWhereInput {
...
export class OwnedItemQueryBuilder<
  I extends Record<string, never> = {},
> implements QueryBuilder<OwnedItem> {
  readonly _table = "owned_items";
  readonly _schema: WasmSchema = wasmSchema;
  declare readonly _rowType: OwnedItem;
  declare readonly _initType: OwnedItemInit;
  ...
  where(conditions: OwnedItemWhereInput): OwnedItemQueryBuilder<I> {
  ...
  orderBy(column: keyof OwnedItem, direction: "asc" | "desc" = "asc"): OwnedItemQueryBuilder<I> {
    const clone = this._clone();
    clone._orderBys.push([column as string, direction]);
    return clone;
  }

  limit(n: number): OwnedItemQueryBuilder<I> {
    const clone = this._clone();
    clone._limitVal = n;
    return clone;
  }

  offset(n: number): OwnedItemQueryBuilder<I> {
    const clone = this._clone();
    clone._offsetVal = n;
    return clone;
  }
  ...
  gather(options: {
    start: OwnedItemWhereInput;
    step: (ctx: { current: any }) => unknown;
    maxDepth?: number;
  }): OwnedItemQueryBuilder<I> {
  ...
  private _clone(): OwnedItemQueryBuilder<I> {
    const clone = new OwnedItemQueryBuilder<I>();
