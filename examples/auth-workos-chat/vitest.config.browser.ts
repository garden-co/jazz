...
export default defineConfig(async () => {
  // Generate a single ES256 keypair for the entire test run. The JWKS server
  // (started in tests/browser/global-setup.ts) hosts the public half so the
  // Jazz testing server can verify JWTs minted here against the private half.
  ...
  // template the README documents (https://workos.com .../JWT Template). Jazz's
  // permission DSL reads `session.claims.role` via the path "claims.role".
