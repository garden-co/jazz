...
import { getMessageAttachmentsQuery, getSenderProfileQuery } from "@/model/queries";
import type { Message, Profile } from "../../../schema.js";
...
export const ChatMessage = reatomComponent(
  ({ message, sender, isMe, onDelete }: ChatMessageProps) => {
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);

    const senderProfiles = getSenderProfileQuery(message.senderId)();
    const resolvedSender = (senderProfiles[0] as unknown as Profile) ?? sender;

    const attachments = getMessageAttachmentsQuery(message.id)();
    ...
    return (
      <>
        ...
        >
          <ChatMetadata date={new Date(message.createdAt)} senderName={resolvedSender?.name} />
