---
name: reatom-expert
description: Use when the task needs reactive state in this project — adding atoms, actions, derived/computed values, async data flow, form state, or wiring components to existing atoms via @reatom/core / @reatom/react. Invoke BEFORE writing any reatom code so the implementation matches the project's conventions and the up-to-date v1001 API.
---
...
If the project already uses `react-router` / Next.js / TanStack Start / Expo Router, **don't replace it**. Define `reatomRoute` clones with the _same URL pattern_ — the external router stays the navigator, the clone becomes a **reactive entry point** for state.
...
export const Counter = reatomComponent(
  () => <button onClick={wrap(() => count.set((n) => n + 1))}>{count()}</button>,
  "Counter",
);
```
...
Both components accept `abortOnUnmount` (default `false`). It controls only the **render phase** — atoms/actions/effects created (or triggered) _during render_:
...
additional features, recipes, adapters, and edge cases in the docs: https://sreatom.dev/reference/TOPIC_NAME.
...
- **fetch** wrapper for consistent context usageи
