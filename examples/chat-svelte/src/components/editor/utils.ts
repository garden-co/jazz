...
import { Bold as BoldIcon, Italic as ItalicIcon, Underline as UnderlineIcon } from 'lucide-svelte';
...
const MARK_TYPES = ['strong', 'em', 'underline'] as const;
...
function safeToggleMark(markType: any) {
...
export function createEditorKeymap(schema: Schema, onEnter: () => void) {
  return keymap({
    'Mod-b': safeToggleMark(schema.marks.strong),
    'Mod-i': safeToggleMark(schema.marks.em),
    'Mod-u': safeToggleMark(schema.marks.underline),
    'Mod-z': undo,
    'Mod-y': redo,
    'Shift-Mod-z': redo,
    Enter: () => {
    ...
    'Shift-Enter': chainCommands(exitCode, (state, dispatch) => {
      ...
      const tr =
        hasHardBreak && hardBreakType
          ? state.tr.replaceSelectionWith(hardBreakType.create())
