...
<script lang="ts">
  let { renderedOn }: { renderedOn: RenderedOn } = $props();
</script>

<span
  class={`mb-4 inline-block rounded-full border px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest ${styles[renderedOn]}`}
>
  rendered: {renderedOn}
</span>
