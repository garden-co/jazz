...
test.describe("SvelteKit CSR / SSR todos", () => {
  test("client and server columns round-trip todos through the shared Jazz backend", async ({
    page,
  }) => {
    // Flow:
    // browser client submit -> sync server -> reload -> server load render
    // server form action submit -> sync server -> live client subscription update
    const clientTitle = `alice books train tickets ${Date.now()}`;
    const serverTitle = `bob files travel reimbursement ${Date.now()}`;

    await page.goto("/");

    const clientPane = todoSection(page, "Client-side (Svelte)");
    const serverPane = todoSection(page, "Server-side (load)");

    await expect(page.getByRole("heading", { name: "jazz — sveltekit CSR / SSR" })).toBeVisible();
    ...
    await reloadUntilSectionContains(page, "Server-side (load)", clientTitle);

    const reloadedClientPane = todoSection(page, "Client-side (Svelte)");
    const reloadedServerPane = todoSection(page, "Server-side (load)");
