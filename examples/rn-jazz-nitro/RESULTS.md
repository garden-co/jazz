...
> That was expected for this spike and validated the flush contract.
...
- The storage engine opens, writes, reads, flushes, and closes successfully on iOS Simulator with no crashes or errors.
...
- The Nitro Rust bridge (via `aarch64-apple-ios-sim` static lib) adds no observable overhead relative to expected local storage performance.
