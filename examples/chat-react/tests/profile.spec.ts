  ...
  test("Should change and remove avatar", async ({ page }) => {
    ...
    await test.step("Upload Avatar", async () => {
      ...
      const profileAvatarImg = page.locator(
        '[role="dialog"] span.relative img.object-cover',
      );
...
test.describe("Account Recovery", () => {
  test("Should recover account using passphrase", async ({ page }) => {
    ...
    await test.step("Set unique name and copy passphrase", async () => {
      ...
      const passphraseTextarea = page.locator("textarea").filter({
        hasText: /.+/,
      });
    ...
    await test.step("Restore account via passphrase", async () => {
      ...
      const textarea = page.locator(
        'div[role="tabpanel"]:has-text("passphrase") textarea',
      );
      await expect(textarea).toBeVisible();
      await textarea.fill(passphrase);
      await page
        .getByRole("button", { name: /log in using passphrase/i })
        .click();
    });
  ...
  test("Should recover account using passkey", async ({
    page,
    browserName,
  }) => {
    test.skip(
      browserName !== "chromium",
      "Virtual authenticator is best supported in Chromium",
    );
    ...
    await test.step("Log Out", async () => {
      ...
      await page
        .locator("header button.flex.gap-2.items-center")
        .click({ force: true });
