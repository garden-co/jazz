...
**Attachments** store binary data in a `files` row, with the `attachments` row carrying the message linkage and metadata. They inherit their read policy from the parent message via `allowedTo.read("message")` — no separate asset server required.
