...
**Collaborative canvases** attach to a chat. Strokes are rows, synced in real time. Delete access is scoped directly to `{ $createdBy: session.author }`, so provenance is issuer-scoped; the canvas component has no explicit access checks.
