...
**Collaborative canvases** attach to a chat. Strokes are rows, synced in real time. Delete access uses the canonical logical author (`session.author`, via the creator helper) so provenance is issuer-scoped; the canvas component has no explicit access checks.
