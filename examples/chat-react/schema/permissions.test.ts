...
beforeEach(async () => {
  testApp = await createPolicyTestApp(SCHEMA_DIR);
});
...
describe("chat permissions", () => {
  it("allows pre-authorized private chat reads via join_code claim", async () => {
    const privateChat = testApp.seed((db) => {
      const privateChat = db.insert(app.chats, {
        name: "Private room",
        isPublic: false,
        createdBy: "alice",
        joinCode: "invite-123",
    ...
    const bobWithoutClaim = testApp.as({ user_id: "bob", claims: {} });
    const bobWithClaim = testApp.as({ user_id: "bob", claims: { join_code: "invite-123" } });
  ...
  it("allows message inserts only for chat members", async () => {
    const { aliceProfile, bobProfile, privateChat } = testApp.seed((db) => {
      const aliceProfile = db.insert(app.profiles, {
        userId: "alice",
        name: "Alice",
      });
      const bobProfile = db.insert(app.profiles, {
        userId: "bob",
        name: "Bob",
      });
      const privateChat = db.insert(app.chats, {
    ...
    const aliceDb = testApp.as({ user_id: "alice", claims: {} });
    const bobDb = testApp.as({ user_id: "bob", claims: {} });
  ...
  it("inherits reaction reads from the parent message/chat chain", async () => {
    const reaction = testApp.seed((db) => {
      ...
      const reaction = db.insert(app.reactions, {
    ...
    const bobDb = testApp.as({ user_id: "bob", claims: {} });
    const carolDb = testApp.as({ user_id: "carol", claims: {} });

    await expect(bobDb.all(app.reactions.where({ id: reaction.id }))).resolves.toEqual([
      expect.objectContaining({ id: reaction.id, emoji: "🔥" }),
    ]);
