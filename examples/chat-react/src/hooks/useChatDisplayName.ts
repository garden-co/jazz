...
export function useChatDisplayName(chatId: string, chatName?: string): string {
  ...
  const { data: messages = [] } = useAll(
    app.messages.where({ chatId }).orderBy("occurredAt", "asc").limit(1),
  );
  ...
  const dateSuffix = firstMessage ? ` · ${formatChatDate(firstMessage.occurredAt)}` : "";
  ...
  return firstMessage ? formatChatDate(firstMessage.occurredAt) : "Chat";
}
