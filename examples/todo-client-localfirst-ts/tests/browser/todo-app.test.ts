...
interface RealReloadState {
  phase: "reloaded";
  dbName: string;
}

const REAL_RELOAD_STATE_KEY = "__todo-real-reload-state";

function readRealReloadState(): RealReloadState | null {
  const raw = sessionStorage.getItem(REAL_RELOAD_STATE_KEY);
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as Partial<RealReloadState>;
    if (parsed.phase === "reloaded" && typeof parsed.dbName === "string") {
      return { phase: "reloaded", dbName: parsed.dbName };
    }
  } catch {
    // Ignore malformed state and treat as missing.
  }
  sessionStorage.removeItem(REAL_RELOAD_STATE_KEY);
  return null;
}

function writeRealReloadState(state: RealReloadState): void {
  sessionStorage.setItem(REAL_RELOAD_STATE_KEY, JSON.stringify(state));
}
...
describe("Vanilla TS Todo App E2E", () => {
  ...
  it("persists todos across real page reload (OPFS)", async () => {
    const existingState = readRealReloadState();
    if (!existingState) {
      const dbName = uniqueDbName("real-reload");
      const el = await mount({ dbName });

      addTodo(el, "Survive full reload");
      await waitFor(
        () => el.querySelectorAll("#todo-list li").length === 1,
        3000,
        "Todo should appear before triggering reload",
      );

      writeRealReloadState({ phase: "reloaded", dbName });
      window.location.reload();

      // The page will reload immediately; this promise should never settle.
      await new Promise<never>(() => {});
    }

    sessionStorage.removeItem(REAL_RELOAD_STATE_KEY);

    const el = await mount({ dbName: existingState.dbName });
    await waitFor(
      () => el.querySelectorAll("#todo-list li").length === 1,
      5000,
      "Todo should survive full page reload from OPFS",
    );
    expect(el.querySelector("#todo-list li span")!.textContent).toBe("Survive full reload");
  });
