...
use jazz::db::{Db, DbConfig, DbIdentity, PreparedQuery, ReadOpts, SubscriptionEvent, block_on};
...
fn insert(db: &BenchDb, table: &str, id: RowUuid, cells: BTreeMap<String, Value>) {
    let write = block_on(db.insert_with_id(table, id, cells)).expect("insert Wequencer row");
