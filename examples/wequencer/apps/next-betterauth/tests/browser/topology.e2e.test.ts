...
describe("Wequencer cross-topology recovery", () => {
  it(
    ...
    async () => {
          ...
          phases: [
              ...
              run: async () => {
                ...
                ctx.trackSubscription(
                  editor.subscribeAll(trackSteps(tracks[0].id), (delta) => {
                    subscribedTrackSteps = applySubscriptionDelta<Step>(
                      subscribedTrackSteps,
                      delta,
                    );
                  }),
                );
                ...
                const presence = await waitForQuery(
                  ...
                  (rows) =>
                    rows.some((row) => row.id === ownerPresence.id && row.cursor_step === 1),
            ...
            },
            {
              ...
              run: async () => {
                ...
                const refreshedPresence = await waitForQuery(
                  ...
                  sessionQueries(session.id).presence,
                  (rows) =>
                    rows.some((row) => row.id === ownerPresence.id && row.cursor_step === 7),
            ...
            },
            {
              ...
              run: async () => {
                ...
                expect(
                  restoredOfflineStep.find((step) => step.id === offlineStep.id),
                ).toMatchObject({
                  enabled: true,
                });
      ...
      expect(receipt.envelopes[0].activities.map((activity) => activity.action)).toContain(
        "retried",
      );
    },
    OUTER_TIMEOUT_MS,
