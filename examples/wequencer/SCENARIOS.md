...
between its UI, benchmarks, and topology E2E tests. The first native benchmark
implements the local `grid-read`, `subscribed-pad-edit`, and `editor-burst`
operations below. The browser receipt runs its covered profiles against the
current fingerprinted correctness artifacts; remaining rows are scenario
contracts until their own runner exists.
...
| `permission-roles`      | creator, editor, viewer               | one shared session               | editor changes pads; viewer attempts the same; creator changes membership after its initial `owner` row is deleted or downgraded | editor write settles; viewer write is rejected; immutable `$createdBy` retains creator administration; visibility follows membership after refresh/reconnect |
