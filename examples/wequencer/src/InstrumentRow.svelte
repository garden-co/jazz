<script lang="ts">
	...
	import { app, type Beat, type Instrument } from '../schema/app.js';
	...
	const beatIndices = $derived(Array.from({ length: audio.beatCount }, (_, i) => i));
...
<div class="instrument-row">
	...
	{#if mixerOpen}
		<div class="instrument-mixer">
			<label class="mixer-label">Vol</label>
			<input
				class="vol-slider"
				type="range"
				min="-60"
				...
				oninput={(e) =>
					audio.setInstrumentVolume(instrument.id, Number(e.currentTarget.value))}
			/>
			<label class="mixer-label">Pan</label>
			<input
				class="pan-slider"
				type="range"
				min="-100"
				...
				oninput={(e) =>
					audio.setInstrumentPan(instrument.id, Number(e.currentTarget.value) / 100)}
			/>
		</div>
	{/if}
</div>
