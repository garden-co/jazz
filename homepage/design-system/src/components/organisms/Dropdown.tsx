...
export function DropdownItem({
  ...
  let classes = clsx(
    className,
    // Base styles
    "group rounded-md flex gap-2 focus:outline-hidden px-2.5 py-1.5",
    // Text styles
    ...
    "data-[slot=icon]:*:text-stone-500 data-[slot=icon]:*:data-focus:text-white [&>[data-slot=icon]]: dark:data-[slot=icon]:*:data-focus:text-white",
    // Avatar
...
>) {
  return (
    <Headless.MenuHeading
      ...
      className={clsx(
        className,
        "col-span-full grid grid-cols-[1fr_auto] gap-x-12 px-3.5 pb-1 pt-2 text-sm/5 font-medium text-stone-500 sm:px-3 sm:text-xs/5",
      )}
    />
  );
}
