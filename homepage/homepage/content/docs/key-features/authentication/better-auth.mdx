...
import { Alert, CodeGroup, ContentByFramework, FileName, SvelteLogo, ReactLogo, TabbedCodeGroup, TabbedCodeGroupItem, VanillaLogo } from "@/components/forMdx";
...
<ContentByFramework frameworks={['react', 'svelte', 'react-native', 'react-native-expo']}>
Wrap your app with the `AuthProvider`, passing the `betterAuthClient` instance:
</ContentByFramework>

<TabbedCodeGroup default="react" savedPreferenceKey="framework" id="better-auth">
