    ...
    return (
        <div className="grid grid-cols-6 text-sm border-r border-b [&>*]:p-2 [&>*]:border-l [&>*]:border-t">
            ...
            <select
                value={issue.status}
                onChange={(event) => {
                    issue.$jazz.set("status", event.target.value as "backlog" | "in progress" | "done")
                }}
            >
...
<aside className="text-sm border border-muted rounded px-4 my-4 max-w-3xl [&_pre]:mx-0">
  ...
  <CodeGroup>
        ```ts
        const unsub = issue.$jazz.subscribe({ resolve: true }, (updatedIssue) => console.log(updatedIssue));
        ```
  </CodeGroup>
...
function App() {
    ...
    const createIssue = () => {
        ...
        setIssueID(newIssue.$jazz.id);
        window.history.pushState({}, "", `?issue=${newIssue.$jazz.id}`); // [!code ++]
    };
...
function App() {
    ...
    const createProject = () => { // [!code ++:14]
        ...
        setProjectID(newProject.$jazz.id);
        window.history.pushState({}, "", `?project=${newProject.$jazz.id}`);
    };
        ...
        <div>
            ...
            <div className="border-r border-b">
                {project.issues?.map((issue) => (
                    issue && <IssueComponent key={issue.$jazz.id} issue={issue} />
                ))}
