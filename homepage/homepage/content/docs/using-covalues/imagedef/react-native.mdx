...
You may choose to display a custom placeholder while your image is loading. You can do this by specifying a `customPlaceholder` string. This will be passed to the `source` property of the `RNImage` component, which makes it suitable for data URLs. If you use a custom placeholder, any existing placeholders for your image will be ignored.
