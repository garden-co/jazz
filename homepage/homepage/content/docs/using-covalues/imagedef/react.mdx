...
You may choose to display a custom placeholder while your image is loading. You can do this by specifying a `customPlaceholder` string. This will be passed directly to the `src` property of the img element, which makes it suitable for data URLs. If you use a custom placeholder, any existing placeholders for your image will be ignored.

<CodeGroup>
```tsx
<Image imageId="123" customPlaceholder={myCustomDataURL} />
```
</CodeGroup>
