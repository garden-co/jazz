...
export function getCurrentTasks(
  list: Loaded<typeof ListOfTasks>
): Loaded<typeof Task>[] {
  return Array.from(list.values()).filter(
    (task): task is Loaded<typeof Task> => !task?.deleted
  );
}

async function main() {
  const myTaskList = ListOfTasks.create([]);
  myTaskList.push(Task.create({ name: "Tomatoes", deleted: false }));
  myTaskList.push(Task.create({ name: "Cucumbers", deleted: true }));
  myTaskList.push(Task.create({ name: "Carrots", deleted: false }));

  const activeTasks = getCurrentTasks(myTaskList);
  console.log(activeTasks.map((task) => task.name)); // Output: ["Tomatoes", "Carrots"]
}
```
</CodeGroup>
