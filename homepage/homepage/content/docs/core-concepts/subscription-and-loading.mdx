...
However, the selector function itself still runs every time your CoValue updates, even if your `equalityFn` returns `true` and it doesn't trigger a component re-render. Because of this, you should **avoid doing expensive computation during a selector function**.

In case you need to run an expensive computation, use a lightweight selector which only tracks the minimum necessary to identify when the dependencies change. More expensive operations should run separately, wrapped in a `useMemo` hook. This way, React will only re-run the memoised expensive operation if the dependencies have changed.
