...
It is possible that in more complex cases, TypeScript cannot accurately infer types with circular references. For example, below, attempting to resolve `createdBy` on `Project` will in turn force `MyAppAccount` to resolve `projects`, which refers back to `Project`. This will result in an error.
