...
```ts twoslash
import { type Account, type Group, type ImageDefinition } from "jazz-tools";
...
<CodeGroup>
```ts twoslash
import type { ImageDefinition, Group, Account, Loaded } from "jazz-tools";
// ---cut---
...
Disabled by default. This option allows you to automatically generate a low resolution preview for use while the image is loading. Currently, only `"blur"` is supported.
