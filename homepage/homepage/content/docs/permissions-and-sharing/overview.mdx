...
You can define permissions at the schema level by using the `withPermissions` method on any CoValue schema. These permissions will be applied to all CoValues created with that schema.
...
This is useful if you want to modify inline CoValue creation to always reuse the container CoValue's owner.
