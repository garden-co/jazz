...
- Placeholder generation is now a configurable property, disabled by default. Currently, only `"blur-sm"` is supported, with more built-in options planned for future releases
