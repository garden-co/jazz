...
With native Rust crypto now running everywhere, Jazz crypto is faster and more reliable. 
This also helps us speed up the migration of Jazz Core to Rust which will improve the Jazz overall performance.
...
<CodeGroup>
```
Error: Failed to initialize WasmCrypto. WebAssembly is required for crypto operations on web platforms.
```
</CodeGroup>
...
If you previously had `CryptoProvider={...}` and/or `import { RNCrypto } ...`, you can remove both:
