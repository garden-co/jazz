...
- There is no more fallback to Javascript crypto. If the crypto fails to initialize, it will throw an error. See [Edge runtimes](#edge-runtimes).
...
- We dropped support for `PureJSCrypto`. Jazz crypto now runs natively in each environment. See [Expo](#expo) and [React Native](#react-native-non-expo).
- We dropped support for `RNQuickCrypto`. RNCrypto is now the default crypto provider on React Native. See [Expo](#expo) and [React Native](#react-native-non-expo).
...
- We have introduced a way to permanently [delete CoValues](/docs/core-concepts/covalues/deleting). This brings a new `deleted` loading state. See [New `deleted` loading state](#new-deleted-loading-state).
...
<CodeGroup>
```ts
// Before (integers cause TypeScript errors but still work at runtime)
Task.create({ text: "X" }, { unique: 123 }); // TYPE ERROR (deprecated)

// After - Recommended: convert to string or object with string values
Task.create({ text: "X" }, { unique: "123" }); // SUPPORTED (RECOMMENDED)

// Alternative: bypass type check (integers still work at runtime, but deprecated)
// @ts-expect-error - We are bypassing the type check
Task.create({ text: "X" }, { unique: 123 }); // SUPPORTED (but use string instead)
```
</CodeGroup>

### New `deleted` loading state

With the introduction of [CoValue deletion](/docs/core-concepts/covalues/deleting), there's a new `deleted` loading state. If you're handling loading states explicitly, you should add a case for `deleted`:
