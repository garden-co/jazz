...
only worked when WASM was explicitly initialized (see [WASM on Edge runtimes](/docs/server-side/deployment#wasm-on-edge-runtimes)).
...
See more details at [WASM on Edge runtimes](/docs/server-side/deployment#wasm-on-edge-runtimes).
