...
- We have restricted the type of [unique](/docs/react/core-concepts/covalues/comaps#uniqueness) to make the cross-language serialization more stable. You can now pass only strings or records of strings as unique params. 

## Breaking changes

### A more strict `unique` param

The [unique](/docs/react/core-concepts/covalues/comaps#uniqueness) param is a attribute that is used to identify a CoValue uniquely with a human readable identifier. 
It is used to create a deterministic ID for the CoValue.
...
- Floating-point numbers (e.g., `1.5`) are strictly forbidden and will throw an error.
...
We expect that this change **does not affect any real usages of Jazz** based on our data from SaaS-hosted projects.
However, since we do **not have visibility into self-hosted sync server deployments**, there is a very small possibility that your code may rely on the previously allowed unique value types.
