...
In a traditional database, you might model different data types as 'tables' or 'collections'. With Jazz, you model data types as schemas, and you model your data as an explicitly linked graph.
...
<Alert variant="warning" title="Use migrations carefully">
  Migrations run *every* time a CoValue is loaded. A poorly-written migration could cause a lot of unnecessary work being done, and potentially slow your app down. Exit as early as possible from the migration, and check before updating the update is necessary.
</Alert>
