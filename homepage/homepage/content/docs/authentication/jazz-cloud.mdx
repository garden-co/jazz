...
const authUrl: string = "https://auth.jazz.tools/exampleApiKey/api/auth/";
// ---cut---
// @annotate: src/contexts/Auth.tsx
...
Within any component where your context is imported, you may then use `const { auth, user, account } = useAuth();`, where `auth` is the authentication client, `user` is the decrypted Jazz account credentials, and `account` is the authenticated account state.
