...
import { useAll, useMutate, useOne } from "@jazz/react";
...
export function IssueDetail({ issueId, open, onOpenChange }: IssueDetailProps) {
  ...
  const createdDate = issue
    ? new Date(Number(issue.createdAt)).toLocaleDateString()
    : "";
  const updatedDate = issue
    ? new Date(Number(issue.updatedAt)).toLocaleDateString()
    : "";
