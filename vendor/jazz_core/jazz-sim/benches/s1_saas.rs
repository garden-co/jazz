...
use jazz::abi::{
    ABI_FEATURE_NONE, ABI_VERSION, AbiEncodedCellPatch, AbiReadOpts, AbiRowBatch,
    AbiSubscriptionDelta, AbiSubscriptionStreamChunk, AbiTxKind, CloseReason, DbIdentityPayload,
    Event, EventBudget, EventKind, Handle, OpenDbConfig, WriteStateTarget,
};
...
use jazz::groove::records::{RecordDescriptor, Value};
...
fn main() {
    ...
    if phase_selection.should_run("abi_direct_surface") {
        emit_abi_direct_surface_summary(&config, &abi_direct_surface_summary(&config));
    }
}
    ...
    fn assert_supported(&self) {
        ...
        for phase in selected {
            assert!(
                matches!(
                    phase.as_str(),
                    "default" | "high_fan_out_hydration" | "abi_direct_surface"
                ),
                "unsupported JAZZ_BENCH_PHASES value {phase:?}; supported values: default, high_fan_out_hydration, abi_direct_surface"
            );
        }
    }
}
...
pub fn db_surface_smoke() {
    ...
    let mut subscription1 =
        block_on(db.subscribe(&query1, ReadOpts::default())).expect("db subscribe q1");
    let mut subscription2 =
        block_on(db.subscribe(&query2, ReadOpts::default())).expect("db subscribe q2");
    ...
    let _ = db.one(&query1).expect("db one q1");
}

pub fn abi_direct_surface_smoke() {
    let config = Config {
        seed: 11,
        profile: "s1-abi-smoke".to_owned(),
        orgs: 2,
        teams_per_org: 2,
        users_per_org: 3,
        issues_per_org: 6,
    ...
    let summary = abi_direct_surface_summary(&config);
    assert_eq!(summary.prepared_queries, 2);
    assert!(summary.read_rows > 0);
    assert!(summary.write_verbs > 0);
    assert!(summary.encoded_write_bytes > 0);
    assert!(summary.encoded_write_verbs > 0);
    assert!(summary.encoded_probe_verbs > 0);
    assert!(summary.encoded_tx_stage_verbs > 0);
    assert!(
        summary.subscription_delta_added
            + summary.subscription_delta_updated
            + summary.subscription_delta_removed
            > 0
    );
}

fn abi_direct_surface_summary(config: &Config) -> AbiDirectSurfaceSummary {
    let start = Instant::now();
    let schema = schema();
    let fixture = build_fixture(&config);
    let plan = representative_plan(&fixture);
    let mut oracle = DbS1Oracle::default();
    let mut abi = S1AbiHarness::open(schema.clone(), AuthorId(plan.user.0));
    let mut summary = AbiDirectSurfaceSummary::default();

    let phase_start = Instant::now();
    let query1 = abi.prepare_query(db_query1(&plan));
    let query2 = abi.prepare_query(db_query2(&plan));
    summary.prepare_us += phase_start.elapsed().as_micros();
    summary.prepared_queries = 2;

    for commit in &fixture.commits {
        let phase_start = Instant::now();
        abi.insert_with_id(&commit.table, commit.row_uuid, commit.cells.clone());
        summary.write_us += phase_start.elapsed().as_micros();
        oracle.apply_insert(commit);
        summary.write_verbs += 1;
    }

    let phase_start = Instant::now();
    let q1_batches = abi.read_batches(query1);
    summary.read_us += phase_start.elapsed().as_micros();
    summary.record_read_batches(&q1_batches);
    assert_eq!(
        abi_row_set(&q1_batches),
        oracle.query1(&plan),
        "ABI q1 after cold load mismatch"
    );
    let phase_start = Instant::now();
    let q2_batches = abi.read_batches(query2);
    summary.read_us += phase_start.elapsed().as_micros();
    summary.record_read_batches(&q2_batches);
    assert_eq!(
        abi_row_set(&q2_batches),
        oracle.query2(&plan),
        "ABI q2 after cold load mismatch"
    );

    let expected_q1 = oracle.query1(&plan);
    let phase_start = Instant::now();
    let mut subscription = abi.subscribe(query1, expected_q1.clone());
    summary.watch_us += phase_start.elapsed().as_micros();
    let phase_start = Instant::now();
    let subscription_snapshot = abi.subscription_snapshot_row_set(&mut subscription);
    summary.watch_us += phase_start.elapsed().as_micros();
    assert_eq!(
        subscription_snapshot, expected_q1,
        "ABI q1 subscription snapshot mismatch"
    );
    ...
    let issues = schema
        ...
        .expect("issues table");
    let phase_start = Instant::now();
    let table_check_batches = abi.read_batches(query1);
    summary.read_us += phase_start.elapsed().as_micros();
    summary.record_read_batches(&table_check_batches);
    assert!(
        table_check_batches
            .into_iter()
            .all(|batch| batch.table == issues.name)
    );
    ...
    let edited_issue = oracle
        ...
        .map(|(_, row)| row)
        .expect("representative S1 plan should match at least one issue");
    let restore_cells = oracle
        .row_cells(ISSUES, edited_issue)
        .expect("edited issue exists in oracle")
        .clone();
    let patch = BTreeMap::from([
        ("state".to_owned(), Value::U8(STATE_DONE)),
        (
            "title".to_owned(),
            Value::String("abi-direct-state-transition".to_owned()),
        ),
    ]);
    abi.assert_can_read(query1, "q1 can_read");
    let encoded_probe_patch = encode_abi_cell_patch(&schema, ISSUES, &patch);
    summary.record_encoded_write_payload(&encoded_probe_patch);
    abi.assert_can_update_encoded(
        ISSUES,
        edited_issue,
        encoded_probe_patch,
        "issue encoded can_update",
    );
    summary.encoded_probe_verbs += 1;
    abi.assert_can_delete(ISSUES, edited_issue, "issue can_delete");

    let upserted_issue = row(8_800_001);
    let mut upsert_cells = restore_cells.clone();
    upsert_cells.extend(BTreeMap::from([
        (
            "title".to_owned(),
            Value::String("abi-direct-upsert".to_owned()),
        ),
        (
            "body".to_owned(),
            Value::String("abi direct body".to_owned()),
        ),
        ("state".to_owned(), Value::U8(STATE_IN_PROGRESS)),
        ("priority".to_owned(), Value::U64(99)),
        ("assignee".to_owned(), Value::Uuid(plan.user.0)),
        ("project".to_owned(), Value::Uuid(plan.project.0)),
        ("cycle".to_owned(), Value::Uuid(plan.active_cycle.0)),
    ]));
    let encoded_upsert_cells = encode_abi_cell_patch(&schema, ISSUES, &upsert_cells);
    summary.record_encoded_write_payload(&encoded_upsert_cells);
    abi.assert_can_insert_encoded(
        ISSUES,
        encoded_upsert_cells.clone(),
        "issue encoded can_insert",
    );
    summary.encoded_probe_verbs += 1;

    let tx_inserted_issue = row(8_800_002);
    let mut tx_insert_cells = restore_cells.clone();
    tx_insert_cells.extend(BTreeMap::from([
        (
            "title".to_owned(),
            Value::String("abi-direct-tx-insert".to_owned()),
        ),
        (
            "body".to_owned(),
            Value::String("abi direct tx body".to_owned()),
        ),
        ("state".to_owned(), Value::U8(STATE_IN_PROGRESS)),
        ("priority".to_owned(), Value::U64(100)),
        ("assignee".to_owned(), Value::Uuid(plan.user.0)),
        ("project".to_owned(), Value::Uuid(plan.project.0)),
        ("cycle".to_owned(), Value::Uuid(plan.active_cycle.0)),
    ]));
    let phase_start = Instant::now();
    let encoded_patch = encode_abi_cell_patch(&schema, ISSUES, &patch);
    let encoded_tx_insert_cells = encode_abi_cell_patch(&schema, ISSUES, &tx_insert_cells);
    summary.record_encoded_write_payload(&encoded_patch);
    summary.record_encoded_write_payload(&encoded_tx_insert_cells);
    abi.mergeable_tx_update_and_insert_encoded(
        ISSUES,
        edited_issue,
        encoded_patch.clone(),
        tx_inserted_issue,
        encoded_tx_insert_cells,
    );
    summary.write_us += phase_start.elapsed().as_micros();
    oracle.apply_patch(ISSUES, edited_issue, patch.clone());
    oracle.apply_upsert(ISSUES, tx_inserted_issue, tx_insert_cells);
    summary.write_verbs += 2;
    summary.encoded_tx_stage_verbs += 2;
    let phase_start = Instant::now();
    let q1_batches = abi.read_batches(query1);
    summary.read_us += phase_start.elapsed().as_micros();
    summary.record_read_batches(&q1_batches);
    assert_eq!(
        abi_row_set(&q1_batches),
        oracle.query1(&plan),
        "ABI q1 after direct tx mergeable commit mismatch"
    );
    let phase_start = Instant::now();
    let q2_batches = abi.read_batches(query2);
    summary.read_us += phase_start.elapsed().as_micros();
    ...
    assert_eq!(
        abi_row_set(&q2_batches),
        oracle.query2(&plan),
        "ABI q2 after direct tx mergeable commit mismatch"
    );

    let phase_start = Instant::now();
    abi.upsert_encoded(ISSUES, upserted_issue, encoded_upsert_cells);
    summary.write_us += phase_start.elapsed().as_micros();
    oracle.apply_upsert(ISSUES, upserted_issue, upsert_cells);
    summary.write_verbs += 1;
    summary.encoded_write_verbs += 1;
    let phase_start = Instant::now();
    let q1_batches = abi.read_batches(query1);
    summary.read_us += phase_start.elapsed().as_micros();
    summary.record_read_batches(&q1_batches);
    assert_eq!(
        abi_row_set(&q1_batches),
        oracle.query1(&plan),
        "ABI q1 after upsert mismatch"
    );

    let phase_start = Instant::now();
    abi.delete(ISSUES, edited_issue);
    summary.write_us += phase_start.elapsed().as_micros();
    oracle.apply_delete(ISSUES, edited_issue);
    summary.write_verbs += 1;
    let phase_start = Instant::now();
    let q1_batches = abi.read_batches(query1);
    summary.read_us += phase_start.elapsed().as_micros();
    ...
    assert_eq!(
        abi_row_set(&q1_batches),
        oracle.query1(&plan),
        "ABI q1 after delete mismatch"
    );

    let phase_start = Instant::now();
    abi.restore(ISSUES, edited_issue, restore_cells.clone());
    summary.write_us += phase_start.elapsed().as_micros();
    oracle.apply_restore(ISSUES, edited_issue, restore_cells);
    summary.write_verbs += 1;
    let phase_start = Instant::now();
    let q1_batches = abi.read_batches(query1);
    summary.read_us += phase_start.elapsed().as_micros();
    ...
    assert_eq!(
        abi_row_set(&q1_batches),
        oracle.query1(&plan),
        "ABI q1 after restore mismatch"
    );

    let phase_start = Instant::now();
    abi.update_encoded(ISSUES, edited_issue, encoded_patch);
    summary.write_us += phase_start.elapsed().as_micros();
    oracle.apply_patch(ISSUES, edited_issue, patch);
    summary.write_verbs += 1;
    summary.encoded_write_verbs += 1;
    ...
    assert_eq!(
        ...
        oracle.query1(&plan),
        "ABI q1 subscription snapshot after writes mismatch"
    );
    let phase_start = Instant::now();
    let incremental = abi.subscription_incremental_delta(&mut subscription);
    summary.watch_us += phase_start.elapsed().as_micros();
    assert!(
        !incremental.added.is_empty()
            || !incremental.updated.is_empty()
            || !incremental.removed.is_empty(),
        "ABI q1 incremental subscription_delta should include at least one row change"
    );
    summary.subscription_delta_added = incremental.added.len();
    summary.subscription_delta_updated = incremental.updated.len();
    summary.subscription_delta_removed = incremental.removed.len();
    summary.elapsed_us = start.elapsed().as_micros();
    summary
}

#[derive(Default)]
struct AbiDirectSurfaceSummary {
    elapsed_us: u128,
    prepare_us: u128,
    write_us: u128,
    read_us: u128,
    watch_us: u128,
    row_encode_us: u128,
    prepared_queries: usize,
    read_rows: usize,
    write_verbs: usize,
    subscription_delta_added: usize,
    subscription_delta_updated: usize,
    subscription_delta_removed: usize,
    encoded_row_bytes: usize,
    encoded_write_bytes: usize,
    encoded_write_verbs: usize,
    encoded_probe_verbs: usize,
    encoded_tx_stage_verbs: usize,
}

impl AbiDirectSurfaceSummary {
    fn record_read_batches(&mut self, batches: &[AbiRowBatch]) {
        self.read_rows += batches.iter().map(|batch| batch.rows.len()).sum::<usize>();
        let phase_start = Instant::now();
        self.encoded_row_bytes += postcard::to_allocvec(batches)
            .expect("S1 ABI row batches encode")
            .len();
        self.row_encode_us += phase_start.elapsed().as_micros();
    }

    fn record_encoded_write_payload(&mut self, payload: &[u8]) {
        self.encoded_write_bytes += payload.len();
    }
}

fn encode_abi_cell_patch(schema: &JazzSchema, table: &str, cells: &RowCells) -> Vec<u8> {
    let table_schema = schema
        .tables
        .iter()
        .find(|candidate| candidate.name == table)
        .unwrap_or_else(|| panic!("S1 table {table} exists"));
    let descriptor = RecordDescriptor::new(cells.keys().map(|name| {
        let column = table_schema
            .columns
            .iter()
            .find(|column| column.name == *name)
            .unwrap_or_else(|| panic!("S1 column {table}.{name} exists"));
        (name.clone(), column.column_type.clone().value_type())
    }));
    let raw = descriptor
        .create(cells.values().cloned().collect::<Vec<_>>().as_slice())
        .expect("S1 ABI cell patch raw record encodes");
    postcard::to_allocvec(&AbiEncodedCellPatch { descriptor, raw })
        .expect("S1 ABI encoded cell patch encodes")
}
...
impl S1AbiHarness {
    fn open(schema: JazzSchema, author: AuthorId) -> Self {
        let mut runtime = AbiRuntime::new();
        let AbiCallResult::RuntimeInitialized {
            runtime: runtime_handle,
        } = runtime.runtime_init(Some(1), ABI_VERSION, ABI_FEATURE_NONE)
        else {
            panic!("ABI runtime init should allocate handle");
        };
        drain_abi_events(&mut runtime, runtime_handle, 2);

        assert_eq!(
            runtime.storage_open_memory(Some(3), Vec::new()),
            AbiCallResult::Ack
        );
        let storage = drain_abi_events(&mut runtime, runtime_handle, 4)
            .into_iter()
            .find_map(|event| {
                if let EventKind::StorageOpened { storage, .. } = event.kind {
                    Some(storage)
                } else {
                    None
                }
            })
            .expect("ABI memory storage opens");

        let open_config = OpenDbConfig {
            identity: DbIdentityPayload {
                node: node(71),
                author,
            },
            row_id_seed: None,
        ...
        assert_eq!(
            runtime.db_open_memory(
                Some(5),
                storage,
                postcard::to_allocvec(&schema).expect("S1 schema encodes"),
                postcard::to_allocvec(&open_config).expect("S1 ABI open config encodes"),
            ),
            AbiCallResult::Ack
        );
        let db = drain_abi_events(&mut runtime, runtime_handle, 6)
            .into_iter()
            .find_map(|event| {
                if let EventKind::DbOpened { db, .. } = event.kind {
                    Some(db)
                } else {
                    None
                }
    ...
    fn prepare_query(&mut self, query: Query) -> Handle {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime.db_prepare_query(
                Some(request_id),
                self.db,
                postcard::to_allocvec(&query).expect("S1 query encodes"),
            ),
            AbiCallResult::Ack
        );
        self.drain(request_id + 1)
            .into_iter()
            .find_map(|event| {
                if let EventKind::QueryPrepared { query, .. } = event.kind {
                    Some(query)
                } else {
                    None
                }
            })
            .expect("ABI query prepares")
    }

    fn insert_with_id(&mut self, table: &str, row_id: RowUuid, cells: RowCells) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime.db_insert_with_id(
                Some(request_id),
                self.db,
                table.to_owned(),
                row_id,
                postcard::to_allocvec(&cells).expect("S1 row cells encode"),
            ),
            AbiCallResult::Ack
        );
        let write = self.expect_started_write(request_id, row_id, "insert");
        self.expect_write_state(write, "insert");
        self.expect_write_settled(write, "insert");
    }

    fn update_encoded(&mut self, table: &str, row_id: RowUuid, patch: Vec<u8>) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime.db_update_encoded(
                Some(request_id),
                self.db,
                table.to_owned(),
                row_id,
        ...
        let write = self.expect_started_write(request_id, row_id, "encoded update");
        self.expect_write_state(write, "encoded update");
        self.expect_write_settled(write, "encoded update");
    }

    fn upsert_encoded(&mut self, table: &str, row_id: RowUuid, cells: Vec<u8>) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime.db_upsert_encoded(
                Some(request_id),
                self.db,
                table.to_owned(),
                row_id,
        ...
        let write = self.expect_started_write(request_id, row_id, "encoded upsert");
        self.expect_write_state(write, "encoded upsert");
        self.expect_write_settled(write, "encoded upsert");
    }

    fn delete(&mut self, table: &str, row_id: RowUuid) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime
                .db_delete(Some(request_id), self.db, table.to_owned(), row_id),
            AbiCallResult::Ack
        );
        let write = self.expect_started_write(request_id, row_id, "delete");
        self.expect_write_state(write, "delete");
        self.expect_write_settled(write, "delete");
    }

    fn restore(&mut self, table: &str, row_id: RowUuid, cells: RowCells) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime.db_restore(
                Some(request_id),
                self.db,
                table.to_owned(),
                row_id,
                postcard::to_allocvec(&cells).expect("S1 restore cells encode"),
            ),
            AbiCallResult::Ack
        );
        let write = self.expect_started_write(request_id, row_id, "restore");
        self.expect_write_state(write, "restore");
        self.expect_write_settled(write, "restore");
    }

    fn mergeable_tx_update_and_insert_encoded(
        &mut self,
        table: &str,
        updated_row: RowUuid,
        patch: Vec<u8>,
        inserted_row: RowUuid,
        inserted_cells: Vec<u8>,
    ) {
        let begin_request = self.next_request_id();
        assert_eq!(
            self.runtime
                .tx_begin(Some(begin_request), self.db, AbiTxKind::Mergeable),
            AbiCallResult::Ack
        );
        let tx = self
            .drain(begin_request + 1)
            .into_iter()
            .find_map(|event| {
                if let EventKind::TxOpened { tx, db, kind } = event.kind {
                    assert_eq!(db, self.db);
                    assert_eq!(kind, AbiTxKind::Mergeable);
                    Some(tx)
                } else {
                    None
                }
            })
            .expect("ABI encoded mergeable tx opens");

        let update_request = self.next_request_id();
        assert_eq!(
            self.runtime.tx_update_encoded(
                Some(update_request),
                tx,
                table.to_owned(),
                updated_row,
        ...
        let insert_request = self.next_request_id();
        assert_eq!(
            self.runtime.tx_insert_with_id_encoded(
                Some(insert_request),
                tx,
                table.to_owned(),
                inserted_row,
        ...
        let commit_request = self.next_request_id();
        assert_eq!(
            self.runtime.tx_commit(Some(commit_request), tx),
            AbiCallResult::Ack
        );
        let (write, started_tx_id, committed_tx_id) = self
            .drain(commit_request + 1)
            .into_iter()
            .fold((None, None, None), |mut acc, event| {
                match event.kind {
                    EventKind::WriteStarted {
                        write,
                        tx_id,
                        row_id: _,
                        durability,
                    } => {
                        assert_eq!(durability, DurabilityTier::Local);
                        acc.0 = Some(write);
                        acc.1 = Some(tx_id);
                    }
                    EventKind::TxCommitted {
                        ...
                        assert_eq!(event_tx, tx);
                        if let Some(started_write) = acc.0 {
                            assert_eq!(write, started_write);
                        } else {
                            acc.0 = Some(write);
                        }
                        acc.2 = Some(tx_id);
        ...
        let write = write.expect("ABI encoded mergeable tx commit starts write");
        let started_tx_id = started_tx_id.expect("ABI encoded mergeable tx commit write has tx_id");
        let committed_tx_id = committed_tx_id.expect("ABI encoded mergeable tx commits");
        assert_eq!(started_tx_id, committed_tx_id);
        self.expect_write_state(write, "encoded tx commit");
        self.expect_write_settled(write, "encoded tx commit");
    }

    fn subscribe(
        &mut self,
        query: Handle,
        expected: BTreeSet<(String, RowUuid)>,
    ) -> AbiSubscriptionStream {
        let request_id = self.next_request_id();
        let mut subscription = self
            .runtime
            .db_subscribe_stream(Some(request_id), self.db, query, AbiReadOpts::default())
            .expect("ABI subscribe opens stream");
        let chunk = self.next_subscription_chunk(&mut subscription);
        let AbiSubscriptionStreamChunk::Snapshot(snapshot) = chunk else {
            panic!("expected ABI subscription snapshot chunk");
        };
        assert!(snapshot.settled);
        assert_eq!(snapshot.tier, DurabilityTier::Local);
        assert_eq!(
            abi_row_set(&snapshot.rows),
            expected,
            "ABI subscribe snapshot row set mismatch"
        );
        subscription
    }
    ...
    fn subscription_snapshot_row_set(
        &mut self,
        subscription: &mut AbiSubscriptionStream,
    ) -> BTreeSet<(String, RowUuid)> {
        let chunk = self.next_subscription_chunk(subscription);
        let AbiSubscriptionStreamChunk::Snapshot(snapshot) = chunk else {
            panic!("expected ABI subscription snapshot chunk");
        };
        assert_eq!(snapshot.tier, DurabilityTier::Local);
    ...
    fn subscription_incremental_delta(
        &mut self,
        subscription: &mut AbiSubscriptionStream,
    ) -> AbiSubscriptionDelta {
        let chunk = self.next_subscription_chunk(subscription);
        let AbiSubscriptionStreamChunk::Delta(delta) = chunk else {
            panic!("expected ABI subscription delta chunk");
        };
        assert_eq!(delta.tier, DurabilityTier::Local);
        delta.delta
    }

    fn next_subscription_chunk(
        &mut self,
        subscription: &mut AbiSubscriptionStream,
    ) -> AbiSubscriptionStreamChunk {
        block_on(std::future::poll_fn(|cx| {
            Pin::new(&mut *subscription).poll_next(cx)
        }))
        .expect("ABI subscription stream emits chunk")
        .expect("ABI subscription stream chunk succeeds")
    }

    fn assert_can_insert_encoded(&mut self, table: &str, cells: Vec<u8>, label: &str) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime
                .db_can_insert_encoded(Some(request_id), self.db, table.to_owned(), cells),
            AbiCallResult::Ack
        );
        self.expect_permission_allowed(request_id, label);
    }

    fn assert_can_read(&mut self, query: Handle, label: &str) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime.db_can_read(Some(request_id), self.db, query),
            AbiCallResult::Ack
        );
        self.expect_permission_allowed(request_id, label);
    }

    fn assert_can_update_encoded(
        &mut self,
        table: &str,
        row_id: RowUuid,
        patch: Vec<u8>,
        ...
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime.db_can_update_encoded(
                Some(request_id),
                self.db,
                table.to_owned(),
                row_id,
    ...
    fn assert_can_delete(&mut self, table: &str, row_id: RowUuid, label: &str) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime
                .db_can_delete(Some(request_id), self.db, table.to_owned(), row_id),
            AbiCallResult::Ack
        );
        self.expect_permission_allowed(request_id, label);
    }

    fn expect_permission_allowed(&mut self, request_id: u64, label: &str) {
        let (allowed, denial) = self
            .drain(request_id + 1)
            .into_iter()
            .find_map(|event| {
                if let EventKind::PermissionResult { allowed, denial } = event.kind {
                    Some((allowed, denial))
                } else {
                    None
                }
            })
            .unwrap_or_else(|| panic!("ABI {label} emits permission result"));
        assert!(allowed, "ABI {label} should be allowed");
        assert!(denial.is_none(), "ABI {label} should not include denial");
    }

    fn expect_tx_write_staged(
        &mut self,
        request_id: u64,
        expected_tx: Handle,
        expected_row: Option<RowUuid>,
        expected_staged_writes: u32,
        label: &str,
    ) {
        let (row_id, staged_writes) = self
            .drain(request_id + 1)
            .into_iter()
            .find_map(|event| {
                if let EventKind::TxWriteStaged {
                    tx,
                    row_id,
                    staged_writes,
                } = event.kind
                {
                    ...
                    Some((row_id, staged_writes))
                } else {
                    None
                }
            })
            .unwrap_or_else(|| panic!("ABI {label} stages tx write"));
        assert_eq!(row_id, expected_row);
        assert_eq!(staged_writes, expected_staged_writes);
    }

    fn expect_started_write(
        &mut self,
        request_id: u64,
        ...
        let started = self
            ...
            .find_map(|event| {
                ...
                {
                    Some((write, row_id, durability))
                } else {
                    None
                }
            })
            .unwrap_or_else(|| panic!("ABI {verb} starts write"));
        assert_eq!(
            (started.1, started.2),
            (expected_row, DurabilityTier::Local)
        );
        started.0
    }

    fn expect_write_state(&mut self, write: Handle, verb: &str) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime
                .db_get_write_state(Some(request_id), WriteStateTarget::Handle(write)),
            AbiCallResult::Ack
        );
        let state = self
            .drain(request_id + 1)
            .into_iter()
            .find_map(|event| {
                if let EventKind::WriteState { state, .. } = event.kind {
                    Some(state)
                } else {
                    None
                }
            })
            .unwrap_or_else(|| panic!("ABI {verb} reports write state"));
        assert!(matches!(state.fate, Fate::Pending | Fate::Accepted));
        assert_eq!(state.durability, DurabilityTier::Local);
        assert!(state.rejection.is_none());
    }

    fn expect_write_settled(&mut self, write: Handle, verb: &str) {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime
                .db_wait_write(Some(request_id), write, DurabilityTier::Local),
            AbiCallResult::Ack
        );
        let state = self
            .drain(request_id + 1)
            .into_iter()
            .find_map(|event| {
                if let EventKind::WriteSettled { state, .. } = event.kind {
                    Some(state)
                } else {
                    None
                }
            })
            .unwrap_or_else(|| panic!("ABI {verb} wait settles write"));
        assert!(matches!(state.fate, Fate::Pending | Fate::Accepted));
        assert_eq!(state.durability, DurabilityTier::Local);
        assert!(state.rejection.is_none());
    }

    fn read_batches(&mut self, query: Handle) -> Vec<AbiRowBatch> {
        let request_id = self.next_request_id();
        assert_eq!(
            self.runtime
                .db_read(Some(request_id), self.db, query, DurabilityTier::Local),
            AbiCallResult::Ack
        );
        self.drain(request_id + 1)
            .into_iter()
            .find_map(|event| {
                if let EventKind::Rows { rows, tier, .. } = event.kind {
                    assert_eq!(tier, DurabilityTier::Local);
                    Some(postcard::from_bytes(&rows).expect("S1 ABI rows decode"))
                } else {
                    None
                }
            })
            .expect("ABI read emits rows")
    }

    fn drain(&mut self, request_id: u64) -> Vec<Event> {
        drain_abi_events(&mut self.runtime, self.runtime_handle, request_id)
    }
...
fn drain_abi_events(
    ...
    let AbiCallResult::Events { events } = runtime.events_poll(
        ...
        panic!("ABI event poll should return events");
    };
    events
}
...
fn assert_db_query_matches_oracle(
    ...
    assert_eq!(
        row_set(db.read(query).expect("db read")),
        oracle,
        "{label} read mismatch"
    );
    assert_eq!(
        row_set(block_on(db.all(query, ReadOpts::default())).expect("db all")),
        oracle,
        "{label} all mismatch"
    );
    let issues = schema
...
fn abi_row_set(batches: &[AbiRowBatch]) -> BTreeSet<(String, RowUuid)> {
    batches
        .iter()
        .flat_map(|batch| {
            batch
                .rows
                .iter()
                .map(move |row| (batch.table.clone(), row.row_id))
        })
        .collect()
}
...
fn emit_abi_direct_surface_summary(config: &Config, summary: &AbiDirectSurfaceSummary) {
    let mut fields = metadata_fields("s1_saas", "abi-runtime", config.seed, &config.profile);
    fields.insert("phase".to_owned(), json!("abi_direct_surface"));
    fields.insert("elapsed_us".to_owned(), json!(summary.elapsed_us));
    fields.insert("prepare_us".to_owned(), json!(summary.prepare_us));
    fields.insert("write_us".to_owned(), json!(summary.write_us));
    fields.insert("read_us".to_owned(), json!(summary.read_us));
    fields.insert("watch_us".to_owned(), json!(summary.watch_us));
    fields.insert("row_encode_us".to_owned(), json!(summary.row_encode_us));
    fields.insert(
        "prepared_queries".to_owned(),
        json!(summary.prepared_queries),
    );
    fields.insert("read_rows".to_owned(), json!(summary.read_rows));
    fields.insert("write_verbs".to_owned(), json!(summary.write_verbs));
    fields.insert(
        "subscription_delta_added".to_owned(),
        json!(summary.subscription_delta_added),
    );
    fields.insert(
        "subscription_delta_updated".to_owned(),
        json!(summary.subscription_delta_updated),
    );
    fields.insert(
        "subscription_delta_removed".to_owned(),
        json!(summary.subscription_delta_removed),
    );
    fields.insert(
        "encoded_row_bytes".to_owned(),
        json!(summary.encoded_row_bytes),
    );
    fields.insert(
        "encoded_write_bytes".to_owned(),
        json!(summary.encoded_write_bytes),
    );
    fields.insert(
        "encoded_write_verbs".to_owned(),
        json!(summary.encoded_write_verbs),
    );
    fields.insert(
        "encoded_probe_verbs".to_owned(),
        json!(summary.encoded_probe_verbs),
    );
    fields.insert(
        "encoded_tx_stage_verbs".to_owned(),
        json!(summary.encoded_tx_stage_verbs),
    );
    emit_object(fields);
}
