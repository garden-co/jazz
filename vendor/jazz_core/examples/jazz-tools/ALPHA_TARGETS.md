...
- Delete/restore remains a skipped gate: `restore(...).wait({ tier: "edge" })`
  succeeds and the writer sees the restored row, but a fresh websocket client
  times out querying that restored row at edge.
