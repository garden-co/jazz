...
{ parent: true } } })`, plus a depth-3 reverse include path over the direct
  ...
  subscription rows. Selected include projections, hops, and gather
  subscriptions remain query-builder gaps. The nested-forward slice is pinned by
...
6. **Close include/hop/gather semantics.** Add app-shaped checks for related
   ...
   For subscriptions, simple/nested forward includes and one depth-3 reverse
   include websocket gate work. Hop/gather subscriptions and selected include
