...
impl FrameDecoder {
    ...
    pub fn read_streaming(&mut self, output: &mut [u8]) -> usize {
        match self.state.as_mut() {
            Some(state) => state
                .decoder_scratch
                .buffer
                .read_streaming(output, state.frame_finished),
            None => 0,
        }
    }
    ...
    pub fn streaming_available(&self) -> usize {
        self.state.as_ref().map_or(0, |state| {
            state.decoder_scratch.buffer.streaming_available()
        })
    }
