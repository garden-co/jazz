...
impl BlockDecoder {
    ...
    fn decompress_block(
        ...
        check_output_limit(
            seq_section.num_sequences as usize,
            workspace.max_block_output / 3,
        )?;
        if seq_section.num_sequences != 0 {
            ...
            for sequence in &workspace.sequences {
                output_size = output_size.checked_add(sequence.ml as usize).ok_or(
                    DecompressBlockError::OutputTooLarge {
                        requested: usize::MAX,
                        limit: workspace.max_block_output,
                    },
                )?;
